...
function createChildProcessLogger({ logger, root }) {
	...
	return async ({ cmd, args, env }) => {
		try {
			...
			const subprocess = (0, child_process.spawn)(cmd, args, {
				cwd: root,
				shell: true,
				env,
				stdio: [
...
var Deps = class extends _mastra_core_base.MastraBase {
	...
	findLockFile(dir) {
		for (const file of [
			...
			"yarn.lock",
			"bun.lock"
		]) if (fs.default.existsSync(path.default.join(dir, file))) return file;
		const parentDir = path.default.resolve(dir, "..");
	...
	findPnpmWorkspaceFile(dir) {
		const workspaceYamlPath = path.default.join(dir, "pnpm-workspace.yaml");
		if (fs.default.existsSync(workspaceYamlPath)) return workspaceYamlPath;
		const parentDir = path.default.resolve(dir, "..");
	...
	async writePnpmConfig(dir, options = {}) {
		...
		await fs_promises.default.writeFile(path.default.join(dir, "pnpm-workspace.yaml"), copyPnpmWorkspaceSettings(sourceWorkspaceYaml, options), "utf-8");
	}
	...
	*/
	...
	async checkDependencies(dependencies) {
		try {
			const packageJsonPath = path.default.join(this.rootDir, "package.json");
			try {
				await fs_promises.default.access(packageJsonPath);
			} catch {
				return "No package.json file found in the current directory";
			}
			const packageJson = await (0, fs_extra_esm.readJSON)(packageJsonPath);
	...
	async getProjectName() {
		try {
			return (await (0, fs_extra_esm.readJSON)(path.default.join(this.rootDir, "package.json"))).name;
		} catch (err) {
			throw err;
		}
	}
	async getPackageVersion() {
		...
		return (await (0, fs_extra_esm.readJSON)(path.default.join(__dirname, "..", "..", "package.json"))).version;
	}
	async addScriptsToPackageJson(scripts) {
		const packageJson = await (0, fs_extra_esm.readJSON)("package.json");
		packageJson.scripts = {
			...packageJson.scripts,
			...scripts
		};
		await (0, fs_extra_esm.writeJSON)("package.json", packageJson, { spaces: 2 });
	}
};
...
var FileService = class {
	...
	*/
	async copyStarterFile(inputFile, outputFilePath, replaceIfExists) {
		const __filename = (0, url.fileURLToPath)(require("url").pathToFileURL(__filename).href);
		const __dirname = path.default.dirname(__filename);
		const filePath = path.default.resolve(__dirname, "..", "starter-files", inputFile);
		const fileString = fs.default.readFileSync(filePath, "utf8");
		if (fs.default.existsSync(outputFilePath) && !replaceIfExists) {
			console.info(`${outputFilePath} already exists`);
			return false;
		}
		await fs_extra_esm.default.outputFile(outputFilePath, fileString);
		return true;
	}
	async setupEnvFile({ dbUrl }) {
		const envPath = path.default.join(process.cwd(), ".env.development");
		await fs_extra_esm.default.ensureFile(envPath);
		await new FileEnvService(envPath).setEnvValue("DB_URL", dbUrl);
	}
	getFirstExistingFile(files) {
		for (const f of files) if (fs.default.existsSync(f)) return f;
	...
	*/
	...
	replaceValuesInFile({ filePath, replacements }) {
		let fileContent = fs.default.readFileSync(filePath, "utf8");
...
Object.defineProperty(exports, "createPinoStream", {
	enumerable: true,
	get: function() {
		return createPinoStream;
	}
});

//# sourceMappingURL=services-DrT0fgoG.cjs.map