...
require("../index-CLddUTqr-C8lMvBz9.cjs");
let fs = require("fs");
...
//#endregion
//#region ../../../../../setup-pnpm/node_modules/.bin/store/v11/links/@hono/swagger-ui/0.5.3/e2a67d786252daef9d3d1d955111365f4d3b8b4c3d7bbe3cd9046cc5c088d984/node_modules/@hono/swagger-ui/dist/index.js
const RENDER_TYPE = {
...
//#endregion
//#region ../../../../../setup-pnpm/node_modules/.bin/store/v11/links/@/hono-openapi/1.3.1/053f98053f08198fa5b6e0d29547d4cd5182ae4dbab8de92581c6160e3026d7d/node_modules/hono-openapi/dist/index.js
...
async function createHonoServer(mastra, options = { tools: {} }) {
	...
	const browserStreamSetup = options.browserStream === false ? null : await setupBrowserStream(app, {
		getToolset: async (agentId) => {
			try {
				...
				if (runtimeAgent) return runtimeAgent.browser;
			} catch {}
			try {
				return (await mastra.getEditor?.()?.agent.getById(agentId))?.browser;
			} catch {
				return;
			}
		},
	...
	if (options?.studio) {
		app.get(`${studioBasePath}/refresh-events`, describeRoute({ hide: true }), handleClientsRefresh);
		app.post(`${studioBasePath}/__refresh`, describeRoute({ hide: true }), handleTriggerClientsRefresh);
		app.get(`${studioBasePath}/__hot-reload-status`, describeRoute({ hide: true }), (c) => {
			return c.json({
...
async function createNodeServer(mastra, options = { tools: {} }) {
	...
	const shutdown = async (signal) => {
		...
		if (typeof lifecycle.shutdown === "function") {
			...
			try {
				if (await Promise.race([lifecycle.shutdown(), timeoutPromise]) === timedOut) logger.warn("Mastra shutdown timed out; forcing exit", { timeoutMs: SHUTDOWN_TIMEOUT_MS });
			} catch (error) {
