...
const require_analyze = require("./analyze-D2KHXb6M.cjs");
const require_utils = require("./utils-O98BGdnP.cjs");
require("./services-DrT0fgoG.cjs");
const require_bundler = require("./bundler-CsD_DAiK.cjs");
...
function workspaceDepsWatcher(options) {
	...
	return {
		...
		async buildStart() {
			...
			for (const file of sourceFiles) this.addWatchFile((0, path.resolve)(file));
			if (isFirstBuild) {
				isFirstBuild = false;
				return;
			}
			...
			const { externalsPreset, mergedExternals } = require_analyze.normalizeExternals(bundlerOptions?.externals ?? true);
			await require_analyze.bundleExternals(depsCopy, outputDir, {
				bundlerOptions: {
					externalsPreset,
					mergedExternals,
					isDev: true
...
function collectUsedPackageRoots(depsToOptimize, workspaceMap) {
	...
	for (const [dep, metadata] of depsToOptimize.entries()) {
		...
		if (metadata.rootPath) {
			roots.add(require_utils.slash(metadata.rootPath));
			continue;
		}
		const pkgName = require_utils.getPackageName(dep);
		const location = pkgName ? workspaceMap.get(pkgName)?.location : void 0;
		if (location) roots.add(require_utils.slash(location));
	}
	return Array.from(roots);
}
function collectAbsoluteCachePaths(optimizedDependencyFiles, workspaceRoot) {
	return Array.from(optimizedDependencyFiles.values()).map((relativePath) => (0, path.resolve)(workspaceRoot, relativePath));
}
async function discoverWorkspaceSourceFiles(packageRoots) {
	if (packageRoots.length === 0) return [];
	return (await Promise.all(packageRoots.map((root) => (0, tinyglobby.glob)(SOURCE_GLOBS, {
		cwd: root,
		absolute: true,
...
async function isCacheMissing(cachePaths) {
	for (const cachePath of cachePaths) if (!(0, fs.existsSync)(cachePath)) return true;
	return false;
}
async function isWorkspaceOutputStale(sourceFiles, cachePaths) {
	...
	for (const cachePath of cachePaths) try {
		const { mtimeMs } = await (0, fs_promises.stat)(cachePath);
		cacheBaselineMs = Math.max(cacheBaselineMs, mtimeMs);
	} catch {
		return true;
	}
	for (const file of sourceFiles) try {
		const { mtimeMs } = await (0, fs_promises.stat)(file);
...
async function getInputOptions(entryFile, platform, env, { sourcemap = false, bundlerOptions = {
	...
	const outputDir = path.posix.join(process.cwd(), ".mastra", ".build");
...
*/
async function parseMarkdownSchedule(path$8, key) {
	const raw = await (0, fs_promises.readFile)(path$8, "utf-8");
	let parsed;
	try {
		parsed = (0, gray_matter.default)(raw);
	} catch (error) {
		const detail = error instanceof Error ? error.message.split("\n")[0] : String(error);
		throw new Error(`Schedule "${key}" in ${path$8} has invalid YAML frontmatter: ${detail}. Cron expressions must be quoted (cron: "*/5 * * * *"), because a leading "*" is a YAML alias.`);
	}
	const frontmatter = parsed.data ?? {};
	...
	if (unknown.length > 0) {
		...
		throw new Error(`Schedule "${key}" in ${path$8} has unknown frontmatter field(s): ${unknown.join(", ")}.${hint}`);
	}
	if (!frontmatter.cron || typeof frontmatter.cron !== "string") throw new Error(`Schedule "${key}" in ${path$8} is missing a required "cron". Add a YAML frontmatter block with a "cron:" field (e.g. cron: "0 9 * * *").`);
	if (!prompt) throw new Error(`Schedule "${key}" in ${path$8} has an empty body. The document body is used as the prompt, so it cannot be blank.`);
	const definition = {
...
async function discoverSchedules(schedulesDir, prefix = "") {
	...
	let entries;
	try {
		entries = await (0, fs_promises.readdir)(schedulesDir);
	} catch {
		return [];
	}
	const schedules = [];
	for (const basename of entries.sort()) {
		const path$9 = (0, path.join)(schedulesDir, basename);
		let stats;
		try {
			stats = await (0, fs_promises.lstat)(path$9);
		} catch {
			continue;
		}
		if (stats.isSymbolicLink()) continue;
		if (stats.isDirectory()) {
			schedules.push(...await discoverSchedules(path$9, `${prefix}${basename}/`));
			continue;
		}
		if (isTestFile(basename)) continue;
		if (SCHEDULE_MODULE_EXTENSIONS.some((ext) => basename.endsWith(ext))) {
			schedules.push({
				...
				path: require_utils.slash(path$9)
			});
			continue;
		}
		if (basename.endsWith(".md")) {
			...
			schedules.push({
				...
				key,
				path: require_utils.slash(path$9),
				definition: await parseMarkdownSchedule(path$9, key)
			});
		}
	}
...
async function discoverAgentDir(dir, name, depth, onWarn) {
	...
	const schedules = await discoverSchedules((0, path.join)(dir, "schedules"));
...
async function discoverFsWorkflows(mastraDir) {
	...
	for (const basename of entries.sort()) {
		if (isTestFile(basename)) continue;
		if (!TOOL_EXTENSIONS.some((ext) => basename.endsWith(ext))) continue;
		const path$10 = (0, path.join)(workflowsDir, basename);
		const stats = await (0, fs_promises.lstat)(path$10);
		if (stats.isSymbolicLink() || stats.isDirectory()) continue;
		const source = await (0, fs_promises.readFile)(path$10, "utf-8");
		if (!/\bexport\s+default\b/.test(source)) continue;
		discovered.push({
			key: toolKey(basename),
			path: require_utils.slash(path$10)
		});
	}
	return discovered;
}
...
//# sourceMappingURL=build-Hzj7Fw8y.cjs.map