...
function routeToHandler(handler, event, logger) {
	try {
		switch (event.type) {
			...
			case _mastra_core_observability.TracingEventType.SPAN_ENDED: return catchAsyncResult((handler.onTracingEvent ? handler.onTracingEvent.bind(handler) : handler.exportTracingEvent.bind(handler))(event), handler.name, "tracing", logger);
			case "log":
				if (handler.onLogEvent) return catchAsyncResult(handler.onLogEvent(event), handler.name, "log", logger);
				break;
			case "metric":
				if (handler.onMetricEvent) return catchAsyncResult(handler.onMetricEvent(event), handler.name, "metric", logger);
				break;
			case "score":
				if (handler.onScoreEvent) return catchAsyncResult(handler.onScoreEvent(event), handler.name, "score", logger);
				if (handler.addScoreToTrace) {
					const score = event.score;
					if (!score.traceId) break;
					return catchAsyncResult(handler.addScoreToTrace({
					...
					}), handler.name, "score", logger);
				}
				break;
			case "feedback":
				if (handler.onFeedbackEvent) return catchAsyncResult(handler.onFeedbackEvent(event), handler.name, "feedback", logger);
				break;
		}
	} catch (err) {
...
function routeDropToHandler(handler, event, logger) {
	try {
		if (handler.onDroppedEvent) return catchAsyncResult(handler.onDroppedEvent(event), handler.name, "drop", logger);
	} catch (err) {
		logger.error(`[Observability] Handler error [handler=${handler.name}]:`, err);
	}
}
...
function catchAsyncResult(result, handlerName, signal, logger) {
	if (result && typeof result.then === "function") return result.catch((err) => {
		logger.error(`[Observability] ${signal} handler error [handler=${handlerName}]:`, err);
	});
}
//#endregion
//#region src/bus/base.ts
/**
...
*/
var BaseObservabilityEventBus = class BaseObservabilityEventBus extends _mastra_core_base.MastraBase {
	subscribers = /* @__PURE__ */ new Set();
	/** In-flight async subscriber promises. Self-cleaning via .finally(). */
	pendingSubscribers = /* @__PURE__ */ new Set();
	constructor({ name } = {}) {
		super({
			component: _mastra_core_logger.RegisteredLogger.OBSERVABILITY,
			name: name ?? "EventBus"
		});
	}
	/**
	...
	emit(event) {
		for (const handler of this.subscribers) try {
			...
			if (result && typeof result.then === "function") {
				...
				promise.finally(() => this.pendingSubscribers.delete(promise));
			}
		} catch (err) {
			this.logger.error("[ObservabilityEventBus] Handler error:", err);
		}
	...
	async flush() {
		...
		while (this.pendingSubscribers.size > 0) {
			...
			if (iterations >= BaseObservabilityEventBus.MAX_FLUSH_ITERATIONS) {
				this.logger.error(`[ObservabilityEventBus] flush() exceeded ${BaseObservabilityEventBus.MAX_FLUSH_ITERATIONS} drain iterations — ${this.pendingSubscribers.size} promises still pending. Handlers may be re-emitting during flush.`);
				break;
			}
		}
	}
...
//#endregion
//#region src/spans/serialization.ts
/**
...
function getMapKeyType(key) {
	...
	if (key instanceof Date) return "date";
...
function restoreSerializedMapKey(keyType, key) {
	switch (keyType) {
		...
		case "bigint": return typeof key === "string" && key.endsWith("n") ? BigInt(key.slice(0, -1)) : key;
		case "date": return typeof key === "string" ? new Date(key) : key;
		default: return key;
	}
}
...
function reconstructSerializedMap(value) {
	return new Map(value.__map_entries.map(([keyType, key, mapValue]) => [restoreSerializedMapKey(keyType, key), mapValue]));
}
/**
...
function isJsonSchema(val) {
	...
	if (val.$schema && typeof val.$schema === "string" && val.$schema.includes("json-schema")) return true;
	if (val.type === "object" && val.properties && typeof val.properties === "object") return true;
	return false;
}
/**
	...
	function helper(val, depth) {
		...
		try {
			if (val instanceof Error) {
				...
				if (typeof errorStack === "string") cleanedError.stack = truncateString(errorStack, maxStringLength);
				else if (errorStack !== void 0) cleanedError.stack = errorStack;
				if (causeReadFailed) cleanedError.cause = rawCause;
				else if (rawCause !== void 0) try {
			...
			if (val instanceof Map) {
				...
				if (omittedMapEntries > 0) cleanedMap.__truncated = `${omittedMapEntries} more keys omitted`;
				return cleanedMap;
			}
			if (val instanceof Set) {
				...
				if (totalSetSize > maxArrayLength) cleanedSet.push(`[…${totalSetSize - maxArrayLength} more items]`);
				return cleanedSet;
			}
			if (Array.isArray(val)) {
				const cleaned = [];
				for (let i = 0; i < Math.min(val.length, maxArrayLength); i++) try {
					cleaned.push(helper(val[i], depth + 1));
				} catch (error) {
					cleaned.push(formatSerializationError(error));
				}
				if (val.length > maxArrayLength) cleaned.push(`[…${val.length - maxArrayLength} more items]`);
				return cleaned;
			}
			if (typeof Buffer !== "undefined" && Buffer.isBuffer(val)) return `[Buffer length=${val.length}]`;
			if (ArrayBuffer.isView(val)) return `[${val.constructor?.name ?? "TypedArray"} byteLength=${val.byteLength ?? "?"}]`;
			...
			if (typeof serializeForSpan === "function") try {
...
//#endregion
//#region src/bus/observability-bus.ts
/**
...
function cleanEvent(event, options) {
	switch (event.type) {
		case "log": return {
			type: "log",
			log: deepClean(event.log, options)
		};
		case "metric": return {
			type: "metric",
			metric: deepClean(event.metric, options)
		};
		case "score": return {
			type: "score",
			score: deepClean(event.score, options)
		};
		case "feedback": return {
			type: "feedback",
			feedback: deepClean(event.feedback, options)
		};
...
var ObservabilityBus = class extends BaseObservabilityEventBus {
	...
	*/
	registerBridge(bridge) {
		if (this.bridge) this.logger.warn(`[ObservabilityBus] Replacing existing bridge with new bridge`);
		this.bridge = bridge;
	}
	/**
	...
	*/
	emit(event) {
		...
		for (const exporter of this.exporters) this.trackPromise(routeToHandler(exporter, cleaned, this.logger));
		if (this.bridge) this.trackPromise(routeToHandler(this.bridge, cleaned, this.logger));
		super.emit(cleaned);
	}
	/**
	...
	*/
	emitDropEvent(event) {
		if (this.handlerBufferFlushDepth > 0) this.dropEventsEmittedDuringHandlerFlush++;
		for (const exporter of this.exporters) this.trackPromise(routeDropToHandler(exporter, event, this.logger));
		if (this.bridge) this.trackPromise(routeDropToHandler(this.bridge, event, this.logger));
	}
	/**
	...
	*/
	...
	async drainPendingHandlers() {
		...
		while (this.pendingHandlers.size > 0) {
			...
			if (iterations >= MAX_FLUSH_ITERATIONS) {
				this.logger.error(`[ObservabilityBus] flush() exceeded ${MAX_FLUSH_ITERATIONS} drain iterations — ${this.pendingHandlers.size} promises still pending. Handlers may be re-emitting during flush.`);
				if (this.pendingHandlers.size > 0) await Promise.allSettled([...this.pendingHandlers]);
				break;
			}
		}
	}
	...
	async flushHandlerBuffers() {
		...
		try {
			...
			if (this.bridge) bufferFlushPromises.push(this.bridge.flush());
			if (bufferFlushPromises.length > 0) await Promise.allSettled(bufferFlushPromises);
	...
	*/
	async flush() {
		...
		for (let iterations = 0; iterations < MAX_FLUSH_ITERATIONS; iterations++) {
			if (!await this.flushHandlerBuffers() && this.pendingHandlers.size === 0) return;
			await this.drainPendingHandlers();
			await super.flush();
		}
		this.logger.error(`[ObservabilityBus] flush() exceeded ${MAX_FLUSH_ITERATIONS} buffer drain iterations. Handlers may be emitting drop events during every flush.`);
	}
...
const functionSchema = zod_v4.z.custom((value) => typeof value === "function", { message: "Expected function" });
/**
* Zod schema for SamplingStrategy
*/
const samplingStrategySchema = zod_v4.z.discriminatedUnion("type", [
	zod_v4.z.object({ type: zod_v4.z.literal("always") }),
	zod_v4.z.object({ type: zod_v4.z.literal("never") }),
	zod_v4.z.object({
		type: zod_v4.z.literal("ratio"),
		probability: zod_v4.z.number().min(0, "Probability must be between 0 and 1").max(1, "Probability must be between 0 and 1")
	}),
	zod_v4.z.object({
...
*/
const serializationOptionsSchema = zod_v4.z.object({
	maxStringLength: zod_v4.z.number().int().positive().optional(),
	maxDepth: zod_v4.z.number().int().positive().optional(),
	maxArrayLength: zod_v4.z.number().int().positive().optional(),
	maxObjectKeys: zod_v4.z.number().int().positive().optional()
}).optional();
const LOG_LEVELS = [
	"debug",
	"info",
...
const cardinalityConfigSchema = zod_v4.z.object({
	blockedLabels: zod_v4.z.array(zod_v4.z.string()).optional(),
	blockUUIDs: zod_v4.z.boolean().optional()
}).optional();
const loggingConfigSchema = zod_v4.z.object({
	enabled: zod_v4.z.boolean().optional(),
	level: zod_v4.z.enum(LOG_LEVELS).optional()
}).optional();
const spanFilterSchema = functionSchema.optional();
const observabilityInstanceConfigFields = {
	serviceName: zod_v4.z.string().min(1, "Service name is required"),
	sampling: samplingStrategySchema.optional(),
	exporters: zod_v4.z.array(zod_v4.z.any()).optional(),
	bridge: zod_v4.z.any().optional(),
	spanOutputProcessors: zod_v4.z.array(zod_v4.z.any()).optional(),
	includeInternalSpans: zod_v4.z.boolean().optional(),
	excludeSpanTypes: zod_v4.z.array(zod_v4.z.nativeEnum(_mastra_core_observability.SpanType)).optional(),
	spanFilter: spanFilterSchema,
	requestContextKeys: zod_v4.z.array(zod_v4.z.string()).optional(),
...
*/
const observabilityInstanceConfigSchema = zod_v4.z.object({
...
*/
const observabilityConfigValueSchema = zod_v4.z.object(observabilityInstanceConfigFields).refine((data) => {
	const hasExporters = data.exporters && data.exporters.length > 0;
	const hasBridge = !!data.bridge;
	return hasExporters || hasBridge;
}, { message: "At least one exporter or a bridge is required" });
...
const observabilityRegistryConfigSchema = zod_v4.z.object({
	...
	configs: zod_v4.z.union([
		zod_v4.z.record(zod_v4.z.string(), zod_v4.z.any()),
		zod_v4.z.array(zod_v4.z.any()),
		zod_v4.z.null()
	]).optional(),
	configSelector: functionSchema.optional(),
	sensitiveDataFilter: zod_v4.z.union([zod_v4.z.boolean(), sensitiveDataFilterOptionsSchema]).optional()
...
}, { message: "Cannot specify both \"default\" (when enabled) and \"configs\". Use either default observability or custom configs, but not both." }).refine((data) => {
	if ((data.configs && typeof data.configs === "object" && !Array.isArray(data.configs) ? Object.keys(data.configs).length : 0) > 1 && !data.configSelector) return false;
	return true;
}, { message: "A \"configSelector\" function is required when multiple configs are specified to determine which config to use." }).refine((data) => {
	...
	*/
	log(level, message, data) {
		...
		const event = {
			type: "log",
			log: {
				logId: (0, _mastra_core_observability.generateSignalId)(),
	...
	*/
	...
	emit(name, value, labels, options) {
		...
		const event = {
			type: "metric",
			metric: {
				metricId: (0, _mastra_core_observability.generateSignalId)(),
				timestamp: /* @__PURE__ */ new Date(),
	...
	counter(name) {
		return { add: (value, additionalLabels) => {
			this.emit(name, value, additionalLabels);
		} };
	}
	/** @deprecated Use `emit()` instead. */
	gauge(name) {
		return { set: (value, additionalLabels) => {
			this.emit(name, value, additionalLabels);
		} };
	}
	/** @deprecated Use `emit()` instead. */
	histogram(name) {
		return { record: (value, additionalLabels) => {
			this.emit(name, value, additionalLabels);
		} };
	}
};
...
var PricingTier = class {
	...
	matchesUsage(usage) {
		if (!this.when || this.when.length === 0) return true;
	...
	matchesCondition(condition, usage) {
		...
		switch (condition.op) {
			case "gt": return left > condition.value;
			case "gte": return left >= condition.value;
			case "lt": return left < condition.value;
			case "lte": return left <= condition.value;
	...
	getConditionFieldValue(field, usage) {
		switch (field) {
			case "total_input_tokens": return typeof usage.inputTokens === "number" ? usage.inputTokens : null;
			default: return null;
		}
	}
};
var PricingModel = class {
	...
	getPricingTierForUsage(usage) {
		for (const tier of this.tiers) if (tier.when && tier.when.length > 0 && tier.matchesUsage(usage)) return tier;
...
//#endregion
//#region src/metrics/pricing-registry.ts
const DATA_FILE_NAME = "pricing-data.jsonl";
const BEDROCK_GEOGRAPHY_PREFIXES = /* @__PURE__ */ new Set([
	"global",
	"us",
	"eu",
	"apac",
...
const AI_SDK_PROVIDER_NAMESPACE_ALIASES = /* @__PURE__ */ new Map([
	["google.vertex", "google-vertex"],
	["vertex.anthropic", "google-vertex-anthropic"],
	["vertex.maas", "google-vertex"]
]);
const MINIFIED_METER_TO_CANONICAL = {
	it: "input_tokens",
	ot: "output_tokens",
...
const MINIFIED_CONDITION_FIELD_TO_CANONICAL = { tit: "total_input_tokens" };
let cachedLoadError = null;
var PricingRegistry = class PricingRegistry {
	pricingModels;
	...
	static getGlobal() {
		if (PricingRegistry.globalRegistry) return PricingRegistry.globalRegistry;
...
function loadPricingModels() {
	...
	try {
		return parsePricingModelText(fs.default.readFileSync(resolvePricingModelPath(), "utf-8"));
	} catch (error) {
...
function parsePricingModelText(content) {
	...
	for (const line of content.split("\n")) {
		...
		if (!trimmed) continue;
		const pricingModel = expandPricingModelRow(JSON.parse(trimmed));
...
function expandPricingModelRow(row) {
	return new PricingModel({
		...
		schema: row.s.v,
		currency: row.s.d.u,
		tiers: row.s.d.t.map((tier, index) => new PricingTier({
			index,
...
function resolvePricingModelPath() {
	...
	const candidates = [
		path.default.join(packageRoot, "dist", "metrics", DATA_FILE_NAME),
		path.default.join(packageRoot, "src", "metrics", DATA_FILE_NAME),
		path.default.join(process.cwd(), "observability", "mastra", "src", "metrics", DATA_FILE_NAME),
		path.default.join(process.cwd(), "src", "metrics", DATA_FILE_NAME)
	];
...
function getPackageRoot() {
	try {
		const packageJsonPath = (0, module$1.createRequire)(require("url").pathToFileURL(__filename).href || "file://").resolve("@mastra/observability/package.json");
...
function getNamespacedProviderCandidates(provider) {
	for (const [providerNamespace, pricingProvider] of AI_SDK_PROVIDER_NAMESPACE_ALIASES) if (matchesProviderNamespace(provider, providerNamespace)) return [providerNamespace, pricingProvider];
	return null;
}
...
//#endregion
//#region src/metrics/estimator.ts
function estimateCosts(args, pricingRegistry = PricingRegistry.getGlobal()) {
	const { provider, model, usage } = args;
	const results = /* @__PURE__ */ new Map();
	const pricingModel = pricingRegistry?.get({
		provider,
		model
	});
	if (!pricingModel) {
		applyErrorContextForUsage(results, usage, {
...
function setAggregateCostContext(args) {
	...
	if (successfulDetailCosts.length > 0) {
		...
		results.set(totalMetric, {
			...
			costMetadata: hasFailedDetailCost ? {
				...costMetadata,
				error: "partial_cost"
			} : { ...costMetadata }
		});
...
function estimateCostForMeter(args) {
	...
	if (typeof pricePerUnit !== "number") return {
		success: false,
		costContext: {
			...costContext,
			costMetadata: {
...
//#endregion
//#region src/metrics/auto-extract.ts
/**
...
function emitDurationMetrics(span, metrics) {
	...
	if (!durationMetricName || !span.startTime || !span.endTime) return;
	const durationMs = span.endTime.getTime() - span.startTime.getTime();
	metrics.emit(durationMetricName, durationMs, { status: span.errorInfo ? "error" : "ok" });
}
/** Emit token usage metrics for a model-generation span. */
function emitTokenMetrics(span, metrics) {
	if (span.type !== _mastra_core_observability.SpanType.MODEL_GENERATION) return;
	const attrs = span.attributes;
	if (!attrs?.usage) return;
...
function emitUsageMetrics(attrs, usage, metrics) {
	...
	if (providedCostContext) metricCosts = providedCostContext;
		...
		if (provider && model) {
			...
			if (isNoMatchingModelResult(metricCosts) && attrs.model && attrs.model !== model) {
				...
				if (!isNoMatchingModelResult(configuredModelCosts)) metricCosts = configuredModelCosts;
			}
		}
	} catch {
	...
	for (const sample of getTokenMetricSamples(usage)) emit(sample.name, sample.value);
}
...
function getDurationMetricName(span) {
	switch (span.type) {
		...
		case _mastra_core_observability.SpanType.PROVIDER_TOOL_CALL: return "mastra_tool_duration_ms";
		case _mastra_core_observability.SpanType.CLIENT_TOOL_CALL: return null;
		case _mastra_core_observability.SpanType.WORKFLOW_RUN: return "mastra_workflow_duration_ms";
		case _mastra_core_observability.SpanType.MODEL_GENERATION: return "mastra_model_duration_ms";
		case _mastra_core_observability.SpanType.PROCESSOR_RUN: return "mastra_processor_duration_ms";
		default: return null;
	}
}
//#endregion
//#region src/metrics/cardinality.ts
...
var CardinalityFilter = class {
	...
	*/
	constructor(config) {
		const blocked = config?.blockedLabels ?? [..._mastra_core_observability.DEFAULT_BLOCKED_LABELS];
	...
	*/
	filterLabels(labels) {
		...
		for (const [key, value] of Object.entries(labels)) {
			if (this.blockedLabels.has(key.toLowerCase())) continue;
			if (this.blockUUIDs && UUID_REGEX.test(value)) continue;
			filtered[key] = value;
		}
		return filtered;
	}
...
function extractUsageMetrics(usage, providerMetadata) {
	...
	if (google?.usageMetadata) {
		if (!isDefined(inputDetails.cacheRead) && isDefined(google.usageMetadata.cachedContentTokenCount)) inputDetails.cacheRead = google.usageMetadata.cachedContentTokenCount;
		if (isDefined(google.usageMetadata.thoughtsTokenCount)) outputDetails.reasoning = google.usageMetadata.thoughtsTokenCount;
	}
	if (isDefined(inputTokens)) inputDetails.text = Math.max(0, inputTokens - sumDefinedValues(inputDetails, [
		"cacheRead",
		"cacheWrite",
		"audio",
		"image"
	]));
	if (isDefined(outputTokens)) outputDetails.text = Math.max(0, outputTokens - sumDefinedValues(outputDetails, [
		"reasoning",
		"audio",
		"image"
	]));
	...
	if (Object.keys(inputDetails).length > 0) result.inputDetails = inputDetails;
	if (Object.keys(outputDetails).length > 0) result.outputDetails = outputDetails;
	return result;
}
...
function summarizePart(part) {
	...
	if ("parts" in part && Array.isArray(part.parts)) return part.parts.map(summarizePart).filter(Boolean).join("");
	if ("inlineData" in part && part.inlineData && typeof part.inlineData === "object") return `[${formatPreviewLabel(part.inlineData.mimeType, "binary")}]`;
	if ("image_url" in part) return "[image]";
	if ("functionCall" in part && part.functionCall && typeof part.functionCall === "object") return `[tool: ${formatPreviewLabel(part.functionCall.name, "unknown")}]`;
	if ("function_call" in part && part.function_call && typeof part.function_call === "object") return `[tool: ${formatPreviewLabel(part.function_call.name, "unknown")}]`;
	if ("function" in part && part.function && typeof part.function === "object") return `[tool: ${formatPreviewLabel(part.function.name, "unknown")}]`;
	if ("toolName" in part && !("type" in part && part.type === "tool-result")) return `[tool: ${formatPreviewLabel(part.toolName, "unknown")}]`;
	if ("type" in part && typeof part.type === "string") switch (part.type) {
		case "image": return "[image]";
		case "file": return "[file]";
		case "reasoning": return "[reasoning]";
		case "tool-call": return `[tool: ${formatPreviewLabel(part.toolName, "unknown")}]`;
		case "tool-result": {
			...
			if (mastraMeta?.modelOutput !== void 0) return formatToolResultPreviewValue(mastraMeta.modelOutput);
...
function summarizeMessageContent(content) {
	...
	if (Array.isArray(content)) return content.map(summarizePart).filter(Boolean).join("");
	if (content && typeof content === "object") {
		if ("parts" in content && Array.isArray(content.parts)) return content.parts.map(summarizePart).filter(Boolean).join("");
...
function normalizeMessages(messages) {
	return messages.map((message) => {
		...
		const contentWithToolArrays = appendToolPreview(appendToolPreview(summarizeMessageContent(message.content), message.toolCalls), message.tool_calls);
		...
		return {
			role,
			content: appendPreview(appendPreview(contentWithToolArrays, functionCallPreview), functionCallSnakeCasePreview)
		};
	});
}
function summarizeRequestBody(body) {
	if (body == null) return;
	if (typeof body !== "object") return typeof body === "string" ? body : String(body);
	if (Array.isArray(body.messages)) return normalizeMessages(body.messages);
	if (Array.isArray(body.input)) return normalizeMessages(body.input);
	if (Array.isArray(body.contents)) return body.contents.map((item) => ({
	...
	if (bodyKeys.length > 0) summary.keys = bodyKeys;
...
function extractStepInput(payload) {
	if (Array.isArray(payload?.inputMessages)) return normalizeMessages(payload.inputMessages);
	const request = payload?.request;
	if (!request) return void 0;
	const { body } = request;
	if (body == null) return request;
	try {
		return summarizeRequestBody(typeof body === "string" ? JSON.parse(body) : body);
	} catch {
		return request;
	}
}
...
var ModelSpanTracker = class {
	...
	*/
	#captureCompletionStartTime() {
		if (this.#completionStartTime) return;
	...
	*/
	getTracingContext() {
		return { currentSpan: this.#currentStepSpan ?? this.#modelSpan };
	}
	/**
	...
	*/
	reportGenerationError(options) {
		if (this.#currentInferenceSpan) {
			this.#currentInferenceSpan.error({
				error: options.error,
				endSpan: true
			});
	...
	*/
	startStep(payload) {
		...
		this.#currentStepSpan = this.#modelSpan?.createChildSpan({
			...
			type: _mastra_core_observability.SpanType.MODEL_STEP,
			attributes: {
	...
	*/
	startInference(payload) {
		if (!supportsModelInference()) return;
		if (!this.#currentStepSpan || this.#currentInferenceSpan) return;
		const input = extractStepInput(payload);
		...
		this.#currentInferenceSpan = this.#currentStepSpan.createChildSpan({
			...
			type: _mastra_core_observability.SpanType.MODEL_INFERENCE,
			attributes: {
				stepIndex: this.#stepIndex,
	...
	*/
	updateStep(payload) {
		if (!this.#currentStepSpan || !payload) return;
		...
		if (hasFinalInput) this.#currentStepInputIsFinal = true;
	}
	/**
	...
	*/
		...
		if (cleanMetadata) for (const key of [
			"request",
			"id",
			"timestamp",
			"modelId",
	...
	*/
		...
		if (!this.#currentStepSpan) this.startStep();
		if (!this.#currentInferenceSpan) this.startInference();
	}
	/**
	...
	*/
	#appendToAccumulator(field, text) {
		if (this.#accumulator[field] === void 0) this.#accumulator[field] = text;
		else this.#accumulator[field] += text;
	}
	/**
	...
	*/
		...
		this.#currentChunkSpan.end({ output: output !== void 0 ? output : this.#accumulator });
	...
	*/
	#createEventSpan(chunkType, output, options) {
		this.#ensureStepAndInference();
		if (this.#chunkParent()?.createEventSpan({
			name: `chunk: '${chunkType}'`,
			type: _mastra_core_observability.SpanType.MODEL_CHUNK,
			attributes: {
				chunkType,
	...
	#handleTextChunk(chunk) {
		switch (chunk.type) {
			case "text-start":
				...
				break;
			case "text-delta":
				if (this.#currentChunkType !== "text") this.#startChunkSpan("text");
	...
	#handleReasoningChunk(chunk) {
		switch (chunk.type) {
			...
			case "reasoning-delta":
				if (this.#currentChunkType !== "reasoning") this.#startChunkSpan("reasoning");
				this.#appendToAccumulator("text", chunk.payload.text);
				break;
			case "reasoning-end":
				this.#endChunkSpan();
	...
	*/
	#handleObjectChunk(chunk) {
		switch (chunk.type) {
			case "object":
				if (this.#currentChunkType !== "object") this.#startChunkSpan("object");
				break;
			case "object-result":
	...
	*/
	wrapStream(stream) {
		return stream.pipeThrough(new stream_web.TransformStream({ transform: (chunk, controller) => {
			switch (chunk.type) {
				case "text-delta":
				case "tool-call-delta":
				case "reasoning-delta":
			...
			switch (chunk.type) {
				...
				case "object-result":
					...
					break;
				case "step-start":
					if (this.#currentStepSpan) this.updateStep(chunk.payload);
					else this.startStep(chunk.payload);
					if (!this.#currentInferenceSpan) this.startInference(chunk.payload);
					break;
				case "step-finish":
					...
					} else this.#endStepSpan(chunk.payload);
					break;
				case "raw":
				case "start":
				case "finish":
				...
				case "step-output": break;
				case "tool-call-approval":
					this.#handleToolApprovalChunk(chunk);
					break;
				case "tool-output": break;
				case "tool-result": {
					const { toolCallId, toolName, isError, dynamic, providerExecuted, providerMetadata, result, args: _args } = chunk.payload || {};
					const metadata = {
						toolCallId,
						toolName
					};
...
function isSpanInternal(spanType, flags) {
	if (flags === void 0 || flags === _mastra_core_observability.InternalSpans.NONE) return false;
	switch (spanType) {
		case _mastra_core_observability.SpanType.WORKFLOW_RUN:
		case _mastra_core_observability.SpanType.WORKFLOW_STEP:
		case _mastra_core_observability.SpanType.WORKFLOW_CONDITIONAL:
		case _mastra_core_observability.SpanType.WORKFLOW_CONDITIONAL_EVAL:
		case _mastra_core_observability.SpanType.WORKFLOW_PARALLEL:
		case _mastra_core_observability.SpanType.WORKFLOW_LOOP:
		case _mastra_core_observability.SpanType.WORKFLOW_SLEEP:
		case _mastra_core_observability.SpanType.WORKFLOW_WAIT_EVENT: return (flags & _mastra_core_observability.InternalSpans.WORKFLOW) !== 0;
		case _mastra_core_observability.SpanType.AGENT_RUN: return (flags & _mastra_core_observability.InternalSpans.AGENT) !== 0;
		case _mastra_core_observability.SpanType.TOOL_CALL:
		case _mastra_core_observability.SpanType.MCP_TOOL_CALL:
		case _mastra_core_observability.SpanType.PROVIDER_TOOL_CALL: return (flags & _mastra_core_observability.InternalSpans.TOOL) !== 0;
		case _mastra_core_observability.SpanType.MODEL_GENERATION:
		case _mastra_core_observability.SpanType.MODEL_STEP:
		case _mastra_core_observability.SpanType.MODEL_INFERENCE:
		case _mastra_core_observability.SpanType.MODEL_CHUNK: return (flags & _mastra_core_observability.InternalSpans.MODEL) !== 0;
		default: return false;
	}
}
/**
...
var BaseSpan = class {
	...
	*/
	createTracker() {
		if (this.type !== _mastra_core_observability.SpanType.MODEL_GENERATION) return;
	...
	*/
	...
	getCorrelationContext() {
		if (this.correlationContext) return this.correlationContext;
		...
		let rootSpan = this;
		while (rootSpan.parent) rootSpan = rootSpan.parent;
...
function getOrCreateTraceId(options) {
	if (options.traceId) if (isValidTraceId(options.traceId)) return options.traceId;
	else console.error(`[Mastra Tracing] Invalid traceId: must be 1-32 hexadecimal characters, got "${options.traceId}". Generating new trace ID.`);
	return generateTraceId();
}
//#endregion
//#region src/spans/no-op.ts
...
*/
var BaseObservabilityInstance = class extends _mastra_core_base.MastraBase {
	config;
	/**
	...
	*/
	...
	constructor(config) {
		...
		this.config = {
			...
			name: config.name,
			sampling: config.sampling ?? { type: "always" },
		...
		this.observabilityBus = new ObservabilityBus({ serializationOptions: this.config.serializationOptions });
		for (const exporter of this.exporters) this.observabilityBus.registerExporter(exporter);
		if (this.config.bridge) this.observabilityBus.registerBridge(this.config.bridge);
		if (this.config.bridge?.init) this.config.bridge.init({ config: this.config });
	}
	/**
	...
	*/
	__setLogger(logger) {
		...
		this.exporters.forEach((exporter) => {
			if (typeof exporter.__setLogger === "function") exporter.__setLogger(logger);
		});
		if (this.config.bridge?.__setLogger) this.config.bridge.__setLogger(logger);
		this.logger.debug(`[Observability] Initialized [service=${this.config.serviceName}] [instance=${this.config.name}] [sampling=${this.config.sampling?.type}] [bridge=${!!this.config.bridge}]`);
	}
	get exporters() {
	...
	*/
	startSpan(options) {
		...
		if (options.parent) {
			if (!options.parent.isValid) return new NoOpSpan({
				...rest,
				metadata
			}, this);
		} else if (!this.shouldSample(customSamplerOptions)) return new NoOpSpan({
			...rest,
			metadata
		}, this);
		let traceState;
		...
		if (span.isEvent) this.emitSpanEnded(span);
		else {
	...
	*/
	registerExporter(exporter) {
		...
		if (this.config.exporters.includes(exporter)) return;
		this.config.exporters.push(exporter);
		if (typeof exporter.__setLogger === "function") exporter.__setLogger(this.logger);
	}
	/**
	...
	*/
	wireSpanLifecycle(span) {
		if (!this.config.includeInternalSpans && span.isInternal && span.type !== _mastra_core_observability.SpanType.MODEL_GENERATION) return;
		...
		span.end = (options) => {
			...
			originalEnd(options);
			if (rollupTarget) this.applyUsageRollup(rollupTarget);
		...
		switch (sampling?.type) {
			...
			case "ratio":
				if (sampling.probability === void 0 || sampling.probability < 0 || sampling.probability > 1) {
					this.logger.warn(`Invalid sampling probability: ${sampling.probability}. Expected value between 0 and 1. Defaulting to no sampling.`);
					return false;
				}
				return Math.random() < sampling.probability;
			case "custom": return sampling.sampler(options);
			default: throw new Error(`Sampling strategy type not implemented: ${sampling.type}`);
		}
	}
	/**
	...
	*/
	computeTraceState(tracingOptions) {
		...
		if (allKeys.length === 0 && !hideInput && !hideOutput) return;
		return {
	...
	*/
	extractKeys(requestContext, keys) {
		...
		for (const key of keys) {
			...
			if (value !== void 0) if (parts.length > 1) {
				const nestedValue = (0, _mastra_core_utils.getNestedValue)(value, parts.slice(1).join("."));
				if (nestedValue !== void 0) (0, _mastra_core_utils.setNestedValue)(result, key, nestedValue);
			} else (0, _mastra_core_utils.setNestedValue)(result, key, value);
		}
		return result;
	}
	/**
	...
	*/
	...
	getSpanForExport(span) {
		...
		const exportedSpan = this.processSpan(span)?.exportSpan(this.config.includeInternalSpans);
		if (!exportedSpan) return void 0;
		if (this.config.spanFilter) try {
	...
	*/
	emitSpanStarted(span) {
		...
		if (exportedSpan) {
			const event = {
				type: _mastra_core_observability.TracingEventType.SPAN_STARTED,
				exportedSpan
			};
	...
	*/
	emitSpanEnded(span, excludedModelUsage) {
		...
		if (span.isValid && !(span.isInternal && !this.config.includeInternalSpans)) {
			...
			try {
				if (processedSpan) {
					...
					if (excludedModelUsage && !processedAttrs?.usage) emitTokenMetricsForUsage(excludedModelUsage.usage, excludedModelUsage.provider, excludedModelUsage.model, this.getMetricsContext(processedSpan));
				}
			} catch (err) {
		...
		if (exportedSpan) {
			const event = {
				type: _mastra_core_observability.TracingEventType.SPAN_ENDED,
				exportedSpan
			};
			this.emitTracingEvent(event);
			return;
	...
	*/
	emitSpanUpdated(span) {
		...
		if (exportedSpan) {
			const event = {
				type: _mastra_core_observability.TracingEventType.SPAN_UPDATED,
				exportedSpan
			};
			this.emitTracingEvent(event);
		}
	...
	*/
	captureModelUsageRollup(span, endOptions) {
		if (span.type !== _mastra_core_observability.SpanType.MODEL_GENERATION) return void 0;
		if (!span.isInternal || this.config.includeInternalSpans) return void 0;
	...
	*/
	captureExcludedModelUsage(span, endOptions) {
		...
		if (!this.config.excludeSpanTypes?.includes(_mastra_core_observability.SpanType.MODEL_GENERATION)) return void 0;
		const endAttrs = endOptions?.attributes ?? void 0;
		const liveAttrs = span.attributes;
		const usage = endAttrs?.usage ?? liveAttrs?.usage;
		if (!usage) return void 0;
	...
	*/
	findExportedAncestor(span) {
		...
		while (ancestor && this.isFilteredFromExport(ancestor)) ancestor = ancestor.parent;
		return ancestor;
	}
	/**
	...
	*/
	async exportTracingEvent(event) {
		const targets = [...this.exporters];
		if (this.config.bridge) targets.push(this.config.bridge);
	...
	*/
	async shutdown() {
		...
		const shutdownPromises = [...this.exporters.map((e) => e.shutdown()), ...this.spanOutputProcessors.map((p) => p.shutdown())];
		if (this.config.bridge) shutdownPromises.push(this.config.bridge.shutdown());
		if (shutdownPromises.length > 0) {
			const results = await Promise.allSettled(shutdownPromises);
			for (const result of results) if (result.status === "rejected") this.logger.error(`[Observability] Component shutdown failed [name=${this.name}]:`, result.reason);
		}
...
function nanosToDate(value) {
	if (typeof value === "number") return new Date(Math.round(value / 1e6));
...
function flattenAttributes(otlpAttrs) {
	...
	for (const attr of otlpAttrs) {
		...
		else if ("arrayValue" in value || "kvlistValue" in value) out[key] = value;
	}
	...
	for (const rl of resourceLogs) {
		...
		for (const sl of scopeLogs) {
			...
			for (const record of logRecords) {
				...
				if (!traceId) continue;
				let body = r.body;
				if (body && typeof body === "object" && "stringValue" in body) body = body.stringValue;
				out.push({
					traceId,
...
function buildExportedSpan(decoded, options = {}) {
	const errorInfo = decoded.statusCode === 2 ? { message: decoded.statusMessage ?? "Client tool span reported error status" } : void 0;
	return {
		id: decoded.spanId,
...
var ClientObservabilityProxyImpl = class {
	...
	inject(parentSpan) {
		return { traceparent: formatTraceparent(parentSpan.traceId, parentSpan.id, true) };
	}
	receive(payload, parentContext) {
		if (!payload || !payload.spans && !payload.logs && payload.executionDurationMs === void 0) return;
		let payloadBytes = 0;
		try {
		...
		if (!instance || !(instance instanceof BaseObservabilityInstance)) return;
	...
	#warn(message, data) {
		if (this.#logger) this.#logger.warn(`[ClientObservabilityProxy] ${message}`, data);
	}
};
...
function validateParentLinks(spans, rootParent) {
	...
	for (const s of spans) {
		if (!s.parentSpanId) return false;
		if (!known.has(s.parentSpanId)) return false;
	}
	return true;
}
...
var BaseExporter = class {
	...
	*/
	constructor(config = {}) {
		...
		this.logger = config.logger ?? new _mastra_core_logger.ConsoleLogger({
			level: logLevel,
	...
	*/
	resolveLogLevel(logLevel) {
		if (!logLevel) return _mastra_core_logger.LogLevel.INFO;
		if (typeof logLevel === "number") return logLevel;
		return {
			debug: _mastra_core_logger.LogLevel.DEBUG,
			info: _mastra_core_logger.LogLevel.INFO,
			warn: _mastra_core_logger.LogLevel.WARN,
			error: _mastra_core_logger.LogLevel.ERROR
		}[logLevel] ?? _mastra_core_logger.LogLevel.INFO;
	}
	/**
	...
	*/
	async applySpanFormatter(event) {
		if (this.baseConfig.customSpanFormatter) try {
...
//#endregion
//#region src/exporters/tracking.ts
/**
...
var TraceData = class {
	...
	*/
	addToWaitingQueue(args) {
		...
		if (args.waitingFor === "root") this.#waitingForRoot.push(queuedEvent);
		else {
	...
	*/
	waitingQueueSize() {
		...
		for (const queue of this.#waitingForParent.values()) count += queue.length;
		return count;
	}
	/**
	...
	*/
	getAllQueuedEvents() {
		...
		for (const queue of this.#waitingForParent.values()) all.push(...queue);
		return all;
	}
	/**
	...
	*/
	getParent(args) {
		...
		if (parentId) {
			if (this.#spans.has(parentId)) return this.#spans.get(parentId);
			if (this.#events.has(parentId)) return this.#events.get(parentId);
		}
	}
	/**
...
const DEFAULT_EARLY_QUEUE_TTL_MS = 3e4;
const DEFAULT_TRACE_CLEANUP_DELAY_MS = 3e4;
...
var TrackingExporter = class extends BaseExporter {
	...
	#scheduleProcessWaitingForRoot(traceId) {
		setImmediate(() => {
			this.#processWaitingForRoot(traceId).catch((error) => {
				this.logger.error(`${this.name}: Error processing waiting-for-root queue`, {
					error,
					traceId
				});
			});
	...
	#scheduleProcessWaitingFor(traceId, spanId) {
		setImmediate(() => {
			this.#processWaitingFor(traceId, spanId).catch((error) => {
				this.logger.error(`${this.name}: Error processing waiting queue`, {
					error,
					traceId,
					spanId
				});
	...
	*/
	async #processWaitingForRoot(traceId) {
		...
		for (const queuedEvent of queue) {
			...
			if (!await this.#tryProcessQueuedEvent(queuedEvent, traceData)) {
				const parentId = queuedEvent.event.exportedSpan.parentSpanId;
				if (parentId && traceData.isRootProcessed()) traceData.addToWaitingQueue({
					event: queuedEvent.event,
					waitingFor: parentId,
					attempts: queuedEvent.attempts,
					queuedAt: queuedEvent.queuedAt
				});
				else toKeep.push(queuedEvent);
			}
		}
		traceData.clearWaitingForRoot();
		for (const event of toKeep) traceData.addToWaitingQueue({
			event: event.event,
			waitingFor: "root",
	...
	*/
	async #processWaitingFor(traceId, spanId) {
		...
		if (queue.length === 0) return;
		this.logger.debug(`${this.name}: Processing ${queue.length} events waiting for span`, {
			traceId,
			spanId
		});
		const toKeep = [];
		...
		for (const queuedEvent of queue) {
			...
			queuedEvent.attempts++;
			if (!await this.#tryProcessQueuedEvent(queuedEvent, traceData)) toKeep.push(queuedEvent);
		}
		...
		try {
			switch (method) {
				case "handleEventSpan": {
					...
					const eventData = await this._buildEvent({
						span: exportedSpan,
						traceData
					});
					if (eventData) {
						if (!this.skipCachingEventSpans) traceData.addEvent({
							eventId: exportedSpan.id,
							eventData
						});
				...
				case "handleSpanStart": {
					...
					const spanData = await this._buildSpan({
						span: exportedSpan,
						traceData
					});
					if (spanData) {
						...
						if (exportedSpan.isRootSpan) traceData.markRootSpanProcessed();
						this.#scheduleProcessWaitingFor(exportedSpan.traceId, exportedSpan.id);
						return true;
					}
					return false;
				...
				case "handleSpanEnd":
					...
					await this._finishSpan({
						span: exportedSpan,
						traceData
					});
					if (traceData.activeSpanCount() === 0) this.#scheduleCleanup(exportedSpan.traceId);
					return true;
				default: return false;
			}
		} catch (error) {
			this.logger.error(`${this.name}: Error processing queued event`, {
				error,
				event,
				method
			});
	...
	*/
	#performCleanup(traceId) {
		...
		if (orphanedEvents.length > 0) this.logger.warn(`${this.name}: Dropping ${orphanedEvents.length} orphaned events on cleanup`, {
			traceId,
	...
	*/
		...
		if (this.#pendingCleanups.size <= this.#maxPendingCleanupTraces) return;
	...
	*/
	async #enforceHardCap() {
		if (this.#traceMap.size <= this.#maxTotalTraces || this.#hardCapEnforcementInProgress) return;
		this.#hardCapEnforcementInProgress = true;
		try {
			if (this.#traceMap.size <= this.#maxTotalTraces) return;
	...
	*/
	...
	getMethod(event) {
		if (event.exportedSpan.isEvent) return "handleEventSpan";
		const eventType = event.type;
		switch (eventType) {
			case _mastra_core_observability.TracingEventType.SPAN_STARTED: return "handleSpanStart";
			case _mastra_core_observability.TracingEventType.SPAN_UPDATED: return "handleSpanUpdate";
			case _mastra_core_observability.TracingEventType.SPAN_ENDED: return "handleSpanEnd";
			default: throw new Error(`Unhandled event type: ${eventType}`);
		}
	}
	async _exportTracingEvent(event) {
		if (this.#shutdownStarted) return;
		const method = this.getMethod(event);
		if (method == "handleSpanUpdate" && this.skipSpanUpdateEvents) return;
		...
		if (!this.skipBuildRootTask && !traceData.hasRoot()) if (exportedSpan.isRootSpan) {
			this.logger.debug(`${this.name}: Building root`, {
				traceId: exportedSpan.traceId,
				spanId: exportedSpan.id
			});
			const rootData = await this._buildRoot({
				span: exportedSpan,
				traceData
			});
			if (rootData) {
		...
		try {
			switch (method) {
				...
				case "handleSpanEnd":
					...
					if (traceData.activeSpanCount() === 0) this.#scheduleCleanup(traceId);
					break;
			}
		} catch (error) {
			this.logger.error(`${this.name}: exporter error`, {
				error,
				event,
				method
			});
	...
	*/
	async shutdown() {
		...
		for (const [traceId, traceData] of this.#traceMap) {
			const orphanedEvents = traceData.getAllQueuedEvents();
			if (orphanedEvents.length > 0) this.logger.warn(`${this.name}: Dropping ${orphanedEvents.length} orphaned events on shutdown`, {
				traceId,
				orphanedEvents: orphanedEvents.map((e) => ({
					spanId: e.event.exportedSpan.id,
					waitingFor: e.waitingFor,
...
//#endregion
//#region src/exporters/span-formatters.ts
/**
...
function chainFormatters(formatters) {
	return async (span) => {
		...
		for (const formatter of formatters) currentSpan = await formatter(currentSpan);
		return currentSpan;
	};
}
//#endregion
//#region src/exporters/auth-failure-cooldown.ts
const AUTH_FAILURE_STATUSES = /* @__PURE__ */ new Set([401, 403]);
const AUTH_COOLDOWN_BASE_MS = 6e4;
const AUTH_COOLDOWN_MAX_MS = 15 * 6e4;
const AUTH_COOLDOWN_JITTER_RATIO = .1;
...
var AuthFailureCooldown = class {
	...
	dropEventsIfCoolingDown(count) {
		if (!this.shouldDropEvents()) return false;
	...
	recordFailure(args) {
		...
		const jitterMs = Math.floor(targetCooldownMs * AUTH_COOLDOWN_JITTER_RATIO * (Math.random() - .5) * 2);
		...
		this.getLogger().warn(`${this.exporterName} received an authentication failure; pausing uploads for ${cooldownSeconds}s`, {
			status: args.status,
...
//#endregion
//#region src/exporters/cloud.ts
const SIGNAL_PUBLISH_SUFFIXES$1 = {
...
const DEFAULT_CLOUD_SPAN_FILTER = (span) => span.type !== _mastra_core_observability.SpanType.MODEL_CHUNK;
const SIGNAL_PUBLISH_SEGMENTS$1 = {
	traces: "spans",
	logs: "logs",
	metrics: "metrics",
	scores: "scores",
...
function createInvalidEndpointError$1(endpoint, text, cause) {
...
const VALID_PROJECT_ID$1 = /^[a-zA-Z0-9_-]+$/;
function createInvalidProjectIdError$1(projectId) {
	return new _mastra_core_error.MastraError({
...
function resolveBaseEndpoint$1(baseEndpoint) {
	const normalizedEndpoint = trimTrailingSlashes$1(baseEndpoint);
	...
	try {
		...
		if (parsedEndpoint.pathname !== "/" || parsedEndpoint.search || parsedEndpoint.hash) throw createInvalidEndpointError$1(baseEndpoint, invalidText);
		return trimTrailingSlashes$1(parsedEndpoint.origin);
	} catch (error) {
		if (error instanceof _mastra_core_error.MastraError) throw error;
		throw createInvalidEndpointError$1(baseEndpoint, invalidText, error);
	}
}
function buildSignalPath$1(signal, projectId) {
	const signalSegment = SIGNAL_PUBLISH_SEGMENTS$1[signal];
	if (!projectId) return `/ai/${signalSegment}/publish`;
	return `/projects/${projectId}/ai/${signalSegment}/publish`;
}
function buildSignalEndpoint$1(baseEndpoint, signal, projectId) {
	return `${baseEndpoint}${buildSignalPath$1(signal, projectId)}`;
}
function resolveExplicitSignalEndpoint$1(signal, endpoint, projectId) {
	const normalizedEndpoint = trimTrailingSlashes$1(endpoint);
	const invalidText = `CloudExporter ${signal}Endpoint must be a base origin like "https://collector.example.com" or a full ${signal} publish URL ending in "${SIGNAL_PUBLISH_SUFFIXES$1[signal]}".`;
	try {
		const parsedEndpoint = new URL(normalizedEndpoint);
		if (parsedEndpoint.search || parsedEndpoint.hash) throw createInvalidEndpointError$1(endpoint, invalidText);
		const normalizedOrigin = trimTrailingSlashes$1(parsedEndpoint.origin);
		const normalizedPathname = trimTrailingSlashes$1(parsedEndpoint.pathname);
		if (!normalizedPathname || normalizedPathname === "/") return buildSignalEndpoint$1(normalizedOrigin, signal, projectId);
		if (normalizedPathname.endsWith(SIGNAL_PUBLISH_SUFFIXES$1[signal])) return `${normalizedOrigin}${normalizedPathname}`;
		throw createInvalidEndpointError$1(endpoint, invalidText);
	} catch (error) {
		if (error instanceof _mastra_core_error.MastraError) throw error;
		throw createInvalidEndpointError$1(endpoint, invalidText, error);
	}
}
function deriveSignalEndpointFromTracesEndpoint$1(signal, tracesEndpoint) {
	if (signal === "traces") return tracesEndpoint;
	const normalizedTracesEndpoint = trimTrailingSlashes$1(tracesEndpoint);
	...
	try {
		...
		const normalizedOrigin = trimTrailingSlashes$1(parsedEndpoint.origin);
		const normalizedPathname = trimTrailingSlashes$1(parsedEndpoint.pathname);
		if (!normalizedPathname.endsWith(SIGNAL_PUBLISH_SUFFIXES$1.traces)) throw createInvalidEndpointError$1(tracesEndpoint, invalidText);
		return `${normalizedOrigin}${normalizedPathname.slice(0, -SIGNAL_PUBLISH_SUFFIXES$1.traces.length)}${SIGNAL_PUBLISH_SUFFIXES$1[signal]}`;
	} catch (error) {
		if (error instanceof _mastra_core_error.MastraError) throw error;
		throw createInvalidEndpointError$1(tracesEndpoint, invalidText, error);
	}
}
/**
...
var CloudExporter = class extends BaseExporter {
	...
	constructor(config = {}) {
		super(config);
		if (config.projectId !== void 0 && !VALID_PROJECT_ID$1.test(config.projectId)) throw createInvalidProjectIdError$1(config.projectId);
		const accessToken = config.accessToken ?? process.env.MASTRA_CLOUD_ACCESS_TOKEN;
		const rawProjectId = config.projectId ?? process.env.MASTRA_PROJECT_ID;
		const projectId = rawProjectId && VALID_PROJECT_ID$1.test(rawProjectId) ? rawProjectId : void 0;
		if (!accessToken) this.setDisabled("MASTRA_CLOUD_ACCESS_TOKEN environment variable not set.", "debug");
		const tracesEndpointOverride = config.tracesEndpoint ?? process.env.MASTRA_CLOUD_TRACES_ENDPOINT;
		let baseEndpoint;
		let tracesEndpoint;
		if (tracesEndpointOverride) tracesEndpoint = resolveExplicitSignalEndpoint$1("traces", tracesEndpointOverride, projectId);
		else {
			baseEndpoint = resolveBaseEndpoint$1(config.endpoint ?? "https://observability.mastra.ai");
			tracesEndpoint = buildSignalEndpoint$1(baseEndpoint, "traces", projectId);
		}
		const resolveConfiguredSignalEndpoint = (signal, explicitEndpoint) => {
			if (explicitEndpoint) return resolveExplicitSignalEndpoint$1(signal, explicitEndpoint, projectId);
			if (tracesEndpointOverride) return deriveSignalEndpointFromTracesEndpoint$1(signal, tracesEndpoint);
			return buildSignalEndpoint$1(baseEndpoint, signal, projectId);
		};
		this.cloudConfig = {
			logger: this.logger,
			logLevel: config.logLevel ?? _mastra_core_logger.LogLevel.INFO,
	...
	async _exportTracingEvent(event) {
		...
		if (!DEFAULT_CLOUD_SPAN_FILTER(event.exportedSpan)) return;
	...
	markBufferStart() {
		if (this.buffer.totalSize === 0) this.buffer.firstEventTime = /* @__PURE__ */ new Date();
	}
	formatSpan(span) {
		return {
			...span,
	...
	async handleBufferedEvent() {
		if (this.shouldFlush()) this.flush().catch((error) => {
			this.logger.error("Batch flush failed", { error: error instanceof Error ? error.message : String(error) });
		});
		else if (this.buffer.totalSize === 1) this.scheduleFlush();
	}
	shouldFlush() {
		if (this.buffer.totalSize >= this.cloudConfig.maxBatchSize) return true;
		if (this.buffer.firstEventTime && this.buffer.totalSize > 0) {
			if (Date.now() - this.buffer.firstEventTime.getTime() >= this.cloudConfig.maxBatchWaitMs) return true;
		}
		return false;
	}
	scheduleFlush() {
	...
	async flushBuffer() {
		...
		if (failedSignals.length === 0) {
			...
			if (droppedEventsDuringAuthCooldown > 0) logData.droppedEventsDuringAuthCooldown = droppedEventsDuringAuthCooldown;
			this.logger.debug("Batch flushed successfully", logData);
			return;
		}
		if (authFailure?.authFailureStatus !== void 0) this.authFailureCooldown.recordFailure({
	...
	*/
	async batchUpload(signal, records) {
		...
		const options = {
			method: "POST",
			headers,
			body: JSON.stringify({ [SIGNAL_PUBLISH_SEGMENTS$1[signal]]: records })
		};
	...
	async flushSignalBatch(signal, records) {
		...
		try {
			...
			if (isAuthFailureError(error)) return {
				signal,
				succeeded: false,
				authFailureStatus: error.status
			};
			const mastraError = new _mastra_core_error.MastraError({
				id: `CLOUD_EXPORTER_FAILED_TO_BATCH_UPLOAD_${signal.toUpperCase()}`,
				domain: _mastra_core_error.ErrorDomain.MASTRA_OBSERVABILITY,
				category: _mastra_core_error.ErrorCategory.USER,
				details: {
					signal,
	...
	async flush() {
		...
		while (this.buffer.totalSize > 0 || this.inFlightFlushes.size > 0) {
			if (this.buffer.totalSize > 0) {
				this.logger.debug("Flushing buffered events", { bufferedEvents: this.buffer.totalSize });
	...
	async shutdown() {
		...
		try {
			...
			const mastraError = new _mastra_core_error.MastraError({
				...
				domain: _mastra_core_error.ErrorDomain.MASTRA_OBSERVABILITY,
				category: _mastra_core_error.ErrorCategory.USER,
				details: { remainingEvents: this.buffer.totalSize }
			}, error);
			this.logger.trackException(mastraError);
	...
	async _exportTracingEvent(event) {
		...
		const formatAttributes = (attributes) => {
			try {
				...
				return `[Unable to serialize attributes: ${error instanceof Error ? error.message : "Unknown formatting error"}]`;
			}
		};
		...
		switch (event.type) {
			case _mastra_core_observability.TracingEventType.SPAN_STARTED:
				...
				if (span.input !== void 0) this.logger.info(`   Input: ${formatAttributes(span.input)}`);
			...
			case _mastra_core_observability.TracingEventType.SPAN_ENDED:
				...
				this.logger.info(`   Trace ID: ${span.traceId}`);
				if (span.input !== void 0) this.logger.info(`   Input: ${formatAttributes(span.input)}`);
				if (span.output !== void 0) this.logger.info(`   Output: ${formatAttributes(span.output)}`);
				if (span.errorInfo) this.logger.info(`   Error: ${formatAttributes(span.errorInfo)}`);
				this.logger.info(`   Attributes: ${formatAttributes(span.attributes)}`);
				this.logger.info("─".repeat(80));
				break;
			case _mastra_core_observability.TracingEventType.SPAN_UPDATED:
				...
				this.logger.info("─".repeat(80));
				break;
			default: this.logger.warn(`Tracing event type not implemented: ${event.type}`);
		}
	}
	async shutdown() {
...
//#endregion
//#region src/exporters/event-buffer.ts
/**
...
var EventBuffer = class {
	...
	init(args) {
		if (!this.#storageStrategy) {
			...
			for (const event of this.#preInit) this.addEvent(event);
			this.#preInit = [];
		}
	}
	...
	setFirstEventTime() {
		if (!this.#firstEventTime) this.#firstEventTime = /* @__PURE__ */ new Date();
	}
	pushCreate(event) {
	...
	addEvent(event) {
		...
		switch (event.type) {
			...
			case _mastra_core_observability.TracingEventType.SPAN_UPDATED:
				...
				break;
			case _mastra_core_observability.TracingEventType.SPAN_ENDED:
				if (event.exportedSpan.isEvent) this.pushCreate(event);
				else switch (this.#storageStrategy) {
					case "realtime":
					case "batch-with-updates":
						this.pushUpdate(event);
						break;
	...
	reAddCreates(events) {
		const retryable = [];
		const dropped = [];
		for (const e of events) if (++e.retryCount <= this.#maxRetries) retryable.push(e);
		else dropped.push(e);
	...
	*/
	...
	addCreatedSpans(args) {
		if (this.#storageStrategy === "event-sourced" || this.#storageStrategy === "insert-only") return;
		for (const createRecord of args.records) if (!createRecord.isEvent) this.#allCreatedSpans.add(this.buildSpanKey(createRecord));
	}
	/** Check whether a span's create record has already been flushed to storage. */
	spanExists(span) {
		return this.#allCreatedSpans?.has(this.buildSpanKey({
			traceId: span.traceId,
			spanId: span.id
		}));
	}
...
//#endregion
//#region src/exporters/default.ts
/**
* Resolves the final tracing storage strategy based on config and observability store hints
*/
function resolveTracingStorageStrategy$1(config, observabilityStorage, storageName, logger) {
	const observabilityStrategy = observabilityStorage.observabilityStrategy;
	if (config.strategy && config.strategy !== "auto") {
		if (observabilityStrategy.supported.includes(config.strategy)) return config.strategy;
...
const FLUSH_SUCCEEDED$1 = { failed: false };
/**
...
var DefaultExporter = class extends BaseExporter {
	...
	async init(options) {
		try {
			...
			if (!this.#resolvedStrategy) {
				this.#resolvedStrategy = resolveTracingStorageStrategy$1(this.#config, this.#observabilityStorage, this.#storage.constructor.name, this.logger);
	...
	*/
	shouldFlush() {
		if (this.#resolvedStrategy === "realtime") return true;
		if (this.#eventBuffer.totalSize >= this.#config.maxBufferSize) return true;
		if (this.#eventBuffer.totalSize >= this.#config.maxBatchSize) return true;
		if (this.#eventBuffer.totalSize > 0) {
			if (this.#eventBuffer.elapsed >= this.#config.maxBatchWaitMs) return true;
		}
		return false;
	}
	/**
	...
	*/
	scheduleFlush(delayMs = this.#config.maxBatchWaitMs) {
		if (this.#flushTimer) clearTimeout(this.#flushTimer);
		this.#flushTimer = setTimeout(() => {
			this.#flushTimer = void 0;
			this.flushBuffer().catch((error) => {
				this.logger.error("Scheduled flush failed", { error: error instanceof Error ? error.message : String(error) });
			});
		}, delayMs);
	}
	...
	retryAttempt(events) {
		return events.reduce((earliestAttempt, event) => {
			if (event.retryCount > this.#config.maxRetries) return earliestAttempt;
			return earliestAttempt === void 0 ? event.retryCount : Math.min(earliestAttempt, event.retryCount);
		}, void 0);
	}
	logStorageFailure(signal, events, retryAttempt, error) {
		this.logger.warn("Failed to persist observability events", {
			signal,
			eventCount: events.length,
			retryAttempt: retryAttempt ?? events.reduce((latestAttempt, event) => Math.max(latestAttempt, event.retryCount), 0),
			maxRetries: this.#config.maxRetries,
			...retryAttempt === void 0 ? {} : { nextRetryDelayMs: this.retryDelay(retryAttempt) },
	...
	*/
	async handleBatchedFlush() {
		...
		else if (this.#eventBuffer.totalSize === 1) this.scheduleFlush();
	}
	sanitizeDropError(error) {
		if (error instanceof _mastra_core_error.MastraError) return {
			id: error.id,
	...
	*/
	async flushCreates(signal, events, storageCall) {
		if (events.length === 0) return FLUSH_SUCCEEDED$1;
		...
		try {
			...
			return FLUSH_SUCCEEDED$1;
		} catch (error) {
			if (error instanceof _mastra_core_error.MastraError && error.domain === _mastra_core_error.ErrorDomain.MASTRA_OBSERVABILITY && error.id.endsWith("_NOT_IMPLEMENTED")) {
	...
	*/
	async flushSpanUpdates(events, deferredUpdates, isEnd) {
		...
		for (const event of events) {
			const span = event.exportedSpan;
			if (this.#eventBuffer.spanExists(span)) partials.push({
				traceId: span.traceId,
				spanId: span.id,
				updates: (0, _mastra_core_storage.buildUpdateSpanRecord)(span)
			});
			else deferredUpdates.push(event);
		}
		if (partials.length === 0) return FLUSH_SUCCEEDED$1;
		try {
			await this.#observabilityStorage.batchUpdateSpans({ records: partials });
			if (isEnd) this.#eventBuffer.endFinishedSpans({ records: partials });
			return FLUSH_SUCCEEDED$1;
		} catch (error) {
			if (error instanceof _mastra_core_error.MastraError && error.domain === _mastra_core_error.ErrorDomain.MASTRA_OBSERVABILITY && error.id.endsWith("_NOT_IMPLEMENTED")) {
				this.logger.warn(error.message);
			...
			if (newlyDeferred > 0) deferredUpdates.splice(deferredUpdates.length - newlyDeferred, newlyDeferred);
	...
	*/
	async flushBuffer() {
		...
		if (this.#eventBuffer.totalSize === 0) return;
		const startTime = Date.now();
		...
		const endSpanEvents = [];
		for (const createEvent of creates) switch (createEvent.type) {
			case "feedback":
		...
		for (const updateEvent of updates) switch (updateEvent.type) {
			case _mastra_core_observability.TracingEventType.SPAN_UPDATED:
		...
		const flushResults = await Promise.all([
			this.flushCreates("feedback", createFeedbackEvents, (events) => this.#observabilityStorage.batchCreateFeedback({ feedbacks: events.map((f) => (0, _mastra_core_storage.buildFeedbackRecord)(f)) })),
			this.flushCreates("log", createLogEvents, (events) => this.#observabilityStorage.batchCreateLogs({ logs: events.map((l) => (0, _mastra_core_storage.buildLogRecord)(l)) })),
			this.flushCreates("metric", createMetricEvents, (events) => this.#observabilityStorage.batchCreateMetrics({ metrics: events.map((m) => (0, _mastra_core_storage.buildMetricRecord)(m)) })),
			this.flushCreates("score", createScoreEvents, (events) => this.#observabilityStorage.batchCreateScores({ scores: events.map((s) => (0, _mastra_core_storage.buildScoreRecord)(s)) })),
			this.flushCreates("tracing", createSpanEvents, async (events) => {
				const records = events.map((t) => (0, _mastra_core_storage.buildCreateSpanRecord)(t.exportedSpan));
		...
		if (deferredUpdates.length > 0) if (this.#unsupportedSignals.has("tracing")) {
			deferredDropCount = deferredUpdates.length;
			this.emitDrop("tracing", "unsupported-storage", deferredDropCount);
			deferredUpdates.length = 0;
		} else {
			deferredDropCount = this.#eventBuffer.reAddUpdates(deferredUpdates).length;
			this.emitDrop("tracing", "retry-exhausted", deferredDropCount);
		}
		const retryAttempt = flushResults.reduce((earliestAttempt, result) => {
			if (result.retryAttempt === void 0) return earliestAttempt;
			return earliestAttempt === void 0 ? result.retryAttempt : Math.min(earliestAttempt, result.retryAttempt);
		}, void 0);
		if (retryAttempt !== void 0) this.scheduleFlush(this.retryDelay(retryAttempt));
	...
	*/
	async flush() {
		if (this.#eventBuffer.totalSize > 0) {
			this.logger.debug("Flushing buffered events", { bufferedEvents: this.#eventBuffer.totalSize });
...
//#endregion
//#region src/exporters/mastra-platform.ts
const SIGNAL_PUBLISH_SUFFIXES = {
	traces: "/spans/publish",
	logs: "/logs/publish",
	metrics: "/metrics/publish",
	scores: "/scores/publish",
...
function resolveBaseEndpoint(baseEndpoint) {
	...
	try {
		const parsedEndpoint = new URL(normalizedEndpoint);
		if (parsedEndpoint.pathname !== "/" || parsedEndpoint.search || parsedEndpoint.hash) throw createInvalidEndpointError(baseEndpoint, invalidText);
...
function resolveExplicitSignalEndpoint(signal, endpoint, projectId) {
	...
	try {
		const parsedEndpoint = new URL(normalizedEndpoint);
		if (parsedEndpoint.search || parsedEndpoint.hash) throw createInvalidEndpointError(endpoint, invalidText);
		...
		if (!normalizedPathname || normalizedPathname === "/") return buildSignalEndpoint(normalizedOrigin, signal, projectId);
		if (normalizedPathname.endsWith(SIGNAL_PUBLISH_SUFFIXES[signal])) return `${normalizedOrigin}${normalizedPathname}`;
...
function deriveSignalEndpointFromTracesEndpoint(signal, tracesEndpoint) {
	...
	try {
		...
		const normalizedOrigin = trimTrailingSlashes(parsedEndpoint.origin);
		const normalizedPathname = trimTrailingSlashes(parsedEndpoint.pathname);
		if (!normalizedPathname.endsWith(SIGNAL_PUBLISH_SUFFIXES.traces)) throw createInvalidEndpointError(tracesEndpoint, invalidText);
		return `${normalizedOrigin}${normalizedPathname.slice(0, -SIGNAL_PUBLISH_SUFFIXES.traces.length)}${SIGNAL_PUBLISH_SUFFIXES[signal]}`;
	} catch (error) {
		if (error instanceof _mastra_core_error.MastraError) throw error;
...
var MastraPlatformExporter = class extends BaseExporter {
	...
	constructor(config = {}) {
		...
		const resolveConfiguredSignalEndpoint = (signal, explicitEndpoint) => {
			if (explicitEndpoint) return resolveExplicitSignalEndpoint(signal, explicitEndpoint, projectId);
			if (tracesEndpointOverride) return deriveSignalEndpointFromTracesEndpoint(signal, tracesEndpoint);
	...
	shouldFlush() {
		if (this.buffer.totalSize >= this.platformConfig.maxBatchSize) return true;
		if (this.buffer.firstEventTime && this.buffer.totalSize > 0) {
			if (Date.now() - this.buffer.firstEventTime.getTime() >= this.platformConfig.maxBatchWaitMs) return true;
		}
		return false;
	}
	scheduleFlush() {
	...
	*/
	...
	async flushSignalBatch(signal, records) {
		...
		try {
			...
			const mastraError = new _mastra_core_error.MastraError({
				id: `MASTRA_PLATFORM_EXPORTER_FAILED_TO_BATCH_UPLOAD_${signal.toUpperCase()}`,
				domain: _mastra_core_error.ErrorDomain.MASTRA_OBSERVABILITY,
				category: _mastra_core_error.ErrorCategory.USER,
				details: {
					signal,
...
//#endregion
//#region src/exporters/mastra-storage.ts
/**
* Resolves the final tracing storage strategy based on config and observability store hints
*/
...
const FLUSH_SUCCEEDED = { failed: false };
/**
...
async function getSnapshotsDir() {
	if (!_snapshotsDir) {
		if (typeof require("url").pathToFileURL(__filename).href !== "string") throw new Error("Snapshot functionality requires a Node.js environment. import.meta.url is not available in this runtime.");
		const { fileURLToPath } = await import("url");
		const { dirname, join } = await import("path");
		_snapshotsDir = join(dirname(fileURLToPath(require("url").pathToFileURL(__filename).href)), "..", "__snapshots__");
	}
...
var TestExporter = class extends BaseExporter {
	...
	*/
	async _exportTracingEvent(event) {
		...
		if (this.#config.storeLogs) this.#debugLogs.push(logMessage);
		...
		if (this.#config.validateLifecycle) this.#validateLifecycle(event, state, spanId);
		if (event.type === _mastra_core_observability.TracingEventType.SPAN_STARTED) state.hasStart = true;
		else if (event.type === _mastra_core_observability.TracingEventType.SPAN_ENDED) {
			state.hasEnd = true;
			if (span.isEvent) state.isEventSpan = true;
		} else if (event.type === _mastra_core_observability.TracingEventType.SPAN_UPDATED) state.hasUpdate = true;
	...
	*/
	#validateLifecycle(event, state, spanId) {
		const span = event.exportedSpan;
		if (event.type === _mastra_core_observability.TracingEventType.SPAN_STARTED) {
			if (state.hasStart) this.logger.warn(`Span ${spanId} (${span.type} "${span.name}") started twice`);
		} else if (event.type === _mastra_core_observability.TracingEventType.SPAN_ENDED) {
			if (span.isEvent) {
				if (state.hasStart) this.logger.warn(`Event span ${spanId} (${span.type} "${span.name}") incorrectly received SPAN_STARTED`);
				if (state.hasUpdate) this.logger.warn(`Event span ${spanId} (${span.type} "${span.name}") incorrectly received SPAN_UPDATED`);
			} else if (!state.hasStart) this.logger.warn(`Normal span ${spanId} (${span.type} "${span.name}") ended without starting`);
		}
	}
	/**
	...
	*/
	getSpansByType(type) {
		return Array.from(this.#spanStates.values()).filter((state) => {
			...
			return state.events.find((e) => e.type === _mastra_core_observability.TracingEventType.SPAN_ENDED)?.exportedSpan.type === type;
		}).map((state) => {
			return state.events.find((e) => e.type === _mastra_core_observability.TracingEventType.SPAN_ENDED)?.exportedSpan;
	...
	*/
	getByTraceId(traceId) {
		...
		return {
			events,
			spans: this.#getUniqueSpansFromEvents(events),
			logs: this.#logEvents.filter((e) => e.log.traceId === traceId).map((e) => e.log),
			scores: this.#scoreEvents.filter((e) => e.score.traceId === traceId).map((e) => e.score),
			feedback: this.#feedbackEvents.filter((e) => e.feedback.traceId === traceId).map((e) => e.feedback)
		};
	}
	/**
	...
	*/
	getBySpanId(spanId) {
		...
		const span = state.events.find((e) => e.type === _mastra_core_observability.TracingEventType.SPAN_ENDED)?.exportedSpan ?? state.events[state.events.length - 1]?.exportedSpan;
		return {
			events: state.events,
			span,
			state
	...
	*/
	getAllSpans() {
		return Array.from(this.#spanStates.values()).map((state) => {
			return state.events.find((e) => e.type === _mastra_core_observability.TracingEventType.SPAN_ENDED)?.exportedSpan ?? state.events[state.events.length - 1]?.exportedSpan;
		}).filter((span) => span !== void 0);
	}
	/**
	...
	*/
	getTraceIds() {
		...
		for (const event of this.#tracingEvents) traceIds.add(event.exportedSpan.traceId);
		for (const event of this.#logEvents) if (event.log.traceId) traceIds.add(event.log.traceId);
		for (const event of this.#scoreEvents) if (event.score.traceId) traceIds.add(event.score.traceId);
		for (const event of this.#feedbackEvents) if (event.feedback.traceId) traceIds.add(event.feedback.traceId);
	...
	*/
	getMetricsByType(_metricType) {
		throw new Error("getMetricsByType() has been removed: metricType is no longer stored. Use getMetricsByName(metricName) instead to filter metrics by name.");
	}
	/**
	...
	*/
	getStatistics() {
		...
		for (const state of this.#spanStates.values()) if (state.hasEnd) {
			completedSpans++;
			const spanType = state.events.find((e) => e.type === _mastra_core_observability.TracingEventType.SPAN_ENDED)?.exportedSpan.type;
			if (spanType) bySpanType[spanType] = (bySpanType[spanType] || 0) + 1;
		} else incompleteSpans++;
		const logsByLevel = {};
		...
		return {
			...
			incompleteSpans,
			byEventType: {
				started: this.#tracingEvents.filter((e) => e.type === _mastra_core_observability.TracingEventType.SPAN_STARTED).length,
				updated: this.#tracingEvents.filter((e) => e.type === _mastra_core_observability.TracingEventType.SPAN_UPDATED).length,
				ended: this.#tracingEvents.filter((e) => e.type === _mastra_core_observability.TracingEventType.SPAN_ENDED).length
			},
			bySpanType,
	...
	*/
	toJSON(options) {
		...
		const data = { spans: this.getAllSpans() };
		if (this.#logEvents.length > 0) data.logs = this.getAllLogs();
		if (this.#metricEvents.length > 0) data.metrics = this.getAllMetrics();
		if (this.#scoreEvents.length > 0) data.scores = this.getAllScores();
		if (this.#feedbackEvents.length > 0) data.feedback = this.getAllFeedback();
		if (includeEvents) data.events = this.#tracingEvents;
		if (includeStats) data.statistics = this.getStatistics();
	...
	*/
	buildSpanTree() {
		...
		const roots = [];
		for (const span of spans) nodeMap.set(span.id, {
			span,
			children: []
		});
		for (const span of spans) {
	...
	*/
	toTreeJSON(options) {
		const indent = options?.indent ?? this.#config.jsonIndent;
		const includeStats = options?.includeStats ?? true;
		const data = { tree: this.buildSpanTree() };
		if (includeStats) data.statistics = this.getStatistics();
		return JSON.stringify(data, this.#jsonReplacer, indent);
	}
	/**
	...
	*/
	buildNormalizedTree() {
		...
		const normalizeValue = (value, key) => {
			...
			if (key === "createdAt" && typeof value === "number") return "<date>";
			if (typeof value === "string") {
				if (key === "traceId" && (uuidRegex.test(value) || hexId32Regex.test(value))) {
					if (!traceIdMap.has(value)) traceIdMap.set(value, `<trace-${traceIdCounter++}>`);
					return traceIdMap.get(value);
				}
				if (uuidRegex.test(value)) return normalizeUuid(value, key ?? "uuid");
				...
				if (embeddedPrefixedUuidTest.test(value)) return value.replace(embeddedPrefixedUuidRegex, (_match, prefix, uuid) => {
					return `${prefix}_${normalizeUuid(uuid, prefix)}`;
				});
			}
			...
			if (value && typeof value === "object") {
				...
				for (const [k, v] of Object.entries(value)) {
					if (key === "providerOptions" && k === "mastra" && v && typeof v === "object") {
						const remainingMastraOptions = Object.fromEntries(Object.entries(v).filter(([mastraKey]) => mastraKey !== "createdAt"));
						if (Object.keys(remainingMastraOptions).length > 0) normalized[k] = normalizeValue(remainingMastraOptions, k);
						continue;
					}
					const normalizedValue = normalizeValue(v, k);
					if (normalizedValue !== void 0) normalized[k] = normalizedValue;
				}
				if (key === "providerOptions" && Object.keys(normalized).length === 0) return;
				return normalized;
			}
			return value;
		};
		const assignIds = (nodes) => {
			for (const node of nodes) {
				...
				if (!traceIdMap.has(node.span.traceId)) traceIdMap.set(node.span.traceId, `<trace-${traceIdCounter++}>`);
		...
		const normalizeNode = (node) => {
			...
			if (span.parentSpanId && spanIdMap.has(span.parentSpanId)) normalizedSpan.parentId = spanIdMap.get(span.parentSpanId);
			if (span.entityType) normalizedSpan.entityType = span.entityType;
			if (span.entityId) normalizedSpan.entityId = normalizeValue(span.entityId, "entityId");
			if (span.attributes && Object.keys(span.attributes).length > 0) normalizedSpan.attributes = normalizeValue(span.attributes);
			if (span.metadata && Object.keys(span.metadata).length > 0) normalizedSpan.metadata = normalizeValue(span.metadata);
			if (span.input !== void 0) normalizedSpan.input = normalizeValue(span.input);
			if (span.output !== void 0) normalizedSpan.output = normalizeValue(span.output);
			if (span.errorInfo) normalizedSpan.errorInfo = normalizeValue(span.errorInfo);
			if (span.tags && span.tags.length > 0) normalizedSpan.tags = span.tags;
			const result = { span: normalizedSpan };
			if (node.children.length > 0) result.children = node.children.map(normalizeNode);
			return result;
		};
	...
	*/
	generateStructureGraph(nodes) {
		...
		const buildLines = (node, prefix, isLast, isRoot) => {
			const line = `${prefix}${isRoot ? "" : isLast ? "└── " : "├── "}${node.span.type}: "${node.span.name}"`;
			lines.push(line);
	...
	*/
	toNormalizedTreeJSON(options) {
		...
		if (includeStructure) {
			const data = {
				__structure__: this.generateStructureGraph(normalizedTree),
	...
	*/
	async writeToFile(filePath, options) {
		...
		let json;
		if (format === "normalized") json = this.toNormalizedTreeJSON({ indent: options?.indent });
		else if (format === "tree") json = this.toTreeJSON({
			indent: options?.indent,
			includeStats: options?.includeStats
		});
		else json = this.toJSON(options);
	...
	*/
	async assertMatchesSnapshot(snapshotName, options) {
		...
		try {
			...
			if (err && typeof err === "object" && "code" in err && err.code === "ENOENT") throw new Error(`Snapshot file not found: ${snapshotPath}\nRun with { updateSnapshot: true } to create it.`);
			throw err;
		}
		try {
		...
		if (Array.isArray(snapshotData)) expectedSpans = snapshotData;
	...
	*/
	#compareStructure(actual, expected) {
		...
		for (let i = 0; i < maxLen; i++) {
			...
			if (actualLine !== expectedLine) if (actualLine === void 0) {
	...
	*/
	#deepCompareWithMarkers(actual, expected, path$2, mismatches) {
		if (this.#isOrMarker(expected)) {
			const allowedValues = expected.__or__;
			if (!allowedValues.some((allowed) => {
				const tempMismatches = [];
				this.#deepCompareWithMarkers(actual, allowed, path$2, tempMismatches);
				return tempMismatches.length === 0;
			})) mismatches.push({
				path: path$2,
				expected: { __or__: allowedValues },
				actual
			});
			return;
		}
		if (this.#isAnyMarker(expected)) {
			...
			if (typeConstraint === true) return;
			const actualType = Array.isArray(actual) ? "array" : typeof actual;
			if (actualType !== typeConstraint) mismatches.push({
				path: path$2,
				expected: { __any__: typeConstraint },
		...
		if (Array.isArray(expected)) {
			...
			for (let i = 0; i < expected.length; i++) this.#deepCompareWithMarkers(actual[i], expected[i], `${path$2}[${i}]`, mismatches);
			return;
		}
		if (expected !== null && typeof expected === "object") {
			...
			for (const key of Object.keys(expectedObj)) {
				...
				if (!(key in actualObj)) {
					if (expectedObj[key] !== void 0) mismatches.push({
						path: `${path$2}.${key}`,
				...
				this.#deepCompareWithMarkers(actualObj[key], expectedObj[key], `${path$2}.${key}`, mismatches);
			}
			for (const key of Object.keys(actualObj)) {
				if (this.#isMetadataKey(key)) continue;
				if (!(key in expectedObj)) {
					if (actualObj[key] !== void 0) mismatches.push({
						path: `${path$2}.${key}`,
						expected: void 0,
	...
	*/
	#isAnyMarker(value) {
		...
		if (!("__any__" in value)) return false;
	...
	*/
	#getUniqueSpansFromEvents(events) {
		...
		for (const event of events) {
			const span = event.exportedSpan;
			if (event.type === _mastra_core_observability.TracingEventType.SPAN_ENDED || !spanMap.has(span.id)) spanMap.set(span.id, span);
		}
...
*/
const JsonExporter = TestExporter;
//#endregion
//#region src/instances/default.ts
...
function buildScoreEvent(args) {
	...
	return {
		type: "score",
		score: {
			scoreId: (0, _mastra_core_observability.generateSignalId)(),
			timestamp: /* @__PURE__ */ new Date(),
			traceId,
			spanId,
...
function buildFeedbackEvent(args) {
	...
	return {
		type: "feedback",
		feedback: {
			feedbackId: (0, _mastra_core_observability.generateSignalId)(),
			timestamp: /* @__PURE__ */ new Date(),
			traceId,
			spanId,
...
var RecordedSpanImpl = class {
	...
	async addScore(score) {
		if (!this.#canEmitRecordedEvent()) {
			this.#debugRecordedAnnotationUnavailable({
				kind: "score",
				traceId: this.traceId,
				spanId: this.id
			});
		...
		await this.#emitRecordedEvent(buildScoreEvent({
	...
	async addFeedback(feedback) {
		...
		await this.#emitRecordedEvent(buildFeedbackEvent({
			traceId: this.#raw.traceId,
			spanId: this.id,
			correlationContext: buildCorrelationContext(this.#raw, this.#rootSpan, this.parent),
			feedback,
...
function hydrateRecordedTrace(args) {
	const { trace, emitRecordedEvent, canEmitRecordedEvent = () => true, debugRecordedAnnotationUnavailable = () => {} } = args;
	const rootSpan = findRootSpan(trace.spans);
	if (!rootSpan) return null;
	const recordedSpans = trace.spans.map((raw) => new RecordedSpanImpl({
		raw,
		rootSpan,
		emitRecordedEvent,
		canEmitRecordedEvent,
	...
	if (!hydratedRootSpan) return null;
...
var ObservabilityRegistry = class {
	...
	*/
	register(name, instance, isDefault = false) {
		if (this.#instances.has(name)) throw new Error(`Tracing instance '${name}' already registered`);
		this.#instances.set(name, instance);
		if (isDefault || !this.#defaultInstance) this.#defaultInstance = instance;
	}
	/**
	...
	*/
	getSelected(options) {
		if (this.#configSelector) {
			...
			if (selected && this.#instances.has(selected)) return this.#instances.get(selected);
		}
		return this.#defaultInstance;
	}
	/**
...
*/
var Observability = class extends _mastra_core_base.MastraBase {
	...
	constructor(config) {
		...
		if (config.configs) {
			for (const [name, configValue] of Object.entries(config.configs)) if (!isInstance(configValue)) {
		...
		const buildAutoSensitiveFilter = () => {
			if (!shouldAutoApplySensitiveFilter) return;
			return new SensitiveDataFilter(sensitiveDataFilterOptions);
		};
		if (config.default?.enabled) {
			console.warn("[Mastra Observability] The \"default: { enabled: true }\" configuration is deprecated and will be removed in a future version. Please use explicit configs with MastraStorageExporter and MastraPlatformExporter instead. Sensitive data filtering is applied by default and can be controlled via the top-level \"sensitiveDataFilter\" option. See https://mastra.ai/docs/observability/tracing/overview for the recommended configuration.");
		...
		if (config.configs) Object.entries(config.configs).forEach(([name, tracingDef], index) => {
			let instance;
			if (isInstance(tracingDef)) {
				instance = tracingDef;
				if (shouldAutoApplySensitiveFilter) {
					if (!(instance.getSpanOutputProcessors?.() ?? []).some((p) => p instanceof SensitiveDataFilter)) this.logger?.warn("[Mastra Observability] Pre-instantiated observability instance does not include a SensitiveDataFilter. Auto-applied filtering is skipped for pre-instantiated instances. Add a SensitiveDataFilter to spanOutputProcessors when constructing the instance to redact sensitive data.", { instanceName: name });
				}
			} else {
				const userProcessors = tracingDef.spanOutputProcessors ?? [];
				const autoFilter = !userProcessors.some((p) => p instanceof SensitiveDataFilter) ? buildAutoSensitiveFilter() : void 0;
		...
		if (config.configSelector) this.#registry.setSelector(config.configSelector);
	}
	...
	async getRecordedTrace(args) {
		...
		return hydrateRecordedTrace({
			...
			debugRecordedAnnotationUnavailable: ({ kind, traceId, spanId }) => {
				this.logger?.debug(kind === "score" ? "addScore() is unavailable; rehydrate the trace before calling addScore()" : "addFeedback() is unavailable; rehydrate the trace before calling addFeedback()", {
					traceId,
					spanId
				});
			}
	...
	registerInstance(name, instance, isDefault = false) {
		this.#registry.register(name, instance, isDefault);
		if (this.#mastra) instance.__setMastraEnvironment?.(this.#mastra.getEnvironment?.());
	}
	...
	*/
	getClientObservabilityProxy() {
		if (!this.#clientObservabilityProxy) this.#clientObservabilityProxy = createClientObservabilityProxy({
	...
	*/
	...
	async #emitRecordedEvent(event) {
		...
		if (!instance) {
			this.logger?.debug(event.type === "score" ? "Score event was dropped because no observability instance is registered" : "Feedback event was dropped because no observability instance is registered", { eventType: event.type });
			return;
		}
		...
		const handlerResults = [...instance.getExporters().map((exporter) => routeToHandler(exporter, event, this.logger)), ...bridge ? [routeToHandler(bridge, event, this.logger)] : []].filter((result) => !!result && typeof result.then === "function");
		if (handlerResults.length > 0) await Promise.allSettled(handlerResults);
	}
};
//#endregion
...
* ```
*/
const observabilityFeatures = /* @__PURE__ */ new Set(["model-inference-span", "internal-usage-rollup"]);
//#endregion
//#region src/tracing-options.ts
/**
...
exports.buildExportedLog = buildExportedLog;
exports.buildExportedSpan = buildExportedSpan;
exports.buildTracingOptions = buildTracingOptions;
exports.chainFormatters = chainFormatters;
exports.createClientObservabilityProxy = createClientObservabilityProxy;
exports.decodeResourceLogs = decodeResourceLogs;
exports.decodeResourceSpans = decodeResourceSpans;
exports.deepClean = deepClean;
exports.formatBaggage = formatBaggage;
...
exports.otlpSeverityToLogLevel = otlpSeverityToLogLevel;
exports.parseBaggage = parseBaggage;
