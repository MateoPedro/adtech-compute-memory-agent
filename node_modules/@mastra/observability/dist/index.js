...
function routeToHandler(handler, event, logger) {
	try {
		switch (event.type) {
			...
			case TracingEventType.SPAN_ENDED: return catchAsyncResult((handler.onTracingEvent ? handler.onTracingEvent.bind(handler) : handler.exportTracingEvent.bind(handler))(event), handler.name, "tracing", logger);
			case "log":
...
*/
var BaseObservabilityEventBus = class BaseObservabilityEventBus extends MastraBase {
...
*/
const samplingStrategySchema = z.discriminatedUnion("type", [
...
*/
const serializationOptionsSchema = z.object({
...
const cardinalityConfigSchema = z.object({
...
*/
const observabilityInstanceConfigSchema = z.object({
...
*/
const observabilityConfigValueSchema = z.object(observabilityInstanceConfigFields).refine((data) => {
	const hasExporters = data.exporters && data.exporters.length > 0;
	const hasBridge = !!data.bridge;
	return hasExporters || hasBridge;
}, { message: "At least one exporter or a bridge is required" });
...
function loadPricingModels() {
	...
	try {
		return parsePricingModelText(fs.readFileSync(resolvePricingModelPath(), "utf-8"));
	} catch (error) {
...
function getPackageRoot() {
	try {
		const packageJsonPath = createRequire(import.meta.url || "file://").resolve("@mastra/observability/package.json");
...
function getDurationMetricName(span) {
	switch (span.type) {
		...
		case SpanType.PROVIDER_TOOL_CALL: return "mastra_tool_duration_ms";
		case SpanType.CLIENT_TOOL_CALL: return null;
		case SpanType.WORKFLOW_RUN: return "mastra_workflow_duration_ms";
		case SpanType.MODEL_GENERATION: return "mastra_model_duration_ms";
		case SpanType.PROCESSOR_RUN: return "mastra_processor_duration_ms";
		default: return null;
	}
}
//#endregion
...
function isSpanInternal(spanType, flags) {
	if (flags === void 0 || flags === InternalSpans.NONE) return false;
	switch (spanType) {
		...
		case SpanType.WORKFLOW_WAIT_EVENT: return (flags & InternalSpans.WORKFLOW) !== 0;
		case SpanType.AGENT_RUN: return (flags & InternalSpans.AGENT) !== 0;
		case SpanType.TOOL_CALL:
		case SpanType.MCP_TOOL_CALL:
		case SpanType.PROVIDER_TOOL_CALL: return (flags & InternalSpans.TOOL) !== 0;
		case SpanType.MODEL_GENERATION:
		case SpanType.MODEL_STEP:
		case SpanType.MODEL_INFERENCE:
		case SpanType.MODEL_CHUNK: return (flags & InternalSpans.MODEL) !== 0;
		default: return false;
	}
}
/**
...
var BaseSpan = class {
	...
	*/
	createTracker() {
		if (this.type !== SpanType.MODEL_GENERATION) return;
...
var BaseObservabilityInstance = class extends MastraBase {
	...
	*/
	wireSpanLifecycle(span) {
		if (!this.config.includeInternalSpans && span.isInternal && span.type !== SpanType.MODEL_GENERATION) return;
	...
	extractKeys(requestContext, keys) {
		...
		for (const key of keys) {
			...
			if (value !== void 0) if (parts.length > 1) {
				const nestedValue = getNestedValue(value, parts.slice(1).join("."));
				if (nestedValue !== void 0) setNestedValue(result, key, nestedValue);
			} else setNestedValue(result, key, value);
		}
		return result;
	}
	/**
...
var TrackingExporter = class extends BaseExporter {
	...
	*/
	...
	getMethod(event) {
		...
		switch (eventType) {
			case TracingEventType.SPAN_STARTED: return "handleSpanStart";
			case TracingEventType.SPAN_UPDATED: return "handleSpanUpdate";
			case TracingEventType.SPAN_ENDED: return "handleSpanEnd";
...
const DEFAULT_CLOUD_SPAN_FILTER = (span) => span.type !== SpanType.MODEL_CHUNK;
...
var DefaultExporter = class extends BaseExporter {
	...
	*/
	...
	sanitizeDropError(error) {
		if (error instanceof MastraError) return {
			id: error.id,
	...
	*/
	async flushBuffer() {
		...
		const flushResults = await Promise.all([
			this.flushCreates("feedback", createFeedbackEvents, (events) => this.#observabilityStorage.batchCreateFeedback({ feedbacks: events.map((f) => buildFeedbackRecord(f)) })),
			this.flushCreates("log", createLogEvents, (events) => this.#observabilityStorage.batchCreateLogs({ logs: events.map((l) => buildLogRecord(l)) })),
			this.flushCreates("metric", createMetricEvents, (events) => this.#observabilityStorage.batchCreateMetrics({ metrics: events.map((m) => buildMetricRecord(m)) })),
			this.flushCreates("score", createScoreEvents, (events) => this.#observabilityStorage.batchCreateScores({ scores: events.map((s) => buildScoreRecord(s)) })),
...
async function getSnapshotsDir() {
	if (!_snapshotsDir) {
		if (typeof import.meta.url !== "string") throw new Error("Snapshot functionality requires a Node.js environment. import.meta.url is not available in this runtime.");
		const { fileURLToPath } = await import("url");
		const { dirname, join } = await import("path");
		_snapshotsDir = join(dirname(fileURLToPath(import.meta.url)), "..", "__snapshots__");
	}
...
var TestExporter = class extends BaseExporter {
	...
	*/
	async _exportTracingEvent(event) {
		...
		if (event.type === TracingEventType.SPAN_STARTED) state.hasStart = true;
		else if (event.type === TracingEventType.SPAN_ENDED) {
			state.hasEnd = true;
			if (span.isEvent) state.isEventSpan = true;
		} else if (event.type === TracingEventType.SPAN_UPDATED) state.hasUpdate = true;
	...
	*/
	getSpansByType(type) {
		return Array.from(this.#spanStates.values()).filter((state) => {
			...
			return state.events.find((e) => e.type === TracingEventType.SPAN_ENDED)?.exportedSpan.type === type;
		}).map((state) => {
			return state.events.find((e) => e.type === TracingEventType.SPAN_ENDED)?.exportedSpan;
	...
	*/
	getBySpanId(spanId) {
		...
		const span = state.events.find((e) => e.type === TracingEventType.SPAN_ENDED)?.exportedSpan ?? state.events[state.events.length - 1]?.exportedSpan;
		return {
			events: state.events,
			span,
			state
	...
	*/
	getAllSpans() {
		return Array.from(this.#spanStates.values()).map((state) => {
			return state.events.find((e) => e.type === TracingEventType.SPAN_ENDED)?.exportedSpan ?? state.events[state.events.length - 1]?.exportedSpan;
		}).filter((span) => span !== void 0);
	}
	/**
	...
	*/
	getStatistics() {
		...
		for (const state of this.#spanStates.values()) if (state.hasEnd) {
			completedSpans++;
			const spanType = state.events.find((e) => e.type === TracingEventType.SPAN_ENDED)?.exportedSpan.type;
	...
	*/
	#deepCompareWithMarkers(actual, expected, path, mismatches) {
		...
		if (Array.isArray(expected)) {
			...
			for (let i = 0; i < expected.length; i++) this.#deepCompareWithMarkers(actual[i], expected[i], `${path}[${i}]`, mismatches);
			return;
		}
	...
	*/
	#getUniqueSpansFromEvents(events) {
		...
		for (const event of events) {
			const span = event.exportedSpan;
			if (event.type === TracingEventType.SPAN_ENDED || !spanMap.has(span.id)) spanMap.set(span.id, span);
		}
