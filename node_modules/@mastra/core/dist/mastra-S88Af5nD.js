...
import { t as EventEmitterPubSub } from "./event-emitter-KtHd9pSx.js";
import { n as isRunLocalTopic } from "./topics-BCcUoD5n.js";
import { c as noOpMetricsContext, o as NoOpObservability, s as noOpLoggerContext } from "./observability-DaY8HN4h.js";
import { c as defaultGateways } from "./llm-Dz6Jt17j.js";
import { o as BackgroundTaskManager } from "./background-tasks-OPQtn1YI.js";
import { _ as readPositiveIntEnv } from "./utils-CfkDo9Tr.js";
import { Ct as augmentWithInit, G as AgentChannels, W as createRunScope, bn as createWorkflow, f as __registerMastraCtor, on as createOnScorerHook, sr as isDurableAgentLike, yn as createStep } from "./agent-D5C9QXkF.js";
import { r as normalizeToolPayloadTransformPolicy } from "./payload-transform-C4k4-WlM.js";
import { n as getGatewayId } from "./gateway-helpers-Dh-2ojNA.js";
import { a as parseFsAgentScheduleRowId, i as fsAgentScheduleRowId } from "./define-vaqWwloq.js";
import { t as computeNextFireAt } from "./cron-B2j814dd.js";
import { t as Schedules } from "./schedules-DTRZD7tD.js";
import { DualLogger, noopLogger } from "./logger/index.js";
import { t as WorkflowEventProcessor } from "./workflow-event-processor-mGtERBFZ.js";
import { a as registerHook, r as deregisterHook } from "./hooks-s8qUTtbg.js";
import { it as BackgroundTasksInMemory, l as WorkflowsInMemory, s as InMemoryStore } from "./storage-0ZpJ1SnM.js";
import { r as InMemoryDB } from "./source-BMoAR6uZ.js";
import { g as agentThreadStreamRuntime, r as dispatchDueNotifications } from "./storage-PiemtZJZ.js";
import { Y as getActiveDurableAgentWorkflowExecutions, t as createDurableAgent } from "./create-durable-agent-DiFta7_y.js";
import { t as DatasetsManager } from "./manager-DjocF2Uy.js";
import { LicenseClient } from "./license/index.js";
import { initContextStorage } from "./observability/context-storage.js";
import { r as isToolLoopAgentLike, t as toolLoopAgentToMastraAgent } from "./tool-loop-agent-C2Ld_Raj.js";
import { i as OrchestrationWorker, n as BackgroundTaskWorker, r as SchedulerWorker } from "./worker-Bm96F07W.js";
import { n as normalizeWorkflowBuilderDefinition } from "./builder-CmCvzaA_.js";
import { d as toJsonSchemaOrUndefined, p as collectNestedWorkflowIds, t as assertValidDynamicWorkflow } from "./validate-D-GkUudn.js";
import { t as rehydrateWorkflow } from "./rehydrate-OFEObzdY.js";
...
function createNotificationDispatchWorkflow({ batchSize = 100 } = {}) {
	...
	return createWorkflow({
		...
		options: { shouldPersistSnapshot: NOTIFICATION_DISPATCH_SHOULD_PERSIST_SNAPSHOT }
	}).then(dispatchStep).commit();
}
//#endregion
//#region src/schedules/row-diff.ts
/**
...
//#endregion
//#region src/mastra/fs-agent-schedules.ts
/**
...
*/
function declaredSchedulesOf(agent) {
	return agent?.getDeclaredSchedules?.() ?? [];
}
/** True when any registered agent declares schedules on disk. */
function hasFsAgentSchedule(agents) {
...
*/
function collectFsAgentSchedules(agents) {
	const schedules = [];
	const duplicates = [];
	const seen = /* @__PURE__ */ new Set();
	for (const agent of Object.values(agents ?? {})) for (const { key, definition } of declaredSchedulesOf(agent)) {
...
*/
function findFsAgentScheduleHandler(agents, scheduleId) {
	const parsed = parseFsAgentScheduleRowId(scheduleId);
	if (!parsed) return void 0;
	const handler = declaredSchedulesOf(Object.values(agents ?? {}).find((agent) => agent.id === parsed.agentId)).find((entry) => entry.key === parsed.key)?.definition.handler;
	return typeof handler === "function" ? handler : void 0;
}
/** Build the storage `target` for a declared schedule. */
function buildTarget(agentId, definition) {
	return {
		type: "agent",
		agentId,
...
*/
async function syncFsAgentSchedules({ agents, store, logger, knownRows }) {
	const { schedules: declared, duplicates } = collectFsAgentSchedules(agents);
	for (const { agentId, key } of duplicates) logger?.warn(`Duplicate schedule "${key}" for agent id "${agentId}": two registered agents share that id. Keeping the first and ignoring the rest.`);
	...
	for (const { scheduleId, agentId, key, definition } of declared) try {
		const now = Date.now();
		...
		if (!existing) {
			await store.createSchedule({
				...
				nextFireAt: computeNextFireAt(definition.cron, {
					timezone: definition.timezone,
					after: now
				}),
				createdAt: now,
		...
		const timezoneChanged = (existing.timezone ?? void 0) !== (definition.timezone ?? void 0);
		...
		if (cronChanged || timezoneChanged) patch.nextFireAt = computeNextFireAt(definition.cron, {
			timezone: definition.timezone,
			after: now
		});
		if (Object.keys(patch).length > 0) await store.updateSchedule(scheduleId, patch);
	} catch (error) {
		logger?.error("Failed to register file-based agent schedule", {
			scheduleId,
			agentId,
			key,
			error
	...
	for (const row of allRows) {
		...
		if (!row.id.startsWith("fsa_")) continue;
...
var Mastra = class Mastra {
		...
		if (!this.#pubsubProxy) {
			...
			this.#pubsubProxy = new Proxy(raw, { get(target, prop, _receiver) {
				if (prop === "publish") return function publish(topic, event) {
					...
					} else if (isRunLocalTopic(topic)) return target.publish(topic, event, { localOnly: true });
	...
	*/
	__getFsAgentScheduleHandler(scheduleId) {
		return findFsAgentScheduleHandler(this.#agents, scheduleId);
	}
	/**
	...
	*/
	...
	#shouldEnableScheduler() {
		...
		return this.#hasScheduledWorkflow || this.#schedulerRequested || hasFsAgentSchedule(this.#agents);
	}
	/**
	...
	*/
	async #registerFsAgentSchedules(schedulesStore, knownRows) {
	...
	*/
	async resolveVersionedAgent(agent, version) {
		...
		const resolved = await editor.agent.applyStoredOverrides(agent, "versionId" in version ? version : { status: version.status ?? "published" });
		if (resolved !== agent) resolved.__markStoredVersionApplied();
		return resolved;
	}
	/**
	...
	*/
	#syncLateRegisteredAgentSchedules(agent) {
		if (declaredSchedulesOf(agent).length === 0) return;
		if (!this.#findSchedulerWorker()?.scheduler) return;
	...
	*/
	#touchRunScopedWorkflow(id, runId) {
	...
	*/
	...
	__getInternalWorkflow(id, runId) {
		...
		if (runId) {
			...
			if (workflow) this.#touchRunScopedWorkflow(id, runId);
			else workflow = this.#internalMastraWorkflows[id];
		} else workflow = this.#internalMastraWorkflows[id];
	...
	*/
	#sweepStaleRunScopedWorkflows() {
		const now = Date.now();
		for (const [key, entry] of this.#runScopedWorkflowTimestamps) if (now - entry.lastActivityAt > Mastra.INTERNAL_WORKFLOW_TTL_MS) {
			delete this.#internalMastraWorkflows[key];
			this.#runScopedWorkflowTimestamps.delete(key);
	...
	*/
	...
	#replaceDynamicWorkflow(workflow, key) {
		workflow.__registerMastra(this);
		workflow.__registerPrimitives({
			logger: this.getLogger(),
			storage: this.getStorage()
	...
	* ```
	*/
	async addDynamicWorkflows(defs) {
		if (defs.length === 0) return;
		const seen = /* @__PURE__ */ new Set();
		for (const def of defs) {
			if (seen.has(def.id)) throw new Error(`Dynamic workflow bundle contains more than one definition with id "${def.id}". Ids must be unique within a bundle.`);
			seen.add(def.id);
		}
		const members = defs.map((def) => ({ normalized: normalizeWorkflowBuilderDefinition({
			id: def.id,
		...
		for (const { normalized } of members) assertValidDynamicWorkflow(normalized, index);
		const ordered = [];
		const remaining = new Map(members.map((member) => [member.normalized.id, member]));
		const hydrated = /* @__PURE__ */ new Set();
		let progress = true;
		while (remaining.size > 0 && progress) {
			progress = false;
			for (const [id, member] of Array.from(remaining)) {
				if (Array.from(collectNestedWorkflowIds(member.normalized.graph)).filter((dependency) => dependency !== id && bundleIds.has(dependency) && !hydrated.has(dependency)).length > 0) continue;
				remaining.delete(id);
				hydrated.add(id);
				ordered.push(member);
				progress = true;
		...
		if (remaining.size > 0) throw new Error(`Dynamic workflow bundle has a circular nested-workflow dependency among: ${Array.from(remaining.keys()).sort().join(", ")}.`);
		...
		try {
			for (const { normalized } of ordered) {
				...
				this.#replaceDynamicWorkflow(workflow, normalized.id);
			}
			const store = await this.#storage?.getStore("workflowDefinitions");
			if (store) for (const { normalized } of ordered) await store.upsert({
				id: normalized.id,
	...
	*/
	async #loadDynamicWorkflows() {
		...
		if (remaining.size > 0) {
			...
			this.#logger?.error?.(`Failed to load dynamic workflows (cycle or unresolved nested-workflow reference): ${stuck}`);
		}
	}
	/**
	...
	*/
	...
	async #startSchedulingWorkers() {
		...
		if (!this.#findAgentScheduleWorker()) {
			const { AgentScheduleWorker } = await import("./worker-CRdouRg_.js");
	...
	* @internal
	*/
	async #detectPersistedSchedulerWork() {
		if (!this.#storage) return;
		try {
			const schedulesStore = await this.#storage.getStore("schedules");
			if (!schedulesStore) return;
			if ((await schedulesStore.listSchedules({ ownerType: "agent" })).length > 0) {
				this.#schedulerRequested = true;
				return;
			}
			if (this.#notificationDispatchConfig?.enabled === false) return;
			if (await schedulesStore.getSchedule("__mastra_notification_dispatch")) this.#schedulerRequested = true;
		} catch (err) {
			this.#logger?.warn?.("Failed to detect persisted scheduler work on boot", err);
		}
	}
	...
	*/
	#ackingTopicListeners = /* @__PURE__ */ new WeakMap();
	#ackingTopicListener(listener) {
		const existing = this.#ackingTopicListeners.get(listener);
		if (existing) return existing;
		const wrapped = async (event, ack, nack) => {
			try {
	...
	*/
	async startWorkers(name) {
		...
		if (!name && !this.#schedulerRequested && !schedulerExplicitlyRequested && !this.#schedulerDisabled()) await this.#detectPersistedSchedulerWork();
		...
		if (this.#storage) await this.#loadDynamicWorkflows();
		...
		if (!name) for (const topic in this.#events) {
			...
			for (const listener of listeners) {
				...
				if (this.#userEventSubscriptions.some((sub) => sub.topic === topic && sub.cb === cb)) continue;
	...
	*/
	#syncGatewayRegistry() {
		try {
			...
			import("./provider-registry-DeNcniwM.js").then((n) => n.u).then(async ({ GatewayRegistry }) => {
	...
	*/
	async shutdown() {
		...
		if (this.#backgroundTaskManager) await this.#backgroundTaskManager.shutdown();
		await this.stopWorkers();
		(await Promise.allSettled(getActiveDurableAgentWorkflowExecutions(this))).forEach((result) => {
			if (result.status === "rejected") this.#logger?.error("Durable agent execution failed during shutdown", { error: result.reason });
		});
...
//# sourceMappingURL=mastra-S88Af5nD.js.map