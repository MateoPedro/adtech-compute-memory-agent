	...
	#finalizeWithError(reason) {
		...
		this.#emitter.emit("chunk", {
			...
			payload: {
				...
				output: { usage: this.#usageCount }
			}
		});
		Object.entries(this.#delayedPromises).forEach(([_key, promise]) => {
			if (promise.status.type === "pending") promise.reject(error);
		});
			...
			return {
				...
				payload: {
					...
					stepResult: {
						...
						...rawFinishReason !== void 0 && { rawReason: rawFinishReason }
					},
					output: { usage: normalizeUsage(value.usage, value.providerMetadata) },
...
function normalizeUsage(usage, providerMetadata) {
...
*/
function extractRawFinishReason(finishReason) {
	return isV3FinishReason(finishReason) ? finishReason.raw : void 0;
}
//#endregion
...
//# sourceMappingURL=stream-r-PlaI9f.cjs.map