...
const require_types = require("./types-B1ZSVdrB.cjs");
...
function isStructuredOutputFormatError(error) {
	return require_dist.JSONParseError.isInstance(error) || require_dist.NoObjectGeneratedError.isInstance(error) || require_dist.TypeValidationError.isInstance(error) || error instanceof require_error.MastraError && (error.id === "STRUCTURED_OUTPUT_OBJECT_UNDEFINED" || error.id === "STRUCTURED_OUTPUT_SCHEMA_VALIDATION_FAILED");
}
async function tryGenerateWithJsonFallback(agent, prompt, options) {
	if (!options.structuredOutput?.schema) throw new require_error.MastraError({
	...
	try {
		...
		if (result.object === void 0) throw new require_error.MastraError({
...
async function tryStreamWithJsonFallback(agent, prompt, options) {
	...
	try {
		...
		onStream?.(result);
		try {
			if (await result.object === void 0) throw new require_error.MastraError({
				id: "STRUCTURED_OUTPUT_OBJECT_UNDEFINED",
				domain: require_error.ErrorDomain.AGENT,
				category: require_error.ErrorCategory.USER,
				text: "structuredOutput object is undefined"
...
function createProcessorSendSignal(args) {
	return async (signalInput) => {
		const signal = require_signals.createSignal(signalInput);
...
var runner_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
...
function getProcessableResponseMessages(messageList) {
	...
	return messageList.get.all.db().filter((message) => messagesById.has(message.id)).map((message) => messagesById.get(message.id));
}
/**
...
var ProcessorState = class {
	...
	constructor(options) {
		...
		this.span = parentSpan?.createChildSpan({
			...
			entityType: require_utils.EntityType.OUTPUT_PROCESSOR,
...
function areProcessorMessageArraysEqual(before, after) {
	...
	return before.length === after.length && before.every((message, index) => require_message_list.messagesAreEqual(message, after[index]));
}
...
var ProcessorRunner = class ProcessorRunner {
	...
	*/
	...
	async runComputeStateSignal({ processor, messageList, stepNumber, steps, requestContext, writer, abort, processorState, memory, resourceId, threadId, abortSignal, retryCount, rotateResponseMessageId }) {
		...
		const memoryContext = require_types.parseMemoryRequestContext(requestContext);
		...
		const tracking = require_types.getStateSignalsMetadata(thread.metadata)[stateId];
		const { activeStateSignals, contextWindow, lastSnapshot, deltasSinceSnapshot } = await require_types.resolveStateSignalHistory({
			messageList,
		...
		const result = await computeStateSignal({
			...
			sendStateSignal: async (stateSignal) => {
				const sendResult = await require_types.applyStateSignal({
					input: stateSignal,
					memory: resolvedMemory,
					thread,
					resourceId: resolvedResourceId,
		...
		if (!result) return;
		await require_types.applyStateSignal({
			input: result,
			memory: resolvedMemory,
			thread,
			resourceId: resolvedResourceId,
	...
	*/
	async executeWorkflowAsProcessor(workflow, input, observabilityContext, requestContext, writer, abortSignal) {
		...
		if (!("phase" in output) || !("messages" in output || "part" in output || "messageList" in output)) throw new require_error.MastraError({
			category: "USER",
			domain: "AGENT",
		...
		for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
			...
			try {
				...
				const processResult = await processMethod({
					...
					abort,
					agent: this.agent,
					...require_observability.createObservabilityContext({ currentSpan: processorSpan }),
					requestContext,
					retryCount,
					writer,
				...
				if (processResult instanceof require_message_list.MessageList) {
					if (processResult !== messageList) throw new require_error.MastraError({
						category: "USER",
						domain: "AGENT",
					...
					if (mutations.length > 0) processableMessages = getProcessableResponseMessages(processResult);
				...
				try {
					if (processor.processOutputStream && processedPart) {
						...
						processedPart = await processor.processOutputStream({
							...
							...require_observability.createObservabilityContext({ currentSpan: state.span }),
							requestContext,
							messageList,
							retryCount,
							writer
	...
	async runInputProcessors(messageList, observabilityContext, requestContext, retryCount = 0) {
		for (const [index, processorOrWorkflow] of this.inputProcessors.entries()) {
			...
			const processorSpan = (currentSpan?.findParent("agent_run") || currentSpan?.parent || currentSpan)?.createChildSpan({
				...
				entityType: require_utils.EntityType.INPUT_PROCESSOR,
				entityId: processor.id,
				entityName: processor.name,
				attributes: {
					processorExecutor: "legacy",
			...
			try {
				...
				let mutations;
				if (result instanceof require_message_list.MessageList) {
					if (result !== messageList) throw new require_error.MastraError({
						category: "USER",
						domain: "AGENT",
						id: "PROCESSOR_RETURNED_EXTERNAL_MESSAGE_LIST",
						text: `Processor ${processor.id} returned a MessageList instance other than the one that was passed in as an argument. New external message list instances are not supported. Use the messageList argument instead.`
	...
	*/
	async runProcessInputStep(args) {
		...
		const observabilityContext = require_observability.resolveObservabilityContext(args);
		const stepInput = {
		...
		for (const [index, processorOrWorkflow] of processors.entries()) {
			...
			const processorSpan = (observabilityContext.tracingContext?.currentSpan)?.createChildSpan({
				...
				entityType: require_utils.EntityType.INPUT_STEP_PROCESSOR,
				entityId: processor.id,
				entityName: processor.name,
				attributes: {
					processorExecutor: "legacy",
			...
			try {
				...
				const processMethodArgs = {
					...
					sendStateSignal: async (stateSignal) => {
						...
						const result = await require_types.applyStateSignal({
							input: stateSignal,
							memory: resolvedMemory,
							thread: {
								...loadedThread,
	...
	*/
	async runProcessLLMRequest(args) {
		require_observability.resolveObservabilityContext({ tracingContext: args.tracingContext });
		...
		for (const processorOrWorkflow of this.inputProcessors) {
			...
			try {
				...
				const result = await processMethod({
					...
					abortSignal: args.abortSignal,
					writer: args.writer,
					...require_observability.createObservabilityContext(args.tracingContext)
				});
	...
	async runProcessOutputStep(args) {
		...
		for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
			...
			const processorSpan = (currentSpan?.findParent("agent_run") || currentSpan?.parent || currentSpan)?.createChildSpan({
				...
				entityType: require_utils.EntityType.OUTPUT_STEP_PROCESSOR,
				entityId: processor.id,
				entityName: processor.name,
				attributes: {
					processorExecutor: "legacy",
	...
	async runProcessToolResult(args) {
		...
		for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
			...
			const processorSpan = (currentSpan?.findParent("agent_run") || currentSpan?.parent || currentSpan)?.createChildSpan({
				...
				entityType: require_utils.EntityType.TOOL_RESULT_PROCESSOR,
				entityId: processor.id,
				entityName: processor.name,
				attributes: {
					processorExecutor: "legacy",
			...
			try {
				...
				const mutations = messageList.stopRecording();
				if (processorResult instanceof require_message_list.MessageList) {
					if (processorResult !== messageList) throw new require_error.MastraError({
						category: "USER",
						domain: "AGENT",
						id: "PROCESSOR_RETURNED_EXTERNAL_MESSAGE_LIST",
						text: `Processor ${processor.id} returned a MessageList instance other than the one that was passed in as an argument. New external message list instances are not supported. Use the messageList argument instead.`
	...
	*/
	...
	static async validateAndFormatProcessInputStepResult(result, { messageList, processor, stepNumber }) {
		if (result instanceof require_message_list.MessageList) {
			...
			if (result.messageList && result.messageList !== messageList) throw new require_error.MastraError({
				category: "USER",
				domain: "AGENT",
				id: "PROCESSOR_RETURNED_EXTERNAL_MESSAGE_LIST",
				text: `Processor ${processor.id} returned a MessageList instance other than the one that was passed in as an argument. New external message list instances are not supported. Use the messageList argument instead.`
			});
			if (result.messages && result.messageList) throw new require_error.MastraError({
				category: "USER",
				domain: "AGENT",
			...
			if (result.model) {
				const resolvedModel = await require_llm.resolveModelConfig(result.model);
				if (!isSupportedLanguageModel(resolvedModel)) throw new require_error.MastraError({
					category: "USER",
					domain: "AGENT",
...
var MastraModelInput = class extends require_base.MastraBase {
	initialize({ runId, createStream, onResult }) {
		...
		outputStream = new ReadableStream({ async start(controller) {
			try {
				...
				require_llm.attachModelStreamTransport(outputStream, require_llm.readModelStreamTransport(stream));
...
function asJsonSchema(schema) {
	if (!schema) return;
	if ((0, _mastra_schema_compat_schema.isStandardSchemaWithJSON)(schema)) return (0, _mastra_schema_compat_schema.standardSchemaToJSONSchema)(schema, {
		io: "input",
		target: "draft-07"
	});
	return schema;
...
function getTransformedSchema(schema, options) {
	if (!schema) return;
	const jsonSchema = options?.model ? (0, _mastra_schema_compat.applyCompatLayer)({
		schema,
		compatLayers: [new _mastra_schema_compat.AnthropicSchemaCompatLayer(options.model)],
		mode: "jsonSchema"
	}) : asJsonSchema(schema);
...
var BaseFormatHandler = class {
	...
	*/
	isZodSchema(schema) {
		return (0, _mastra_schema_compat.isZodType)(schema);
	}
	/**
	...
	*/
	async validateValue(value) {
		...
		if (this.isZodSchema(this.schema)) try {
			...
			return {
				success: false,
				error: new require_error.MastraError({
					domain: require_error.ErrorDomain.AGENT,
...
var ObjectFormatHandler = class extends BaseFormatHandler {
	...
	async processPartialChunk({ accumulatedText, previousObject }) {
		const { value: currentObjectJson, state } = await require_dist.parsePartialJson(this.preprocessText(accumulatedText));
		if (this.validatePartialChunks && this.partialSchema) {
			const result = this.partialSchema?.safeParse(currentObjectJson);
			if (result.success && result.data && result.data !== void 0 && !require_dist.isDeepEqualData(previousObject, result.data)) return {
				shouldEmit: true,
		...
		if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !require_dist.isDeepEqualData(previousObject, currentObjectJson)) return {
	...
	async validateAndTransformFinal(finalRawValue) {
		...
		const { value } = await require_dist.parsePartialJson(this.preprocessText(finalRawValue));
...
var ArrayFormatHandler = class extends BaseFormatHandler {
	...
	async processPartialChunk({ accumulatedText, previousObject }) {
		const { value: currentObjectJson, state: parseState } = await require_dist.parsePartialJson(this.preprocessText(accumulatedText));
		if (currentObjectJson !== void 0 && !require_dist.isDeepEqualData(previousObject, currentObjectJson)) {
			...
			if (!require_dist.isDeepEqualData(this.textPreviousFilteredArray, filteredElements)) {
...
var EnumFormatHandler = class extends BaseFormatHandler {
	...
	*/
	findBestEnumMatch(partialResult) {
		if (!this.schema) return;
		const enumValues = (0, _mastra_schema_compat_schema.standardSchemaToJSONSchema)(this.schema)?.enum;
	...
	async processPartialChunk({ accumulatedText, previousObject }) {
		const { value: currentObjectJson } = await require_dist.parsePartialJson(this.preprocessText(accumulatedText));
		if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !Array.isArray(currentObjectJson) && "result" in currentObjectJson && typeof currentObjectJson.result === "string" && !require_dist.isDeepEqualData(previousObject, currentObjectJson)) {
	...
	async validateAndTransformFinal(rawFinalValue) {
		const { value } = await require_dist.parsePartialJson(this.preprocessText(rawFinalValue));
...
function createOutputHandler({ schema }) {
	const normalizedSchema = schema ? (0, _mastra_schema_compat_schema.toStandardSchema)(schema) : void 0;
	...
	return new stream_web.TransformStream({
		transform(chunk, controller) {
			...
			if (outputSchema?.outputFormat === "array" && Array.isArray(chunk.object)) {
				if (pendingFirstArrayChunk !== void 0) {
					controller.enqueue("[");
		...
		flush(controller) {
			if (outputSchema?.outputFormat !== "array") return;
			if (pendingFirstArrayChunk !== void 0) {
				const firstChunk = pendingFirstArrayChunk;
				pendingFirstArrayChunk = void 0;
				controller.enqueue(firstChunk.length > 0 ? JSON.stringify(firstChunk) : "[]");
			} else if (hasStartedArray) controller.enqueue("]");
		}
	});
}
//#endregion
//#region src/stream/base/step-request-dedupe.ts
/**
...
*/
const REQUEST_REF = "__requestRef";
/**
...
*/
function hasInjectiveJson(value, seen = /* @__PURE__ */ new Set()) {
	...
	try {
		...
		if (Array.isArray(object)) return object.every((element, index) => index in object && hasInjectiveJson(element, seen));
		const prototype = Object.getPrototypeOf(object);
		if (prototype !== Object.prototype && prototype !== null) return false;
		if ("toJSON" in object) return false;
		return Reflect.ownKeys(object).every((key) => {
			if (typeof key === "symbol") return false;
			return hasInjectiveJson(object[key], seen);
		});
	} finally {
		seen.delete(object);
	}
}
function dedupeStepRequests(steps) {
	...
	const rewritten = steps.map((step) => {
		if (!step || typeof step !== "object" || step.request === void 0) return step;
		...
		const { request: _dropped, ...rest } = step;
		return {
			...rest,
			[REQUEST_REF]: index
		};
...
function resolveOutputTextSkippingCompletionChecks(messageList) {
	...
	if (responseDbMessages.some((m) => m.content?.metadata?.completionResult)) {
		...
		const converted = lastRealMessage ? require_message_list.convertMessages([lastRealMessage]).to("AIV4.Core") : [];
		const lastConverted = converted[converted.length - 1];
		return lastConverted ? require_message_list.coreContentToString(lastConverted.content) : void 0;
	}
	const responseMessages = messageList.get.response.aiV4.core();
	const lastResponseMessage = responseMessages[responseMessages.length - 1];
	return lastResponseMessage ? require_message_list.coreContentToString(lastResponseMessage.content) : void 0;
}
var MastraModelOutput = class extends require_base.MastraBase {
	#status = "running";
	#error;
	#baseStream;
	#bufferedChunks = [];
	...
	*/
	#bufferedByStepReasoningDetails = {};
	#bufferedByStep = {
		text: "",
		reasoning: [],
		sources: [],
	...
	*/
	...
	constructor({ model: _model, stream, messageList, options, messageId, initialState }) {
		...
		const resultSpan = require_utils.getRootExportSpan(options.tracingContext?.currentSpan);
		...
		if (processorRunner && options.isLLMExecutionStep) {
			...
			processedStream = stream.pipeThrough(new stream_web.TransformStream({ async transform(chunk, controller) {
				if (chunk.type === "finish" && chunk.payload?.stepResult?.reason === "tool-calls") {
					...
					*/
					...
					const { part: processed, blocked, reason, tripwireOptions, processorId } = await processorRunner.processPart(chunk, processorStates, require_observability.resolveObservabilityContext(options), options.requestContext, self.messageList, 0, streamWriter);
					const enqueueTripwire = (r, opts, pid) => {
						controller.enqueue({
							type: "tripwire",
							payload: {
					...
					const reprocessed = await processorRunner.drainReprocessParts(processorStates, require_observability.resolveObservabilityContext(options), options.requestContext, self.messageList, 0, streamWriter);
					for (const r of reprocessed) {
						if (r.blocked) {
							enqueueTripwire(r.reason, r.tripwireOptions, r.processorId);
							return;
		...
		this.#baseStream = processedStream.pipeThrough(new stream_web.TransformStream({
			transform: async (chunk, controller) => {
				switch (chunk.type) {
					...
					case "object-result":
						...
						if (!self.processorRunner && self.#delayedPromises.object.status.type === "pending") self.#delayedPromises.object.resolve(chunk.object);
						break;
					case "source":
					...
					case "reasoning-delta":
						...
						for (const bufferedReasoning of [self.#bufferedReasoningDetails[chunk.payload.id], self.#bufferedByStepReasoningDetails[chunk.payload.id]]) {
						...
						break;
					case "reasoning-end":
						if (chunk.payload.providerMetadata) for (const bufferedReasoning of [self.#bufferedReasoningDetails[chunk.payload.id], self.#bufferedByStepReasoningDetails[chunk.payload.id]]) {
							if (!bufferedReasoning) continue;
					...
					case "step-finish": {
						...
						if (stepTripwire?.retry) self.#bufferedObject = void 0;
						else if (self.#bufferedObject !== void 0 && self.#delayedPromises.object.status.type === "pending") self.#delayedPromises.object.resolve(self.#bufferedObject);
						self.#bufferedByStep = {
							text: "",
							reasoning: [],
							sources: [],
					...
					case "finish":
						...
						try {
							if (self.processorRunner && !self.#options.isLLMExecutionStep) {
								...
								if (self.#status !== "canceled") self.messageList = await self.processorRunner.runOutputProcessors(self.messageList, require_observability.resolveObservabilityContext(options), self.#options.requestContext, 0, outputResultWriter, outputResult);
								const outputText = resolveOutputTextSkippingCompletionChecks(self.messageList);
								if (lastStep && outputText !== void 0 && outputText !== originalText) lastStep.text = outputText;
							...
							if (error instanceof TripWire) {
								...
								self.#error = require_error.getErrorFromUnknown(error, { fallbackMessage: "Unknown error in stream" });
								self.resolvePromises({
									finishReason: "error",
									text: ""
								});
							}
							if (self.#delayedPromises.object.status.type !== "resolved") self.#delayedPromises.object.resolve(self.#bufferedObject);
						}
						...
						if (baseFinishStep) {
							const onFinishPayload = {
								...
								...self.#model.modelId && self.#model.provider && self.#model.version ? { model: self.#model } : {},
								object: self.#delayedPromises.object.status.type === "rejected" ? void 0 : self.#delayedPromises.object.status.type === "resolved" ? self.#delayedPromises.object.status.value : self.#bufferedObject !== void 0 ? self.#bufferedObject : self.#structuredOutputMode === "direct" && baseFinishStep.text ? (() => {
									try {
					...
					case "goal":
						...
						break;
					case "error":
						const error = require_error.getErrorFromUnknown(chunk.payload.error, { fallbackMessage: "Unknown error chunk in stream" });
						self.#error = error;
						self.#status = "failed";
						self.#streamFinished = true;
			...
			flush: () => {
				if (self.#delayedPromises.object.status.type === "pending") self.#delayedPromises.object.resolve(self.#bufferedObject);
	...
	resolvePromise(key, value) {
		if (!(key in this.#delayedPromises)) throw new require_error.MastraError({
	...
	*/
	...
	updateUsageCount(usage) {
		...
		if (usage.cacheCreationInputTokens5m !== void 0) this.#usageCount.cacheCreationInputTokens5m = (this.#usageCount.cacheCreationInputTokens5m ?? 0) + usage.cacheCreationInputTokens5m;
		if (usage.cacheCreationInputTokens1h !== void 0) this.#usageCount.cacheCreationInputTokens1h = (this.#usageCount.cacheCreationInputTokens1h ?? 0) + usage.cacheCreationInputTokens1h;
	...
	populateUsageCount(usage) {
		...
		if (usage.cacheCreationInputTokens5m !== void 0 && this.#usageCount.cacheCreationInputTokens5m === void 0) this.#usageCount.cacheCreationInputTokens5m = usage.cacheCreationInputTokens5m;
		if (usage.cacheCreationInputTokens1h !== void 0 && this.#usageCount.cacheCreationInputTokens1h === void 0) this.#usageCount.cacheCreationInputTokens1h = usage.cacheCreationInputTokens1h;
	...
	*/
	get objectStream() {
		return this.#createEventedStream().pipeThrough(new stream_web.TransformStream({ transform(chunk, controller) {
...
const getModelOutputForTripwire = async ({ tripwire, runId, options, model, messageList, ...rest }) => {
	...
	const tripwireStream = new stream_web.ReadableStream({ start(controller) {
		controller.enqueue({
			type: "tripwire",
			runId,
			from: "AGENT",
...
//#endregion
Object.defineProperty(exports, "DelayedPromise", {
	enumerable: true,
	get: function() {
		return DelayedPromise;
	}
});
Object.defineProperty(exports, "MastraModelInput", {
	enumerable: true,
	get: function() {
		return MastraModelInput;
	}
});
Object.defineProperty(exports, "MastraModelOutput", {
	enumerable: true,
	get: function() {
		return MastraModelOutput;
	}
});
...
Object.defineProperty(exports, "REPROCESS_PART_KEY", {
	enumerable: true,
	get: function() {
		return REPROCESS_PART_KEY;
	}
});
...
Object.defineProperty(exports, "createDestructurableOutput", {
	enumerable: true,
	get: function() {
		return createDestructurableOutput;
	}
});
Object.defineProperty(exports, "createProcessorSendSignal", {
	enumerable: true,
	get: function() {
		return createProcessorSendSignal;
	}
});
Object.defineProperty(exports, "escapeUnescapedControlCharsInJsonStrings", {
	enumerable: true,
	get: function() {
		return escapeUnescapedControlCharsInJsonStrings;
	}
});
Object.defineProperty(exports, "getModelOutputForTripwire", {
	enumerable: true,
	get: function() {
		return getModelOutputForTripwire;
	}
});
Object.defineProperty(exports, "getResponseFormat", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "safeClose", {
	enumerable: true,
	get: function() {
		return safeClose;
	}
});
Object.defineProperty(exports, "safeEnqueue", {
	enumerable: true,
	get: function() {
		return safeEnqueue;
	}
});
Object.defineProperty(exports, "summarizeActiveToolsForSpan", {
	enumerable: true,
	get: function() {
		return summarizeActiveToolsForSpan;
	}
});
Object.defineProperty(exports, "summarizeProcessorModelForSpan", {
	enumerable: true,
	get: function() {
		return summarizeProcessorModelForSpan;
	}
});
Object.defineProperty(exports, "summarizeProcessorResultForSpan", {
	enumerable: true,
	get: function() {
		return summarizeProcessorResultForSpan;
	}
});
Object.defineProperty(exports, "summarizeProcessorToolsForSpan", {
	enumerable: true,
	get: function() {
		return summarizeProcessorToolsForSpan;
	}
});
Object.defineProperty(exports, "summarizeToolChoiceForSpan", {
	enumerable: true,
	get: function() {
		return summarizeToolChoiceForSpan;
	}
});
Object.defineProperty(exports, "supportedLanguageModelSpecifications", {
	enumerable: true,
	get: function() {
		return supportedLanguageModelSpecifications;
	}
});
Object.defineProperty(exports, "tryGenerateWithJsonFallback", {
	enumerable: true,
	get: function() {
		return tryGenerateWithJsonFallback;
	}
});
Object.defineProperty(exports, "tryStreamWithJsonFallback", {
	enumerable: true,
	get: function() {
		return tryStreamWithJsonFallback;
	}
});

//# sourceMappingURL=trip-wire-B-TPWnWV.cjs.map