...
import { o as RequestContext } from "./request-context-bTdjxlgA.js";
import { a as ModelRouterLanguageModel, c as defaultGateways, o as GatewayManager } from "./llm-Dz6Jt17j.js";
import { r as createTool } from "./tool-BwroCWv4.js";
import { toStandardSchema } from "./schema/index.js";
import { safeStringify } from "./utils/safe-stringify.js";
import { X as CHAT_CHANNEL_RENDER_CONTEXT_KEY, p as getServerSideFallbackInfo, t as Agent } from "./agent-D5C9QXkF.js";
import { n as hasTransformedToolPayload, t as getTransformedToolPayload } from "./payload-transform-C4k4-WlM.js";
import { n as createSignal } from "./signals-wX3sXZ79.js";
import { d as taskCheckTool, f as taskCompleteTool, m as taskWriteTool, p as taskUpdateTool, u as summarizeTaskCheck } from "./task-state-processor-BQwvY7TE.js";
import { t as createWorkspaceTools, x as Workspace } from "./workspace-ZZhrDe7P.js";
import { s as askUserTool, t as submitPlanTool } from "./tools-BnWVqKMq.js";
import { t as AgentControllerChannels } from "./channels-f7wk3K5I.js";
import { t as Mastra } from "./mastra-S88Af5nD.js";
...
const ABORT_STREAM_GRACE_MS = 5e3;
...
* race is already won).
*/
async function abortDeadline(run, guard, graceMs) {
	while (!guard.aborted) {
		...
		if (await new Promise((resolve) => {
			...
			run.waitForTeardown(guard).then(() => {
				clearTimeout(timer);
				resolve(false);
			});
		}) && run.isAbortRequested()) break;
	}
	return abortBailed;
}
/**
...
var SessionRunEngine = class {
	...
	*/
	...
	createStreamState() {
		return {
			...
			offeredResponseIds: /* @__PURE__ */ new Set()
		};
	}
	/**
	...
	*/
	applyToolOutcome(state, outcome) {
	...
	*/
	async processStream(response, requestContextInput) {
		...
		try {
			bailed = await Promise.race([consume(), abortDeadline(this.#session.run, bailGuard.signal, ABORT_STREAM_GRACE_MS)]) === abortBailed;
		} finally {
			bailGuard.abort();
		}
		result ??= this.finishStreamState(state);
		...
		await this.#session.finishAgentRun(error ? "error" : result.suspended ? "suspended" : aborted || this.#session.run.isAbortRequested() ? "aborted" : "complete");
	...
	async processStreamChunk(state, chunk, requestContext) {
		...
		switch (chunk.type) {
			case "step-start": {
				...
				if (!this.hasCurrentMessageContent(state)) state.currentMessage.id = messageId;
				break;
			}
			case "text-start": {
			...
			case "tool-result": {
				...
				this.applyToolOutcome(state, {
					...
					providerMetadata: isProviderMetadata(toolResult.providerMetadata) ? toolResult.providerMetadata : void 0
				});
				break;
			}
			case "tool-error": {
				...
				this.applyToolOutcome(state, {
					...
					isError: true,
					providerMetadata: isProviderMetadata(toolError.providerMetadata) ? toolError.providerMetadata : void 0
				});
				break;
			}
			case "tool-output-denied": {
				...
				const args = hasTransformedToolPayload(approvalTransform) ? approvalTransform.transformed : getDisplayTransform(chunk.metadata, "input-available", payload.args);
				const toolIndex = state.toolPartById.get(toolCallId);
				const existing = toolIndex !== void 0 ? state.currentMessage.content.parts[toolIndex] : void 0;
				...
				if (existing && existing.type === "tool-invocation") existing.toolInvocation = Object.assign(existing.toolInvocation, toolInvocation);
				else state.currentMessage.content.parts.push({
					type: "tool-invocation",
					toolInvocation
				});
				this.#session.emit({
			...
			case "tool-call-approval": {
				...
				if (!deferredAbort && approval.decision === "approve") await this.#session.approveToolCall({
					toolCallId,
					...
					declineContext: deferredAbort ? {
						...
						message: ABORTED_BY_USER_REASON
					} : approval.declineContext
				});
				if (deferredAbort) {
					this.settleToolCallAsDenied(state, {
						toolCallId,
						toolName,
						args: toolArgs
					});
			...
			case "step-finish": {
				...
				if (usageRecord) {
					...
					addOptionalUsageField$1(stepUsage, "cacheCreationInputTokens5m", getUsageNumber(usageRecord, "cacheCreationInputTokens5m"));
					addOptionalUsageField$1(stepUsage, "cacheCreationInputTokens1h", getUsageNumber(usageRecord, "cacheCreationInputTokens1h"));
			...
			case "data-sandbox-exit": {
				...
				if (toolCallId && exitCode !== void 0) this.#session.emit({
					type: "command_exit",
					toolCallId,
					exitCode,
	...
	*/
	settleToolCallAsDenied(state, { toolCallId, toolName, args }) {
		const toolInvocation = {
			state: "output-denied",
			toolCallId,
			toolName,
	...
	async handleSubscribedStreamError(error) {
		if (error instanceof Error && error.name === "AbortError") await this.#session.finishAgentRun("aborted");
		else {
			this.#session.emit({
				type: "error",
	...
	async processSubscribedThreadStream(subscription) {
		...
		const consume = async () => {
			for await (const chunk of subscription.stream) {
				...
				if (!currentRun) {
					const runId = ("runId" in chunk ? chunk.runId : void 0) ?? subscription.activeRunId();
		...
		try {
			...
			if (bailed && this.#session.stream.isCurrent({ subscription })) this.#session.stream.detach();
		} catch (error) {
...
*/
const ABORTED_BY_USER_REASON = "Aborted by the user";
/**
...
* in-memory only).
*/
const PERSISTED_STATE_KEYS = ["thinkingLevel", "notifications"];
...
var SessionThread = class {
	...
	*/
	...
	async setSettingOn({ threadId, key, value }) {
		if (!this.#store) return;
...
var SessionSuspensions = class {
	...
	*/
	clear() {
		const dropped = [...this.#pending].map(([toolCallId, { toolName }]) => ({
			toolCallId,
			toolName
		}));
		this.#pending.clear();
...
var SessionRun = class {
	...
	#teardownWaiters = /* @__PURE__ */ new Set();
	#abortRequestWaiters = /* @__PURE__ */ new Set();
	#notifyTeardown() {
		const waiters = [...this.#teardownWaiters];
		this.#teardownWaiters.clear();
		for (const waiter of waiters) waiter();
...
var SessionState = class {
	...
	#schema;
	#bus;
	#capturePersistSetting;
	constructor({ initialState, stateSchema }, bus, capturePersistSetting) {
	...
	async apply(updates, persistSetting) {
	...
	update(updater) {
		...
		const run = this.#updateQueue.then(async () => {
			...
			if (update.updates && Object.keys(update.updates).length > 0) await this.apply(update.updates, persistSetting);
...
*/
const COALESCIBLE_DISPLAY_STATE_EVENTS = /* @__PURE__ */ new Set(["message_update", "tool_input_delta"]);
...
var SessionBus = class {
	...
	*/
	...
	emit(event) {
		...
		if (!COALESCIBLE_DISPLAY_STATE_EVENTS.has(event.type)) this.#flushDisplayState();
		this.#dispatch(event);
	...
	*/
	#scheduleDisplayState() {
		...
		this.#displayStateTimer = setTimeout(() => {
			this.#displayStateTimer = void 0;
			this.#flushDisplayState();
		}, DISPLAY_STATE_COALESCE_MS);
		this.#displayStateTimer.unref?.();
	}
	...
	#flushDisplayState() {
		...
		if (this.#displayState) this.#dispatch({
			type: "display_state_changed",
			displayState: this.#displayState.get()
		});
	}
...
var Session = class {
	...
	#beforeAgentEndListeners = /* @__PURE__ */ new Set();
	...
	*/
	...
	constructor({ resourceId, state, id, ownerId, tags, workspace, browser }) {
		...
		this.state = new SessionState(state ?? { initialState: {} }, this.#bus, () => {
			...
			if (threadId === null) return void 0;
			return (args) => this.thread.setSettingOn({
				threadId,
				...args
			});
		});
		if (workspace !== void 0 && !(workspace instanceof Workspace)) throw new Error(`A session workspace must be a valid Workspace instance.`);
	...
	*/
	...
	async finishAgentRun(reason) {
		const event = {
			type: "agent_end",
			reason
		};
		for (const listener of this.#beforeAgentEndListeners) try {
	...
	*/
	abortRun() {
		...
		for (const { toolCallId, toolName } of this.suspensions.clear()) this.emit({
			type: "tool_suspension_cancelled",
			toolCallId,
			toolName,
			reason: ABORTED_BY_USER_REASON
...
var AgentController = class {
	...
	* concurrent callers.
	*/
	#deletionsInProgress = /* @__PURE__ */ new Map();
	/**
	...
	* session.
	*/
	#sessionsBeingDeleted = /* @__PURE__ */ new WeakSet();
	/**
	...
	*/
	#sessionDeletionPromises = /* @__PURE__ */ new WeakMap();
	/**
	...
	*/
	onSessionCreated(listener, options) {
		...
		return () => {
			...
			if (index !== -1) this.#sessionCreatedListeners.splice(index, 1);
		};
	}
	async #notifySessionCreated(session) {
		...
		for (const { listener, blocking } of entries) {
			...
			try {
				...
				if (result && typeof result === "object" && "catch" in result) result.catch((error) => console.error("Error in session-created listener:", error));
			} catch (error) {
				console.error("Error in session-created listener:", error);
			}
		}
	...
	*/
	onSessionDeleted(listener) {
		...
		return () => {
			...
			if (index !== -1) this.#sessionDeletedListeners.splice(index, 1);
		};
	}
	#notifySessionDeleted(session) {
		for (const listener of [...this.#sessionDeletedListeners]) try {
			const result = listener(session);
			if (result && typeof result === "object" && "catch" in result) result.catch((error) => console.error("Error in session-deleted listener:", error));
		} catch (error) {
	...
	*/
	async deleteSession({ resourceId, scope }) {
		...
		const tolerantPromise = deletionPromise.then(() => void 0, () => void 0);
	...
	*/
	setBrowser(browser) {
		this.browser = browser;
		for (const agent of this.backingAgents()) agent.setBrowser(browser);
	}
	/**
	...
	*/
	...
	async runInit() {
		...
		for (const agent of this.backingAgents()) this.propagateRuntimeServicesToAgent(agent);
		this.startIntervals();
	}
	...
	*/
	setChannels(channels) {
		...
		for (const agent of this.backingAgents()) agent.setChannels(channels);
	}
	...
	*/
	...
	listActiveThreadRuns() {
		...
		for (const agent of this.backingAgents()) for (const run of this.propagateRuntimeServicesToAgent(agent).listActiveThreadRuns()) byRunId.set(run.runId, run);
	...
	*/
	async #dropSessionFromRegistry(registryKey, session) {
		...
		if (currentKey !== registryKey) {
			...
			if (currentPending) {
				if (await currentPending.catch(() => void 0) === session && this.#sessionsByResource.get(currentKey) === currentPending) this.#sessionsByResource.delete(currentKey);
			}
		}
	}
	...
	*/
	async loadOMProgress(session) {
		...
		if (!threadId) return;
		try {
			const record = await (await this.getMemoryStorage()).getObservationalMemory(threadId, session.identity.getResourceId());
			if (!record) return;
			...
			const observationThreshold = getThreshold(config?._overrides?.observation?.messageTokens ?? config?.observation?.messageTokens, 3e4);
			const reflectionThreshold = getThreshold(config?._overrides?.reflection?.observationTokens ?? config?.reflection?.observationTokens, 4e4);
			...
			let bufferedChunks = [];
			if (Array.isArray(rawChunks)) bufferedChunks = rawChunks;
			else if (typeof rawChunks === "string") try {
				const parsed = JSON.parse(rawChunks);
				if (Array.isArray(parsed)) bufferedChunks = parsed;
			} catch {
				bufferedChunks = [];
			}
			const bufferedMessageTokens = Math.min(bufferedChunks.reduce((sum, chunk) => sum + (chunk.messageTokens ?? 0), 0), messageTokens);
			const bufferedObservationTokens = bufferedChunks.reduce((sum, chunk) => sum + (chunk.tokenCount ?? 0), 0);
			const bufferedObs = {
				status: record.isBufferingObservation ? "running" : bufferedChunks.length ? "complete" : "idle",
			...
			const bufferedRef = {
				status: record.isBufferingReflection ? "running" : record.bufferedReflection ? "complete" : "idle",
...
//# sourceMappingURL=agent-controller-RdMQdq7w.js.map