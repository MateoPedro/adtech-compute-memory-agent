...
require("./request-context-CY1DtViW.cjs");
const require_utils = require("./utils-Ox89OFwL.cjs");
...
*/
const AGENT_THREAD_LEASE_TTL_MS = require_utils.readPositiveIntEnv("MASTRA_AGENT_THREAD_LEASE_TTL_MS", 15e3);
/**
...
*/
const AGENT_THREAD_LEASE_RENEW_INTERVAL_MS = require_utils.readPositiveIntEnv("MASTRA_AGENT_THREAD_LEASE_RENEW_INTERVAL_MS", Math.floor(AGENT_THREAD_LEASE_TTL_MS / 3));
/**
...
*/
const AGENT_SUSPENDED_RUN_TTL_MS = require_utils.readPositiveIntEnv("MASTRA_SUSPENDED_RUN_TTL_MS", 1800 * 1e3);
...
let defaultAgentThreadPubSub = new require_event_emitter.EventEmitterPubSub();
/**
...
var AgentThreadStreamRuntime = class {
	...
	*/
	#getLeaseProvider(pubsub) {
		...
		if (typeof unwrap === "function") return unwrap.call(resolved) ?? require_event_emitter.NoopLeaseProvider;
	...
	#resolveLeaseProvider(pubsub) {
		...
		return {
			provider,
			isFallback: provider === require_event_emitter.NoopLeaseProvider
		};
	}
	...
	*/
	...
	resetForTests() {
		...
		defaultAgentThreadPubSub = new require_event_emitter.EventEmitterPubSub();
	}
	#resetState(pubsub) {
	...
	*/
	...
	async #drainPendingSignals(state, pubsub, key, previousRun) {
		...
		try {
			...
			this.#publish(pubsub, key, {
				type: "run-failed",
				runId: failedRunId,
				error: `failed to start follow-up run for queued message: ${require_error.getErrorFromUnknown(err).message}; the message was requeued and will deliver on the next turn`
			});
	...
	#startContinuation(state, pubsub, key, pending) {
		...
		pending.agent.stream(pending.messages, {
			...
			this.#publish(pubsub, key, {
				type: "run-failed",
				runId: pending.runId,
				error: require_error.getErrorFromUnknown(err).message
			});
	...
	continueWithMessages(agent, messages, target, pubsub) {
		...
		const runId = target.runId ?? (0, crypto.randomUUID)();
		const pending = {
			agent,
			messages,
			runId,
	...
	*/
	...
	async subscribeToThread(agent, options, pubsub) {
		...
		const handleEvent = async (event) => {
			...
			if (data.type === "signal-enqueued") {
				...
				const queue = signalsByThread.get(key) ?? [];
				queue.push(require_signals.createSignal(data.signal));
	...
	queueMessage(agent, message, target, pubsub) {
		...
		const signal = require_signals.createMessageSignal(message, {
		...
		const queuedRunId = (0, crypto.randomUUID)();
	...
	async sendStateSignal(agent, stateInput, target, pubsub) {
		...
		const applied = await require_types.applyStateSignal({
			input: stateInput,
			memory,
			thread: {
				...loadedThread,
	...
	*/
	sendSignal(agent, signalInput, target, pubsub) {
		...
		let signal = require_signals.createSignal({
			...signalInput,
		...
		signal = require_signals.resolveDeliveryAttributes(signal, isActiveTarget ? target.ifActive?.attributes : target.ifIdle?.attributes);
		...
		const accepted = (async () => {
			...
			try {
				...
				this.#publish(pubsub, reservedKey, {
					type: "run-failed",
					runId: reservedRunId,
					error: require_error.getErrorFromUnknown(error).message
				});
...
function createNotificationSignal(notification) {
	return require_signals.createSignal({
		type: "notification",
...
var NotificationsStorage = class extends require_base.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "NOTIFICATIONS"
...
var InMemoryNotificationsStorage = class extends NotificationsStorage {
	...
	async createNotification(input) {
		...
		const record = {
			id: input.id ?? (0, crypto.randomUUID)(),
...
//#endregion
Object.defineProperty(exports, "AgentThreadLeaseConflictError", {
	enumerable: true,
	get: function() {
		return AgentThreadLeaseConflictError;
	}
});
Object.defineProperty(exports, "InMemoryNotificationsStorage", {
	enumerable: true,
	get: function() {
		return InMemoryNotificationsStorage;
	}
});
Object.defineProperty(exports, "MAX_NOTIFICATION_DELIVERY_ATTEMPTS", {
	enumerable: true,
	get: function() {
		return MAX_NOTIFICATION_DELIVERY_ATTEMPTS;
	}
});
...
Object.defineProperty(exports, "agentThreadStreamRuntime", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "notificationSignalAttributes", {
	enumerable: true,
	get: function() {
		return notificationSignalAttributes;
	}
});
Object.defineProperty(exports, "notificationSignalMetadata", {
	enumerable: true,
	get: function() {
		return notificationSignalMetadata;
	}
});
Object.defineProperty(exports, "notificationSummaryContents", {
	enumerable: true,
	get: function() {
		return notificationSummaryContents;
	}
});
Object.defineProperty(exports, "notificationSummarySignalMetadata", {
	enumerable: true,
	get: function() {
		return notificationSummarySignalMetadata;
	}
});
Object.defineProperty(exports, "resolveDeliveryFailureUpdate", {
	enumerable: true,
	get: function() {
		return resolveDeliveryFailureUpdate;
	}
});
...
//# sourceMappingURL=storage-DvYq_0a1.cjs.map