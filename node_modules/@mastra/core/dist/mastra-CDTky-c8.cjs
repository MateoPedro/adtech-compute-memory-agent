...
const require_gateway_helpers = require("./gateway-helpers-B_IPtGRQ.cjs");
const require_define = require("./define-DUmMT-Cb.cjs");
const require_cron = require("./cron-CrmzJKFJ.cjs");
const require_schedules = require("./schedules-DXCSqlKT.cjs");
const require_logger_index = require("./logger/index.cjs");
const require_workflow_event_processor = require("./workflow-event-processor-CzW_1uHR.cjs");
...
const require_create_durable_agent = require("./create-durable-agent-CkAOOOwc.cjs");
const require_manager = require("./manager-D99YS21Q.cjs");
...
const require_tool_loop_agent = require("./tool-loop-agent-BdG7csnF.cjs");
const require_worker = require("./worker-CyD51SCP.cjs");
const require_builder = require("./builder-CbWxFrTo.cjs");
const require_validate = require("./validate-DQmAb1Gs.cjs");
const require_rehydrate = require("./rehydrate-CFEAhHuW.cjs");
...
function collectFsAgentSchedules(agents) {
	...
	for (const agent of Object.values(agents ?? {})) for (const { key, definition } of declaredSchedulesOf(agent)) {
		const scheduleId = require_define.fsAgentScheduleRowId(agent.id, key);
...
function findFsAgentScheduleHandler(agents, scheduleId) {
	const parsed = require_define.parseFsAgentScheduleRowId(scheduleId);
...
async function syncFsAgentSchedules({ agents, store, logger, knownRows }) {
	...
	for (const { scheduleId, agentId, key, definition } of declared) try {
		...
		if (!existing) {
			await store.createSchedule({
				...
				nextFireAt: require_cron.computeNextFireAt(definition.cron, {
					timezone: definition.timezone,
					after: now
				}),
				createdAt: now,
		...
		if (cronChanged || timezoneChanged) patch.nextFireAt = require_cron.computeNextFireAt(definition.cron, {
			timezone: definition.timezone,
			after: now
		});
	...
	for (const row of allRows) {
		...
		const ownerAgentId = require_define.parseFsAgentScheduleRowId(row.id)?.agentId;
...
var Mastra = class Mastra {
		...
		if (!this.#pubsubProxy) {
			...
			this.#pubsubProxy = new Proxy(raw, { get(target, prop, _receiver) {
				if (prop === "publish") return function publish(topic, event) {
					...
					} else if (require_topics.isRunLocalTopic(topic)) return target.publish(topic, event, { localOnly: true });
	...
	*/
	async addDynamicWorkflows(defs) {
		...
		const members = defs.map((def) => ({ normalized: require_builder.normalizeWorkflowBuilderDefinition({
			id: def.id,
		...
		for (const { normalized } of members) require_validate.assertValidDynamicWorkflow(normalized, index);
		const ordered = [];
		...
		while (remaining.size > 0 && progress) {
			...
			for (const [id, member] of Array.from(remaining)) {
				if (Array.from(require_validate.collectNestedWorkflowIds(member.normalized.graph)).filter((dependency) => dependency !== id && bundleIds.has(dependency) && !hydrated.has(dependency)).length > 0) continue;
				remaining.delete(id);
				hydrated.add(id);
				ordered.push(member);
				progress = true;
		...
		try {
			for (const { normalized } of ordered) {
				const { workflow } = await require_rehydrate.rehydrateWorkflow(normalized, this);
	...
	async #loadDynamicWorkflows() {
		...
		while (remaining.size > 0 && progress) {
			...
			for (const [id, def] of Array.from(remaining)) {
				...
				progress = true;
				try {
					const { workflow } = await require_rehydrate.rehydrateWorkflow({
						id: def.id,
						description: def.description,
						metadata: def.metadata,
						inputSchema: def.inputSchema,
	...
	*/
	...
	async #startSchedulingWorkers() {
		...
		if (!this.#findAgentScheduleWorker()) {
			const { AgentScheduleWorker } = await Promise.resolve().then(() => require("./worker-DDjMxBCK.cjs"));
	...
	*/
	#syncGatewayRegistry() {
		try {
			...
			Promise.resolve().then(() => require("./provider-registry-Bpx3y4iE.cjs")).then((n) => n.provider_registry_exports).then(async ({ GatewayRegistry }) => {
	...
	*/
	async shutdown() {
		...
		(await Promise.allSettled(require_create_durable_agent.getActiveDurableAgentWorkflowExecutions(this))).forEach((result) => {
...
//# sourceMappingURL=mastra-CDTky-c8.cjs.map