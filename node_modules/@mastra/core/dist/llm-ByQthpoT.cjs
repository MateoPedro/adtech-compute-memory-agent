...
const require_model = require("./model-BuFJRmwx.cjs");
...
function remapToolResultMediaPartsToV3(options) {
	return {
		...options,
		prompt: require_to_prompt.aiV5PromptToAIV6Prompt(options.prompt)
	};
}
...
var GatewayManager = class GatewayManager {
	...
	*/
	async resolveAuth(routerId) {
		...
		if (require_gateway_helpers.hasAuthCredentials(gatewayAuth)) return {
			...gatewayAuth,
	...
	*/
	async hasAuth(routerId) {
		try {
			return require_gateway_helpers.hasAuthCredentials(await this.resolveAuth(routerId));
		} catch (error) {
...
async function resolveModelAuth({ gateway, request, explicit }) {
	if (require_gateway_helpers.hasAuthCredentials(explicit)) return mergeAuthHeaders({
		...explicit,
		source: "explicit"
	}) ?? { source: "explicit" };
...
//# sourceMappingURL=llm-ByQthpoT.cjs.map