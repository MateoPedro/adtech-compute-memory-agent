...
//#region src/workflows/dynamic/graph.ts
/**
...
//#endregion
//#region src/workflows/dynamic/validate/refs.ts
/**
...
//#endregion
//#region src/workflows/dynamic/validate/schema-utils.ts
/**
...
function toJsonSchemaOrUndefined(schema) {
	...
	try {
		return (0, _mastra_schema_compat_schema.standardSchemaToJSONSchema)((0, _mastra_schema_compat_schema.toStandardSchema)(schema));
	} catch {
		return;
	}
}
...
//#endregion
//#region src/workflows/dynamic/validate/types.ts
...
//#endregion
//#region src/workflows/dynamic/validate/repair-actions.ts
...
//#endregion
//#region src/workflows/dynamic/mapping-config.ts
/**
...
function analyzeMapConfig(rawConfig, opts) {
	...
	for (const [key, descriptor] of Object.entries(config)) {
		...
		if (typeof descriptor.template === "string") {
			let syntaxError;
			try {
				require_workflow_event_processor.validateTemplate(descriptor.template);
			} catch (err) {
			...
			const unknownStep = syntaxError === void 0 ? require_workflow_event_processor.collectTemplateStepIds(descriptor.template).find((stepId) => !availableOutputs.has(stepId)) : void 0;
...
//#endregion
//#region src/workflows/dynamic/validate/schema-flow.ts
...
//#endregion
//#region src/workflows/dynamic/json-schema-to-zod.ts
/**
...
function walk(schema, opts) {
	if (!schema || typeof schema !== "object") return zod.z.any();
	for (const key of UNSUPPORTED_SCHEMA_KEYS) if (key in schema) return unsupported(`Dynamic workflow schema uses unsupported JSON Schema keyword "${key}". This converter only supports the static subset that Zod round-trips through standardSchemaToJSONSchema (object, array, string, number, integer, boolean, null, enum, const). Simplify the schema or extend jsonSchemaToZod to cover this keyword.`, opts);
	let out;
	if ("const" in schema) {
		if (!isLiteralValue(schema.const)) return unsupported(`Dynamic workflow schema uses a non-primitive "const" value (${JSON.stringify(schema.const)}). Only string, number, boolean, and null literals are supported.`, opts);
		out = zod.z.literal(schema.const);
	} else if (Array.isArray(schema.enum) && schema.enum.length > 0) {
		const values = schema.enum;
		if (!values.every(isLiteralValue)) return unsupported("Dynamic workflow schema uses an \"enum\" with non-primitive members. Only string, number, boolean, and null enum members are supported.", opts);
		if (values.every((v) => typeof v === "string")) out = zod.z.enum(values);
		else {
			const literals = values.map((v) => zod.z.literal(v));
			out = literals.length === 1 ? literals[0] : zod.z.union(literals);
		}
		...
		case "object": {
			...
			const obj = zod.z.object(shape);
			out = schema.additionalProperties === true ? obj.passthrough() : obj;
			break;
		}
		case "array":
			if (Array.isArray(schema.items)) return unsupported("Dynamic workflow schema uses tuple-form \"items\" (an array of positional schemas). Only a single item schema is supported; use \"items\": { ... } instead.", opts);
			out = zod.z.array(walk(schema.items ?? {}, opts));
			break;
		case "string":
			out = zod.z.string();
			break;
		...
		case void 0:
			out = zod.z.any();
			break;
		default: return unsupported(`Dynamic workflow schema uses unsupported JSON Schema type "${String(schema.type)}". This converter only supports object, array, string, number, integer, boolean, null, and enum.`, opts);
	}
...
//#endregion
//#region src/workflows/dynamic/validate/schemas.ts
/**
...
//#endregion
//#region src/workflows/dynamic/validate/structure.ts
/**
...
//#endregion
//#region src/workflows/dynamic/validate/index.ts
/**
...
*/
function validateDynamicWorkflow(def, index = {}) {
...
function assertValidDynamicWorkflow(def, index = {}) {
...
//#endregion
Object.defineProperty(exports, "analyzeMapConfig", {
	enumerable: true,
	get: function() {
		return analyzeMapConfig;
	}
});
Object.defineProperty(exports, "assertValidDynamicWorkflow", {
	enumerable: true,
	get: function() {
		return assertValidDynamicWorkflow;
	}
});
Object.defineProperty(exports, "collectNestedWorkflowIds", {
	enumerable: true,
	get: function() {
		return collectNestedWorkflowIds;
	}
});
Object.defineProperty(exports, "forEachSingleStepEntry", {
	enumerable: true,
	get: function() {
		return forEachSingleStepEntry;
	}
});
Object.defineProperty(exports, "forEachSingleStepEntryWithPath", {
	enumerable: true,
	get: function() {
		return forEachSingleStepEntryWithPath;
	}
});
Object.defineProperty(exports, "inferGraphSchemas", {
	enumerable: true,
	get: function() {
		return inferGraphSchemas;
	}
});
...
Object.defineProperty(exports, "parseMapConfig", {
	enumerable: true,
	get: function() {
		return parseMapConfig;
	}
});
Object.defineProperty(exports, "schemaCompatibility", {
	enumerable: true,
	get: function() {
		return schemaCompatibility;
	}
});
Object.defineProperty(exports, "toJsonSchemaOrUndefined", {
	enumerable: true,
	get: function() {
		return toJsonSchemaOrUndefined;
	}
});
Object.defineProperty(exports, "validateDynamicWorkflow", {
	enumerable: true,
	get: function() {
		return validateDynamicWorkflow;
	}
});
Object.defineProperty(exports, "validateStorableJsonSchema", {
	enumerable: true,
	get: function() {
		return validateStorableJsonSchema;
	}
});
Object.defineProperty(exports, "validateWorkflowRefs", {
	enumerable: true,
	get: function() {
		return validateWorkflowRefs;
	}
});
Object.defineProperty(exports, "validateWorkflowSchemas", {
	enumerable: true,
	get: function() {
		return validateWorkflowSchemas;
	}
});
Object.defineProperty(exports, "validateWorkflowStructure", {
	enumerable: true,
	get: function() {
		return validateWorkflowStructure;
	}
});

//# sourceMappingURL=validate-DQmAb1Gs.cjs.map