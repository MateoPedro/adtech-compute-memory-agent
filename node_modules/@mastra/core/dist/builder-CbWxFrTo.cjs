const require_agent = require("./agent-BVtn9FqD.cjs");
const require_validate = require("./validate-DQmAb1Gs.cjs");
let zod = require("zod");
//#region src/workflows/builder/authoring-playbook.ts
const WORKFLOW_BUILDER_AUTHORING_CONSTRAINTS = `# Persisted workflow authoring contract
...
const WORKFLOW_BUILDER_AUTHORING_PLAYBOOK = `${WORKFLOW_BUILDER_AUTHORING_CONSTRAINTS}
...
//#endregion
//#region src/workflows/builder/preflight.ts
/**
...
*/
function preflightWorkflowDefinition(definition, context = {}) {
	const issues = require_validate.validateDynamicWorkflow(definition, context);
...
//#endregion
//#region src/workflows/builder/inspection.ts
function inspectWorkflowBuilderSchemas(definition, registry = {}) {
	return require_validate.inferGraphSchemas(definition, registry);
}
function compareWorkflowBuilderSchemas(source, destination) {
	return require_validate.schemaCompatibility(source, destination);
}
//#endregion
//#region src/workflows/builder/authoring-schema.ts
/**
...
* each surface.
*/
const WORKFLOW_BUILDER_MAPPING_CONFIG_DESCRIPTION = "An object whose top-level keys become the mapping output fields. Each value must use exactly one canonical source form: { \"template\": \"<text with ${placeholders}>\" }, { \"value\": <constant> }, { \"step\": \"<stepId>\", \"path\": \"<field.path>\" }, { \"initData\": true, \"path\": \"<workflow-input-field.path>\" }, or { \"requestContextPath\": \"<field.path>\" }. IMPORTANT: initData is the boolean true, never a field name string; put the workflow input field name in path. Template placeholders use JavaScript-style ${initData.<field>}, ${inputData.<field>}, ${stepResults.<stepId>.<field>}, ${state.<field>}, or ${requestContext.<field>} — never Handlebars {{...}} and never separate sources/data bindings. May also be provided as a JSON-encoded string of the same object.";
const jsonSchema = zod.z.record(zod.z.string(), zod.z.unknown());
const STEP_OPTIONS_DESCRIPTION = "JSON-safe subset of step options that round-trips through storage. `onFinish` callbacks and function-valued scorers are NOT supported.";
const stepOptionsSchema = zod.z.object({
	retries: zod.z.number().int().nonnegative().optional().describe("Retry count on failure. Static number only."),
	metadata: jsonSchema.optional().describe("Arbitrary JSON-safe metadata attached to the step.")
}).optional().describe(STEP_OPTIONS_DESCRIPTION);
const stepOptionsInputSchema = zod.z.object({
	retries: zod.z.number().int().nonnegative().nullish().describe("Retry count on failure. Static number only."),
	metadata: zod.z.unknown().nullish().describe("Arbitrary JSON-safe metadata object attached to the step.")
}).nullish().describe(STEP_OPTIONS_DESCRIPTION);
const agentOutputSchemaDescription = "OPTIONAL JSON Schema (Draft 2020-12) describing the structured output the agent must produce for this step. When set, the agent runs with structured output and the step's output IS that shape (not `{ text: string }`). Use this when a downstream step needs a machine-readable field — for example, an agent that reads a listing and emits `{ files: string[] }`, which a subsequent `foreach` iterates over.";
const AGENT_ENTRY_DESCRIPTION = "Agent step. Default agents consume { prompt: string } and return { text: string }; insert a mapping step producing { prompt } before the agent when shapes differ, and map its result from the `text` field afterwards — never invent output fields such as `response`. When `outputSchema` is set the output IS that schema shape instead. Use an agent ID returned by resource discovery; never invent IDs.";
const workflowBuilderAgentEntrySchema = zod.z.strictObject({
	type: zod.z.literal("agent"),
	id: zod.z.string().min(1).describe("Step id — kebab-case, unique within the workflow."),
	agentId: zod.z.string().min(1).describe("Id of an agent registered on this Mastra instance (from resource discovery)."),
	description: zod.z.string().optional(),
	outputSchema: zod.z.unknown().optional().describe(agentOutputSchemaDescription),
	options: stepOptionsSchema
}).describe(AGENT_ENTRY_DESCRIPTION);
const workflowBuilderAgentEntryInputSchema = workflowBuilderAgentEntrySchema.extend({
	description: zod.z.string().nullish(),
	outputSchema: zod.z.unknown().nullish().describe(agentOutputSchemaDescription),
	options: stepOptionsInputSchema
}).describe(AGENT_ENTRY_DESCRIPTION);
const TOOL_ENTRY_DESCRIPTION = "Tool step. The previous step's output is validated against the tool's inputSchema and the step produces the tool's outputSchema shape exactly.";
const workflowBuilderToolEntrySchema = zod.z.strictObject({
	type: zod.z.literal("tool"),
	id: zod.z.string().min(1).describe("Step id — kebab-case, unique within the workflow."),
	toolId: zod.z.string().min(1).describe("Id of a tool registered on this Mastra instance (from resource discovery)."),
	description: zod.z.string().optional(),
	options: stepOptionsSchema
}).describe(TOOL_ENTRY_DESCRIPTION);
const workflowBuilderToolEntryInputSchema = workflowBuilderToolEntrySchema.extend({
	description: zod.z.string().nullish(),
	options: stepOptionsInputSchema
}).describe(TOOL_ENTRY_DESCRIPTION);
const workflowBuilderMappingDescriptorSchema = zod.z.union([
	zod.z.object({ value: zod.z.unknown() }).strict().describe("Constant source: { \"value\": <JSON value> }."),
	zod.z.object({ template: zod.z.string().min(1).describe("JavaScript-style interpolation using ${initData.name}, ${inputData.field}, ${stepResults.stepId.field}, ${state.field}, or ${requestContext.field}. Do not use Handlebars {{...}} or separate sources/data bindings.") }).strict().describe("Template source: { \"template\": \"Hello, ${initData.name}!\" }. The descriptor contains only template."),
	zod.z.object({ requestContextPath: zod.z.string().min(1) }).strict(),
	zod.z.object({
		initData: zod.z.literal(true),
		path: zod.z.string().min(1)
	}).strict().describe("Workflow-input source: { \"initData\": true, \"path\": \"field.path\" }. initData must be true."),
	zod.z.object({
		step: zod.z.union([zod.z.string().min(1), zod.z.array(zod.z.string().min(1)).min(1)]),
		path: zod.z.string()
	}).strict().describe("Prior-step source: { \"step\": \"step-id\", \"path\": \"field.path\" }.")
]).describe("Use exactly one source form. Never combine initData and step.");
const workflowBuilderMappingConfigSchema = zod.z.record(zod.z.string(), workflowBuilderMappingDescriptorSchema);
const workflowBuilderMappingEntrySchema = zod.z.strictObject({
	type: zod.z.literal("mapping"),
	id: zod.z.string().min(1).describe("Step id — kebab-case, unique within the workflow."),
	mapConfig: zod.z.string().min(1).describe(`A JSON-ENCODED STRING of ${WORKFLOW_BUILDER_MAPPING_CONFIG_DESCRIPTION}`)
}).describe("Mapping step. Its output is an object whose top-level keys are exactly the keys of mapConfig.");
const workflowBuilderMappingEntryInputSchema = zod.z.strictObject({
	type: zod.z.literal("mapping"),
	id: zod.z.string().min(1).describe("Step id — kebab-case, unique within the workflow."),
	mapConfig: zod.z.union([workflowBuilderMappingConfigSchema, zod.z.string().min(1)]).describe(WORKFLOW_BUILDER_MAPPING_CONFIG_DESCRIPTION)
}).describe("Mapping step. Its output is an object whose top-level keys are exactly the keys of mapConfig.");
const workflowBuilderNestedWorkflowEntrySchema = zod.z.strictObject({
	type: zod.z.literal("workflow"),
	id: zod.z.string().min(1).describe("Call-site step id — kebab-case, unique within the workflow. This is the id that addresses this step result (stepResults.<id>); it does not need to equal workflowId."),
	workflowId: zod.z.string().min(1).describe("Authoritative ID of another workflow registered on this Mastra instance, exactly as returned by resource discovery. Never invent workflow IDs, self-reference, or create cycles."),
	description: zod.z.string().optional(),
	options: stepOptionsSchema
}).describe("Nested workflow step. The referenced workflow runs as a single step: its input is the current step input (a first top-level nested workflow receives the parent input directly when schemas match) and its output becomes this step output. Map its output through stepResults.<id> — this step own id — when a different final shape is required.");
const workflowBuilderNestedWorkflowEntryInputSchema = workflowBuilderNestedWorkflowEntrySchema.extend({
	description: zod.z.string().nullish(),
	options: stepOptionsInputSchema
}).describe(String(workflowBuilderNestedWorkflowEntrySchema.description));
const executableInnerStepSchema = zod.z.union([
	workflowBuilderAgentEntrySchema,
	workflowBuilderToolEntrySchema,
	workflowBuilderNestedWorkflowEntrySchema
]);
const executableInnerStepInputSchema = zod.z.union([
	workflowBuilderAgentEntryInputSchema,
	workflowBuilderToolEntryInputSchema,
	workflowBuilderNestedWorkflowEntryInputSchema
]);
const literalScalarSchema = zod.z.union([
	zod.z.string(),
	zod.z.number(),
	zod.z.boolean(),
	zod.z.null()
]);
const predicatePathSchema = zod.z.string().regex(/^(initData|inputData|stepResults|state)(\.[A-Za-z0-9_-]+)*$/, "Use a canonical predicate path root.").describe("Declarative path: initData.<field> for workflow input, inputData.<field> for the previous step output, stepResults.<stepId>.<field> for another step output, or state.<field>.");
const pathOrLiteralSchema = zod.z.union([zod.z.strictObject({ path: predicatePathSchema }), zod.z.strictObject({ literal: literalScalarSchema })]);
const workflowBuilderPredicateSchema = zod.z.lazy(() => zod.z.union([
	zod.z.strictObject({
		op: zod.z.enum([
			"eq",
			"ne",
	...
	zod.z.strictObject({
		op: zod.z.enum(["in", "notIn"]),
		value: pathOrLiteralSchema,
		set: zod.z.array(literalScalarSchema).min(1)
	}),
	zod.z.strictObject({
		op: zod.z.enum(["exists", "notExists"]),
		path: predicatePathSchema
	}),
	zod.z.strictObject({
		op: zod.z.enum(["truthy", "falsy"]),
		value: pathOrLiteralSchema
	}),
	zod.z.strictObject({
		op: zod.z.enum(["and", "or"]),
		args: zod.z.array(workflowBuilderPredicateSchema).min(1)
	}),
	zod.z.strictObject({
		op: zod.z.literal("not"),
		arg: workflowBuilderPredicateSchema
	})
]));
const PARALLEL_DESCRIPTION = "Parallel container. Each child receives the same preceding input and children must be agent/tool/nested workflow — no nested containers or mappings. The result is an object keyed by each child step id containing that child complete output; downstream steps pluck fields via stepResults.<childId>.<field>.";
const FOREACH_DESCRIPTION = "Foreach container. The preceding output MUST be a raw array (not an object with an array field). Each item is passed directly to the child step — no child inputMapping — and the output is an array of child outputs, order preserved. Give the inner step its own unique id.";
const CONDITIONAL_DESCRIPTION = "Conditional container. Predicates align by index with steps and use declarative initData, inputData, stepResults, or state paths; every branch whose predicate is truthy runs on the same preceding input. Branch results are keyed by the BRANCH step ids — map from those, never from the container id, which is not a readable step result. To collapse mutually exclusive branches into one field, give the mapping source a step ARRAY: { \"step\": [\"urgent-support\", \"normal-support\"], \"path\": \"text\" } selects whichever branch actually ran. Add a final mapping after the conditional whenever the branch result does not already match outputSchema.";
const LOOP_DESCRIPTION = "`dowhile` keeps looping while the predicate is TRUE; `dountil` keeps looping until the predicate is TRUE (exit condition). The inner step runs at least once and receives its own previous output on later iterations.";
const workflowBuilderParallelEntrySchema = zod.z.strictObject({
	type: zod.z.literal("parallel"),
	steps: zod.z.array(executableInnerStepSchema).min(1)
}).describe(PARALLEL_DESCRIPTION);
const workflowBuilderForeachEntrySchema = zod.z.strictObject({
	type: zod.z.literal("foreach"),
	step: executableInnerStepSchema,
	opts: zod.z.object({ concurrency: zod.z.number().int().positive() }).optional().describe("Optional concurrency control; defaults to 1 (sequential).")
}).describe(FOREACH_DESCRIPTION);
const workflowBuilderSleepEntrySchema = zod.z.strictObject({
	type: zod.z.literal("sleep"),
	id: zod.z.string().min(1),
	duration: zod.z.number().nonnegative().describe("Milliseconds to wait. Static number only.")
});
const workflowBuilderSleepUntilEntrySchema = zod.z.strictObject({
	type: zod.z.literal("sleepUntil"),
	id: zod.z.string().min(1),
	date: zod.z.string().min(1).describe("ISO 8601 wall-clock date to wait until. Static string only.")
});
const workflowBuilderConditionalEntrySchema = zod.z.strictObject({
	type: zod.z.literal("conditional"),
	steps: zod.z.array(executableInnerStepSchema).min(1),
	predicates: zod.z.array(workflowBuilderPredicateSchema).min(1).describe("One declarative predicate per branch, aligned by array index with steps. No JS closures.")
}).describe(CONDITIONAL_DESCRIPTION);
const workflowBuilderLoopEntrySchema = zod.z.strictObject({
	type: zod.z.literal("loop"),
	step: executableInnerStepSchema,
	loopType: zod.z.enum(["dowhile", "dountil"]),
	predicate: workflowBuilderPredicateSchema.describe("Declarative predicate — no JS closures.")
}).describe(LOOP_DESCRIPTION);
const workflowBuilderParallelEntryInputSchema = zod.z.strictObject({
	type: zod.z.literal("parallel"),
	steps: zod.z.array(executableInnerStepInputSchema).min(1)
}).describe(PARALLEL_DESCRIPTION);
const workflowBuilderForeachEntryInputSchema = zod.z.strictObject({
	type: zod.z.literal("foreach"),
	step: executableInnerStepInputSchema,
	opts: zod.z.object({ concurrency: zod.z.number().int().positive().nullish() }).nullish().describe("Optional concurrency control; defaults to 1 (sequential).")
}).describe(FOREACH_DESCRIPTION);
const workflowBuilderConditionalEntryInputSchema = zod.z.strictObject({
	type: zod.z.literal("conditional"),
	steps: zod.z.array(executableInnerStepInputSchema).min(1),
	predicates: zod.z.array(workflowBuilderPredicateSchema).min(1).describe("One declarative predicate per branch, aligned by array index with steps. No JS closures.")
}).describe(CONDITIONAL_DESCRIPTION);
const workflowBuilderLoopEntryInputSchema = zod.z.strictObject({
	type: zod.z.literal("loop"),
	step: executableInnerStepInputSchema,
	loopType: zod.z.enum(["dowhile", "dountil"]),
	predicate: workflowBuilderPredicateSchema.describe("Declarative predicate — no JS closures.")
}).describe(LOOP_DESCRIPTION);
const workflowBuilderGraphEntrySchema = zod.z.discriminatedUnion("type", [
	workflowBuilderAgentEntrySchema,
	workflowBuilderToolEntrySchema,
...
const workflowBuilderGraphEntryInputSchema = zod.z.discriminatedUnion("type", [
	workflowBuilderAgentEntryInputSchema,
	workflowBuilderToolEntryInputSchema,
...
const GRAPH_DESCRIPTION = "The complete ordered top-level graph covering all ten persisted graph families: agent, tool, mapping, nested workflow, parallel, foreach, sleep, sleepUntil, conditional, and loop. Every adjacent pair must compose: the previous output shape must satisfy the next input schema — insert a mapping step whenever shapes differ. The workflow result is exactly the final top-level entry output, so add an explicit final mapping whenever that output does not match outputSchema.";
const workflowBuilderDefinitionSchema = zod.z.strictObject({
	id: zod.z.string().min(1).describe("Workflow id — kebab-case. Preserve the exact requested workflow ID."),
	description: zod.z.string().optional(),
	metadata: zod.z.record(zod.z.string(), zod.z.unknown()).optional(),
	inputSchema: zod.z.unknown().describe("Complete JSON Schema (Draft 2020-12) for the workflow input."),
	outputSchema: zod.z.unknown().describe("Complete JSON Schema (Draft 2020-12) for the workflow output."),
	stateSchema: zod.z.unknown().optional().describe("Optional JSON Schema for persisted workflow state."),
	requestContextSchema: zod.z.unknown().optional().describe("Optional JSON Schema for request context values."),
	graph: zod.z.array(workflowBuilderGraphEntrySchema).min(1).describe(GRAPH_DESCRIPTION)
});
const workflowBuilderDefinitionInputSchema = zod.z.strictObject({
	id: zod.z.string().min(1).describe("Workflow id — kebab-case. Preserve the exact requested workflow ID unless the user renames it."),
	description: zod.z.string().nullish(),
	metadata: zod.z.record(zod.z.string(), zod.z.unknown()).nullish(),
	inputSchema: zod.z.unknown().describe("Complete JSON Schema (Draft 2020-12) for the workflow input."),
	outputSchema: zod.z.unknown().describe("Complete JSON Schema (Draft 2020-12) for the workflow output."),
	stateSchema: zod.z.unknown().nullish().describe("Optional JSON Schema for persisted workflow state."),
	requestContextSchema: zod.z.unknown().nullish().describe("Optional JSON Schema for request context values."),
	graph: zod.z.array(workflowBuilderGraphEntryInputSchema).min(1).describe(GRAPH_DESCRIPTION)
}).describe("One complete canonical WorkflowDefinition. Submit exactly one complete candidate per attempt — never parallel alternatives. After diagnostics, correct and resubmit the whole definition.");
/** Public HTTP/SDK contract for a persisted workflow definition row. */
const storedWorkflowDefinitionSchema = workflowBuilderDefinitionSchema.extend({
	status: zod.z.enum(["active", "archived"]),
	source: zod.z.literal("storage"),
	authorId: zod.z.string().optional(),
	createdAt: zod.z.union([zod.z.date(), zod.z.string()]),
	updatedAt: zod.z.union([zod.z.date(), zod.z.string()])
});
const listStoredWorkflowsResponseSchema = zod.z.object({
	workflows: zod.z.array(storedWorkflowDefinitionSchema),
	total: zod.z.number()
});
const upsertStoredWorkflowResponseSchema = zod.z.object({
	ok: zod.z.literal(true),
	id: zod.z.string()
});
const deleteStoredWorkflowResponseSchema = zod.z.object({
	success: zod.z.literal(true),
	message: zod.z.string()
});
//#endregion
//#region src/workflows/builder/agent.ts
/**
...
*/
function createWorkflowBuilderAgent({ surfaceInstructions, ...config }) {
...
const OPTIONAL_STEP_OPTION_KEYS = ["retries", "metadata"];
const OPTIONAL_FOREACH_OPT_KEYS = ["concurrency"];
function dropNullKeys(target, keys) {
...
function normalizeEntry(entry) {
	...
	if (normalized.options && typeof normalized.options === "object" && !Array.isArray(normalized.options)) {
		dropNullKeys(normalized.options, OPTIONAL_STEP_OPTION_KEYS);
	...
	if (normalized.opts && typeof normalized.opts === "object" && !Array.isArray(normalized.opts)) {
		dropNullKeys(normalized.opts, OPTIONAL_FOREACH_OPT_KEYS);
		if (Object.keys(normalized.opts).length === 0) delete normalized.opts;
	}
	if (normalized.type === "mapping" && typeof normalized.mapConfig !== "string" && normalized.mapConfig !== null && normalized.mapConfig !== void 0) normalized.mapConfig = JSON.stringify(normalized.mapConfig);
...
//#endregion
Object.defineProperty(exports, "WORKFLOW_BUILDER_AUTHORING_CONSTRAINTS", {
	enumerable: true,
	get: function() {
		return WORKFLOW_BUILDER_AUTHORING_CONSTRAINTS;
	}
});
Object.defineProperty(exports, "WORKFLOW_BUILDER_AUTHORING_PLAYBOOK", {
	enumerable: true,
	get: function() {
		return WORKFLOW_BUILDER_AUTHORING_PLAYBOOK;
	}
});
Object.defineProperty(exports, "WORKFLOW_BUILDER_MAPPING_CONFIG_DESCRIPTION", {
	enumerable: true,
	get: function() {
		return WORKFLOW_BUILDER_MAPPING_CONFIG_DESCRIPTION;
	}
});
Object.defineProperty(exports, "WORKFLOW_BUILDER_SUPPORTED_STEP_TYPES", {
	enumerable: true,
	get: function() {
		return WORKFLOW_BUILDER_SUPPORTED_STEP_TYPES;
	}
});
Object.defineProperty(exports, "compareWorkflowBuilderSchemas", {
	enumerable: true,
	get: function() {
		return compareWorkflowBuilderSchemas;
	}
});
Object.defineProperty(exports, "createWorkflowBuilderAgent", {
	enumerable: true,
	get: function() {
		return createWorkflowBuilderAgent;
	}
});
Object.defineProperty(exports, "deleteStoredWorkflowResponseSchema", {
	enumerable: true,
	get: function() {
		return deleteStoredWorkflowResponseSchema;
	}
});
Object.defineProperty(exports, "inspectWorkflowBuilderSchemas", {
	enumerable: true,
	get: function() {
		return inspectWorkflowBuilderSchemas;
	}
});
Object.defineProperty(exports, "listStoredWorkflowsResponseSchema", {
	enumerable: true,
	get: function() {
		return listStoredWorkflowsResponseSchema;
	}
});
Object.defineProperty(exports, "normalizeWorkflowBuilderDefinition", {
	enumerable: true,
	get: function() {
		return normalizeWorkflowBuilderDefinition;
	}
});
Object.defineProperty(exports, "preflightWorkflowDefinition", {
	enumerable: true,
	get: function() {
		return preflightWorkflowDefinition;
	}
});
Object.defineProperty(exports, "storedWorkflowDefinitionSchema", {
	enumerable: true,
	get: function() {
		return storedWorkflowDefinitionSchema;
	}
});
Object.defineProperty(exports, "upsertStoredWorkflowResponseSchema", {
	enumerable: true,
	get: function() {
		return upsertStoredWorkflowResponseSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderAgentEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderAgentEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderAgentEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderAgentEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderConditionalEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderConditionalEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderConditionalEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderConditionalEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderDefinitionInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderDefinitionInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderDefinitionSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderDefinitionSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderForeachEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderForeachEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderForeachEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderForeachEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderGraphEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderGraphEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderGraphEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderGraphEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderLoopEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderLoopEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderLoopEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderLoopEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderMappingConfigSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderMappingConfigSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderMappingDescriptorSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderMappingDescriptorSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderMappingEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderMappingEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderMappingEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderMappingEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderNestedWorkflowEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderNestedWorkflowEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderNestedWorkflowEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderNestedWorkflowEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderParallelEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderParallelEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderParallelEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderParallelEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderPredicateSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderPredicateSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderSleepEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderSleepEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderSleepUntilEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderSleepUntilEntrySchema;
	}
});
Object.defineProperty(exports, "workflowBuilderToolEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderToolEntryInputSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderToolEntrySchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderToolEntrySchema;
	}
});

//# sourceMappingURL=builder-CbWxFrTo.cjs.map