...
var GatewayRegistry = class GatewayRegistry {
	...
	*/
	async syncGateways(forceRefresh = false, writeToSrc = false) {
		...
		try {
			const { ModelsDevGateway } = await Promise.resolve().then(() => require("./models-dev-BPHvRLCw.cjs")).then((n) => n.models_dev_exports);
			const { NetlifyGateway } = await Promise.resolve().then(() => require("./netlify-Cf0MHcxV.cjs")).then((n) => n.netlify_exports);
			const { MastraGateway } = await Promise.resolve().then(() => mastra_exports);
			const { fetchProvidersFromGateways, writeRegistryFiles } = await Promise.resolve().then(() => require("./registry-generator-DKYqx6j9.cjs"));
...
//# sourceMappingURL=provider-registry-Bpx3y4iE.cjs.map