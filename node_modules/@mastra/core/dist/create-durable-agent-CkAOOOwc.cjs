...
const require_event_emitter = require("./event-emitter-B85_hXV9.cjs");
const require_topics = require("./topics-DZfiEQ5S.cjs");
const require_observability = require("./observability-Ck94E5FA.cjs");
const require_utils = require("./utils-z9W2oXfe.cjs");
require("./tracing-Sjwim3h6.cjs");
const require_request_context = require("./request-context-CY1DtViW.cjs");
const require_llm = require("./llm-ByQthpoT.cjs");
const require_background_tasks = require("./background-tasks-BKQSZmdP.cjs");
const require_toolchecks = require("./toolchecks-g9gKsDDO.cjs");
const require_utils$1 = require("./utils-Ox89OFwL.cjs");
const require_utils_safe_stringify = require("./utils/safe-stringify.cjs");
const require_trip_wire = require("./trip-wire-B-TPWnWV.cjs");
const require_agent = require("./agent-BVtn9FqD.cjs");
const require_payload_transform = require("./payload-transform-DJzDdfpE.cjs");
const require_signals = require("./signals-zuUbXoDr.cjs");
const require_message_list = require("./message-list-pLbR6R6n.cjs");
const require_task_state_processor = require("./task-state-processor-B-3s_Iep.cjs");
const require_logger_index = require("./logger/index.cjs");
const require_workflows_constants = require("./workflows/constants.cjs");
const require_storage = require("./storage-DvYq_0a1.cjs");
...
async function publishAbortRequest(pubsub, runId) {
	await pubsub.publish(require_agent.AGENT_CONTROL_TOPIC(runId), {
		type: require_agent.AgentControlEventTypes.ABORT_REQUEST,
		runId,
		data: {}
	});
}
...
async function subscribeToAbortRequests(pubsub, runId, onAbortRequested) {
	const topic = require_agent.AGENT_CONTROL_TOPIC(runId);
	const handler = async (event, ack) => {
		if (event?.type === require_agent.AgentControlEventTypes.ABORT_REQUEST) onAbortRequested();
		await ack?.();
	};
...
function restoreRequestContext$1(entries, runLevel) {
	if (entries) return new require_request_context.RequestContext(Object.entries(entries));
	return runLevel ?? new require_request_context.RequestContext();
}
/**
...
function restoreRequestContext(entries, fallback) {
	if (!entries) return fallback ?? new require_request_context.RequestContext();
	const restored = new require_request_context.RequestContext(fallback?.entries());
...
function resolveOutputText(messageList) {
	...
	if (responseMessages.some((message) => message.content?.metadata?.completionResult)) {
		...
		const lastConverted = (lastRealMessage ? require_message_list.convertMessages([lastRealMessage]).to("AIV4.Core") : []).at(-1);
		return lastConverted ? require_message_list.coreContentToString(lastConverted.content) : "";
	}
	const lastResponseMessage = messageList.get.response.aiV4.core().at(-1);
	return lastResponseMessage ? require_message_list.coreContentToString(lastResponseMessage.content) : "";
}
/**
...
async function runDurableFinishSideEffects({ runId, initData, messageListState, mastra, requestContext, tracingContext, logger, outputResult }) {
	const effectiveLogger = logger ?? mastra?.getLogger?.() ?? require_logger_index.noopLogger;
	...
	const messageList = (registryEntry?.messageList ?? new require_message_list.MessageList({
	...
	if (registryEntry?.outputProcessors?.length) try {
		...
		await new require_trip_wire.ProcessorRunner({
		...
		}).runOutputProcessors(messageList, require_observability.createObservabilityContext(tracingContext), effectiveRequestContext, 0, void 0, outputResult);
	} catch (error) {
...
async function generateDurableThreadTitle({ agent, memory, threadId, resourceId, memoryConfig, messageListState, requestContext, tracingContext }) {
	...
	const title = await agent.genTitle(userMessage, requestContext ?? new require_request_context.RequestContext(), require_observability.createObservabilityContext(tracingContext), model, instructions, uiMessages);
	if (!title) return;
...
async function prepareForDurableExecution(options) {
	...
	if (memory && threadId && resourceId) {
		...
		if (existingThread) require_agent.assertThreadOwnedByResource({
	...
	if (inputProcessors.length > 0) try {
		const { ProcessorRunner } = await Promise.resolve().then(() => require("./trip-wire-B-TPWnWV.cjs")).then((n) => n.runner_exports);
	...
	if (!tripwireData) await require_agent.fireClientToolOutputHooks({
		messages,
		tools,
...
function resolveDurableToolCallConcurrency({ options, toolsMetadata, toolCalls }) {
	...
	const { limit, strategy } = require_agent.normalizeToolCallConcurrency(options?.toolCallConcurrency);
	let consideredTools;
	if (strategy === "called") {
	...
	return limit > 0 ? limit : require_agent.DurableAgentDefaults.TOOL_CALL_CONCURRENCY;
}
//#endregion
		...
		execute: async (params) => {
			...
			for (let modelIndex = 0; modelIndex < modelList.length; modelIndex++) {
				...
				for (let attempt = 0; attempt <= maxRetries; attempt++) try {
					...
					const responseTraceId = require_utils.getRootExportSpan(modelSpanTracker?.getTracingContext()?.currentSpan ?? tracingContext?.currentSpan)?.externalTraceId;
...
const durableToolCallOutputSchema = durableToolCallInputSchema.extend({
	...
	modelOutputComputed: zod.z.boolean().optional(),
	error: zod.z.object({
		name: zod.z.string(),
		...
		execute: async (params) => {
			...
			if (approvalGated && approvalDecision) {
				...
				if (!approvalDecision.approved) {
					const approval = {
						id: toolCallId,
						approved: false,
						reason: require_agent.resolveDeclineReason(approvalDecision)
					};
					if (pubsub) try {
			...
			try {
				...
				if (toModelOutput) {
					...
					try {
						const modelOutput = require_agent.normalizeModelOutput(await toModelOutput(result));
				...
				if (!toolResult.error && toolResult.result != null && !toolResult.providerExecuted && !toolResult.modelOutputComputed) {
					...
					if (tool?.toModelOutput) {
						...
						try {
							...
							modelOutput = require_agent.normalizeModelOutput(modelOutput);
							mappingSpan?.end({ output: modelOutput });
							if (modelOutput != null) {
...
var DurableAgent = class extends require_agent.Agent {
	...
	*/
	async #acquireRecoveryLease(runId, abortController) {
		...
		const provider = typeof unwrap === "function" ? unwrap.call(pubsub) ?? require_event_emitter.NoopLeaseProvider : require_event_emitter.isLeaseProvider(pubsub) ? pubsub : require_event_emitter.NoopLeaseProvider;
		const key = `mastra:durable-agent-recovery:v1:${JSON.stringify([this.id, runId])}`;
		const owner = crypto.randomUUID();
		if (localRecoveryClaims.get(key)) throw new require_error.MastraError({
		...
		const stopOnLeaseLoss = (cause) => {
			...
			lossError = new require_error.MastraError({
	...
	*/
	async #setupRecoveredStream({ runId, workflowInput, requestContext, threadId, resourceId, messageList, registryEntry, options, scheduleAutoCleanup, recoveryLease }) {
		...
		try {
			...
			recoveryLease.assertOwned();
			threadRegistration = await require_storage.agentThreadStreamRuntime.registerRun(this, stream.output, recoverStreamOptions, this.getPubSub(), {
				strict: true,
	...
	async #reportRecoveryFailure(runId, error) {
		...
		if (normalizedError instanceof require_error.MastraError && normalizedError.id === "DURABLE_AGENT_RECOVER_LEASE_LOST" || normalizedError instanceof require_storage.AgentThreadLeaseConflictError) return false;
		try {
	...
	*/
		...
		if (this.#cacheConfig === false) {
		...
		} else if (this.#innerPubsub instanceof require_topics.CachingPubSub) {
			this.#cachingPubsub = this.#innerPubsub;
			...
			this.#cachingPubsub = new require_topics.CachingPubSub(this.#innerPubsub, resolvedCache, { shouldCache: (topic) => !require_topics.isRunLocalTopic(topic) });
		}
	}
	/**
...
Object.defineProperty(exports, "ensureRemoteAbortListener", {
	enumerable: true,
	get: function() {
		return ensureRemoteAbortListener;
	}
});
Object.defineProperty(exports, "executeDurableAgentScorers", {
	enumerable: true,
	get: function() {
		return executeDurableAgentScorers;
	}
});
Object.defineProperty(exports, "getActiveDurableAgentWorkflowExecutions", {
	enumerable: true,
	get: function() {
		return getActiveDurableAgentWorkflowExecutions;
	}
});
...
Object.defineProperty(exports, "publishAbortRequest", {
	enumerable: true,
	get: function() {
		return publishAbortRequest;
	}
});
...
Object.defineProperty(exports, "runDurableFinishSideEffects", {
	enumerable: true,
	get: function() {
		return runDurableFinishSideEffects;
	}
});
...
Object.defineProperty(exports, "subscribeToAbortRequests", {
	enumerable: true,
	get: function() {
		return subscribeToAbortRequests;
	}
});
...
//# sourceMappingURL=create-durable-agent-CkAOOOwc.cjs.map