...
import { f as embed$2 } from "../message-list-Il1hgrzP.js";
import { r as embed$1 } from "../dist-DIIEuFGB.js";
import { n as embed } from "../dist-_kmO6lmF.js";
import { yt as createVectorErrorId } from "../storage-0ZpJ1SnM.js";
import { BaseFilterTranslator } from "./filter/index.js";
...
var MastraVector = class extends MastraBase {
	...
	constructor({ id, disableInit }) {
		if (!id || typeof id !== "string" || id.trim() === "") throw new MastraError({
...
function validateUpsertInput(storeName, vectors, metadata, ids) {
	if (!vectors || vectors.length === 0) throw new MastraError({
		id: createVectorErrorId(storeName, "UPSERT", "EMPTY_VECTORS"),
		domain: ErrorDomain.MASTRA_VECTOR,
		category: ErrorCategory.USER,
		details: { message: "Vectors array cannot be empty" }
	});
	if (metadata && metadata.length > 0 && metadata.length !== vectors.length) throw new MastraError({
	...
	if (ids && ids.length !== vectors.length) throw new MastraError({
...
function validateTopK(storeName, topK) {
	if (!Number.isInteger(topK) || topK <= 0) throw new MastraError({
...
function validateVectorValues(storeName, vectors) {
	for (let i = 0; i < vectors.length; i++) {
		...
		if (!vector) throw new MastraError({
		...
		for (let j = 0; j < vector.length; j++) {
			...
			if (value === null || value === void 0 || !Number.isFinite(value)) throw new MastraError({
...
//#endregion
export { BaseFilterTranslator, MastraVector, embed as embedV1, embed$1 as embedV2, embed$2 as embedV3, isSupportedEmbeddingModel, supportedEmbeddingModelSpecifications, validateTopK, validateUpsert, validateUpsertInput, validateVectorValues };
