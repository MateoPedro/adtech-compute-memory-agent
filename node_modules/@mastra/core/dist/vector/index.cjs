...
const require_vector_filter_index = require("./filter/index.cjs");
//#region src/vector/vector.ts
/** Specification versions for supported (modern) embedding models */
const supportedEmbeddingModelSpecifications = ["v2", "v3"];
/**
...
*/
const isSupportedEmbeddingModel = (model) => {
	return supportedEmbeddingModelSpecifications.includes(model.specificationVersion);
};
var MastraVector = class extends require_base.MastraBase {
	id;
	disableInit = false;
	constructor({ id, disableInit }) {
		if (!id || typeof id !== "string" || id.trim() === "") throw new require_error.MastraError({
			id: "VECTOR_INVALID_ID",
			text: "Vector id must be provided and cannot be empty",
			domain: require_error.ErrorDomain.MASTRA_VECTOR,
...
//#endregion
//#region src/vector/validation.ts
/**
...
function validateUpsertInput(storeName, vectors, metadata, ids) {
	if (!vectors || vectors.length === 0) throw new require_error.MastraError({
		id: require_storage.createVectorErrorId(storeName, "UPSERT", "EMPTY_VECTORS"),
		domain: require_error.ErrorDomain.MASTRA_VECTOR,
		category: require_error.ErrorCategory.USER,
		details: { message: "Vectors array cannot be empty" }
	});
	if (metadata && metadata.length > 0 && metadata.length !== vectors.length) throw new require_error.MastraError({
		id: require_storage.createVectorErrorId(storeName, "UPSERT", "METADATA_LENGTH_MISMATCH"),
		domain: require_error.ErrorDomain.MASTRA_VECTOR,
		category: require_error.ErrorCategory.USER,
		details: {
	...
	if (ids && ids.length !== vectors.length) throw new require_error.MastraError({
		id: require_storage.createVectorErrorId(storeName, "UPSERT", "IDS_LENGTH_MISMATCH"),
		domain: require_error.ErrorDomain.MASTRA_VECTOR,
		category: require_error.ErrorCategory.USER,
		details: {
...
function validateTopK(storeName, topK) {
	if (!Number.isInteger(topK) || topK <= 0) throw new require_error.MastraError({
		id: require_storage.createVectorErrorId(storeName, "QUERY", "INVALID_TOP_K"),
		domain: require_error.ErrorDomain.MASTRA_VECTOR,
		category: require_error.ErrorCategory.USER,
		details: {
...
function validateVectorValues(storeName, vectors) {
	for (let i = 0; i < vectors.length; i++) {
		...
		if (!vector) throw new require_error.MastraError({
			id: require_storage.createVectorErrorId(storeName, "UPSERT", "INVALID_VECTOR"),
			domain: require_error.ErrorDomain.MASTRA_VECTOR,
			category: require_error.ErrorCategory.USER,
			details: {
		...
		for (let j = 0; j < vector.length; j++) {
			...
			if (value === null || value === void 0 || !Number.isFinite(value)) throw new require_error.MastraError({
				id: require_storage.createVectorErrorId(storeName, "UPSERT", "INVALID_VECTOR_VALUE"),
				domain: require_error.ErrorDomain.MASTRA_VECTOR,
				category: require_error.ErrorCategory.USER,
				details: {
...
function validateUpsert(storeName, vectors, metadata, ids, validateValues = false) {
	...
	if (validateValues && vectors) validateVectorValues(storeName, vectors);
}
//#endregion
exports.BaseFilterTranslator = require_vector_filter_index.BaseFilterTranslator;
exports.MastraVector = MastraVector;
exports.embedV1 = require_dist$1.embed;
exports.embedV2 = require_dist.embed;
exports.embedV3 = require_message_list.embed;
