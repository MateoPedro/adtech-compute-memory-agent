...
import { i as createObservabilityContext, r as wrapMastra } from "./observability-DaY8HN4h.js";
import { a as getOrCreateSpan, f as EntityType, t as executeWithContext } from "./utils-CoqfpE2k.js";
import "./tracing-DYRZCMQm.js";
import { o as RequestContext } from "./request-context-bTdjxlgA.js";
import { a as validateToolInput, c as markBuilderValidatedInput, n as Tool, o as validateToolOutput, s as validateToolSuspendData } from "./tool-BwroCWv4.js";
import { isStandardSchemaWithJSON, standardSchemaToJSONSchema, toStandardSchema } from "./schema/index.js";
import { a as isZodObject, o as safeExtendZodObject } from "./zod-utils-DTkc-hhd.js";
import { i as MastraFGAPermissions } from "./ee-DXvSoTl7-BqTsKmQd.js";
import { n as backgroundOverrideJsonSchema, r as backgroundOverrideZodSchema } from "./background-tasks-OPQtn1YI.js";
import { i as isProviderDefinedTool, o as isVercelTool, t as getNeedsApprovalFn } from "./toolchecks-Buafszej.js";
import { n as ToolStream, t as noopObserve } from "./types-C59tsW89.js";
import { safeStringify } from "./utils/safe-stringify.js";
import { createHash } from "crypto";
import { z } from "zod/v4";
import { jsonSchemaToZod } from "@mastra/schema-compat/json-to-zod";
import { AnthropicSchemaCompatLayer, DeepSeekSchemaCompatLayer, GoogleSchemaCompatLayer, MetaSchemaCompatLayer, OpenAIReasoningSchemaCompatLayer, OpenAISchemaCompatLayer, applyCompatLayer, convertZodSchemaToAISDKSchema, jsonSchema } from "@mastra/schema-compat";
...
var CoreToolBuilder = class extends MastraBase {
		...
		if (!isVercelTool(this.originalTool) && !isProviderDefinedTool(this.originalTool)) {
			if (isBackgroundEligible || isResumableTool) {
				...
				if (isZodObject(schema) && isZodV4Schema(schema)) {
					...
					if (isBackgroundEligible) nextSchema = safeExtendZodObject(nextSchema, { _background: backgroundOverrideZodSchema });
					if (isResumableTool) nextSchema = safeExtendZodObject(nextSchema, {
					...
					const jsonSchema = standardSchemaToJSONSchema(isStandardSchemaWithJSON(schema) ? schema : toStandardSchema(schema), { io: "input" });
	...
	getParameters = () => {
		if (isVercelTool(this.originalTool)) {
			let schema = this.originalTool.parameters ?? ("inputSchema" in this.originalTool ? this.originalTool.inputSchema : void 0) ?? z.object({});
			if (typeof schema === "function") schema = schema();
			return schema;
		}
		let schema = this.originalTool.inputSchema;
	...
	buildCompatValidationSchema(originalSchema, schemaCompatLayers) {
	...
	buildProviderTool(tool) {
		if ("type" in tool && (tool.type === "provider-defined" || tool.type === "provider") && "id" in tool && typeof tool.id === "string" && tool.id.includes(".")) {
			...
			else if (isStandardSchemaWithJSON(parameters)) processedParameters = { jsonSchema: standardSchemaToJSONSchema(parameters, { io: "input" }) };
			else processedParameters = convertZodSchemaToAISDKSchema(parameters);
			...
			else if (isStandardSchemaWithJSON(outputSchema)) processedOutputSchema = { jsonSchema: standardSchemaToJSONSchema(outputSchema) };
			else processedOutputSchema = convertZodSchemaToAISDKSchema(outputSchema);
			return {
	...
	createExecute(tool, options, logType, inputValidationSchema) {
		...
		const execFunction = async (args, execOptions, toolSpan) => {
			try {
				...
				if (isVercelTool(tool)) result = await executeWithContext({
					span: toolSpan,
					...
					*/
					...
					result = await executeWithContext({
						span: toolSpan,
						fn: async () => {
							if (inputValidationSchema) markBuilderValidatedInput(toolContext);
							return tool?.execute?.(args, toolContext);
						}
					});
				}
				if (suspendData) {
					const suspendValidation = validateToolSuspendData(this.getSuspendSchema(), suspendData, options.name);
				...
				if (isVercelTool(tool)) {
					const outputValidation = validateToolOutput(this.getOutputSchema(), result, options.name, false);
		...
		return async (args, execOptions) => {
			...
			if (fgaProvider) {
				const { getAgentToolFGAResourceId, getMCPToolFGAResourceId, getStandaloneToolFGAResourceId, requireFGA } = await import("./auth/ee/fga-check.js");
				await requireFGA({
					fgaProvider,
					user,
					resource: {
	...
	build() {
		...
		if (originalSchema) if (isStandardSchemaWithJSON(originalSchema)) {
			const applicableLayer = schemaCompatLayers.find((layer) => layer.shouldApply());
			let schemaToUse;
			if (applicableLayer) schemaToUse = applicableLayer.processToCompatSchema(originalSchema);
			else schemaToUse = toStandardSchema(originalSchema);
			processedInputSchema = jsonSchema(standardSchemaToJSONSchema(schemaToUse, { io: "input" }), { validate: (value) => {
		...
		} else processedInputSchema = applyCompatLayer({
		...
		let processedOutputSchema;
		if (outputSchema) if (isStandardSchemaWithJSON(outputSchema)) processedOutputSchema = standardSchemaToJSONSchema(outputSchema, { io: "output" });
		else processedOutputSchema = applyCompatLayer({
			schema: outputSchema,
			compatLayers: [],
			mode: "aiSdkSchema"
		});
		...
		return {
			...
			execute: this.originalTool.execute ? this.createExecute(this.originalTool, {
				...this.options,
				description: this.originalTool.description
			}, this.logType, inputValidationSchema) : void 0,
...
function ensureToolProperties(tools) {
	return Object.keys(tools).reduce((acc, key) => {
		...
		if (tool) {
			if (typeof tool === "function" && !(tool instanceof Tool) && !isVercelTool(tool)) throw new MastraError({
			...
			if (isVercelTool(tool)) acc[key] = setVercelToolProperties(tool);
			else acc[key] = tool;
		}
		return acc;
	}, {});
...
function convertVercelToolParameters(tool) {
	...
	if (typeof schema === "function") schema = schema();
	return isZodType$1(schema) ? schema : resolveSerializedZodOutput(jsonSchemaToZod(schema));
}
/**
...
//#endregion
export { readPositiveIntEnv as _, ensureToolProperties as a, selectFields as b, isCoreMessage as c, makeCoreTool as d, makeCoreToolV5 as f, parseSqlIdentifier as g, parseFieldKey as h, delay as i, isUiMessage as l, omitKeys as m, createMastraProxy as n, generateEmptyFromSchema as o, maskStreamTags as p, deepMerge as r, getNestedValue as s, checkEvalStorageFields as t, isZodType$1 as u, removeUndefinedValues as v, setNestedValue as x, resolveSerializedZodOutput as y };

//# sourceMappingURL=utils-CfkDo9Tr.js.map