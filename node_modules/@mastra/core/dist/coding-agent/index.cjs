...
const require_signals = require("../signals-YYYViyb4.cjs");
...
function defaultErrorProcessors() {
	return [
		new require_agent.StreamErrorRetryProcessor({
			retryUnknownErrors: true,
			maxRetries: 2,
			delayMs: 3e3,
			matchers: [{
				match: require_agent.isBadRequestError,
				maxRetries: 1,
				delayMs: 2e3
			}, {
		...
		new require_agent.PrefillErrorHandler(),
		new require_agent.ProviderHistoryCompat()
	];
}
/**
...
function defaultWorkspace(basePath) {
	return new require_workspace.Workspace({
		filesystem: new require_workspace.LocalFilesystem({ basePath }),
		sandbox: new require_workspace.LocalSandbox({ workingDirectory: basePath })
	});
}
/**
...
function createCodingAgent(config) {
	...
	const taskSignals = memory ? [new require_signals.TaskSignalProvider()] : [];
	const resolvedSignals = signals ? [...signals, ...taskSignals] : taskSignals;
	const resolvedGoal = goal ? {
		...goal,
		prompt: goal.prompt ?? require_task_state_processor.DEFAULT_GOAL_JUDGE_PROMPT
	} : void 0;
