...
const require_resolve_agent = require("./resolve-agent-DeG6HOJ-.cjs");
...
var SchedulerWorker = class extends require_pull_transport.MastraWorker {
	...
	async init(deps) {
		...
		const isTargetReady = mastra ? async (target) => {
			if (target.type === "workflow") try {
				mastra.getWorkflowById(target.workflowId);
				return true;
			} catch {
				return false;
			}
			if (target.type === "agent") return (await require_resolve_agent.resolveAgentById(mastra, target.agentId)).status !== "missing";
			return false;
		} : void 0;
...
var BackgroundTaskWorker = class extends require_pull_transport.MastraWorker {
	...
	*/
	...
	async start() {
		if (this.#running) return;
		if (!this.deps) throw new Error("BackgroundTaskWorker: call init() before start()");
		if (!this.#manager) this.#createOwnedManager(this.deps);
		const manager = this.#manager;
		if (!manager) throw new Error("BackgroundTaskWorker: failed to initialize background task manager");
		if (this.#ownsManager) await manager.init(this.deps.pubsub);
		this.#running = true;
	}
	async stop() {
		if (!this.#running) return;
		if (this.#manager && this.#ownsManager) try {
...
//# sourceMappingURL=worker-CyD51SCP.cjs.map