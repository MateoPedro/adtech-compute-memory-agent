...
const require_deep_equal = require("./deep-equal-0b1rtyz2.cjs");
...
let _mastra_schema_compat_zod_to_json = require("@mastra/schema-compat/zod-to-json");
	...
	emit(input) {
		...
		const delivery = this.#tail.then(async () => {
			...
			try {
				...
				this.#failure = new require_error.MastraError({
...
var ToolMockMatcher = class {
	...
	*/
	resolve(toolName, args) {
		...
		const argsMatch = (entry) => entry.matchArgs === "ignore" || require_deep_equal.deepEqual(entry.args, args);
...
async function executeAgent(agent, item, signal, requestContext, experimentId, versions, toolMocks, unmockedToolPolicy) {
	...
	const reqCtx = requestContext ? new require_request_context.RequestContext(Object.entries(requestContext)) : void 0;
	const tracingOptions = experimentId ? { metadata: { experimentId } } : void 0;
	const contextResourceId = requestContext?.[require_request_context.MASTRA_RESOURCE_ID_KEY];
	const injectedThreadId = (Boolean(contextResourceId) || contextResourceId === void 0) && !requestContext?.["mastra__threadId"] && typeof agent.hasOwnMemory === "function" && agent.hasOwnMemory() ? (0, crypto$1.randomUUID)() : void 0;
	const memoryOption = injectedThreadId ? { memory: {
		thread: {
			id: injectedThreadId,
			...experimentId || item.id ? { metadata: {
				...experimentId ? { experimentId } : {},
				...item.id ? { experimentItemId: item.id } : {}
			} } : {}
		},
		resource: contextResourceId ? String(contextResourceId) : `dataset-experiment:${experimentId ?? (0, crypto$1.randomUUID)()}:thread:${injectedThreadId}`,
	...
	let rawResult;
	try {
		rawResult = require_trip_wire.isSupportedLanguageModel(model) ? await agent.generate(input, {
			scorers: {},
...
async function executeWorkflow(workflow, item, requestContext) {
	const reqCtx = requestContext ? new require_request_context.RequestContext(Object.entries(requestContext)) : void 0;
	const observabilityContext = require_observability.resolveObservabilityContext({});
...
function toScorerTargetEntityType(targetType) {
	switch (targetType) {
		case "agent": return require_utils.EntityType.AGENT;
		case "workflow": return require_utils.EntityType.WORKFLOW_RUN;
		case "scorer": return require_utils.EntityType.SCORER;
		default: return;
	}
}
...
async function extractTrajectoryFromStorage(storage, traceId) {
	...
	try {
		...
		return require_constants.extractTrajectoryFromTrace(trace.spans);
	} catch {
		return;
	}
}
...
async function runScorersForItem(scorers, item, output, storage, runId, targetType, targetId, itemId, scorerInput, scorerOutput, traceId, workflowData, persistScores = true) {
	...
	if (hasTrajectoryScorer) trajectoryOutput = await extractTrajectoryFromStorage(storage, traceId) ?? (scorerOutput ? require_constants.extractTrajectory(scorerOutput) : { steps: [] });
	...
	return (await Promise.allSettled(scorers.map(async (scorer) => {
		...
		if (persistScores && storage && result.error === null && result.score !== null) try {
			await require_agent.validateAndSaveScore(storage, {
				scorerId: scorer.id,
				score: result.score,
...
async function runScorerSafe(scorer, item, output, scorerInput, scorerOutput, targetType, targetTraceId, targetCorrelationContext, trajectoryOutput, workflowData, persistScores = true) {
	try {
		...
		if (error instanceof require_agent.ScorerRunError) {
		...
		const settled = await Promise.allSettled(scorers.map(async (scorer) => {
			try {
				const scoreResult = await scorer.run({
					...
					scoreSource: "experiment",
					targetScope: "span",
					targetEntityType: require_utils.EntityType.WORKFLOW_STEP,
...
const EXPERIMENT_ITEM_BEFORE_EACH_FAILED = "EXPERIMENT_ITEM_BEFORE_EACH_FAILED";
...
async function runExperiment(mastra, config) {
	const { datasetId, targetType, targetId, scorers: scorerInput, version, maxConcurrency = 5, signal, onEvent, itemTimeout, maxRetries = 0, experimentId: providedExperimentId, name, description, metadata, provenance, grouping, requestContext: globalRequestContext, agentVersion, versions, persistence, beforeAll, afterAll, beforeEach, afterEach } = config;
	...
	*/
	const runAfterAll = async (summary) => {
		...
		try {
			...
			mastra.getLogger()?.error(`Experiment ${experimentId} afterAll hook failed: ${errorMessage(hookError)}`, { error: hookError });
		}
	};
	const storage = mastra.getStorage();
	...
	try {
		if (config.data) {
			...
			if (!datasetRecord) throw new require_error.MastraError({
			...
			if (versionItems.length === 0) throw new require_error.MastraError({
	...
	try {
		if (beforeAll) await beforeAll(hookArgs);
		...
		await pMap(items.map((item, idx) => ({
			...
			try {
				...
				if (beforeEach && !scorerConfigError) try {
					await beforeEach({
						...hookArgs,
						item: {
							id: item.id,
					...
					beforeEachError = {
						...
						stack: hookError instanceof Error ? hookError.stack : void 0
					};
				}
				let retryCount = 0;
				const preflightError = scorerConfigError ?? beforeEachError;
				let execResult = preflightError ? {
					output: null,
				...
				if (afterEach && !beforeEachError) try {
					await afterEach({
						...hookArgs,
						item: {
							id: item.id,
					...
					mastra.getLogger()?.error(`Experiment ${experimentId} afterEach hook failed for item ${item.id}: ${errorMessage(hookError)}`, { error: hookError });
				}
				if (experimentsStore) {
					const now = Date.now();
...
var Dataset = class {
	...
	*/
	...
	async #getDatasetsStore() {
		...
		if (!storage) throw new require_error.MastraError({
		...
		if (!store) throw new require_error.MastraError({
	...
	*/
	async getDetails() {
		...
		if (!record) throw new require_error.MastraError({
			id: "DATASET_NOT_FOUND",
			text: `Dataset not found: ${this.id}`,
			domain: "STORAGE",
			category: "USER"
	...
	*/
	async update(input) {
		...
		if (inputSchema !== void 0 && inputSchema !== null && (0, _mastra_schema_compat.isZodType)(inputSchema)) inputSchema = (0, _mastra_schema_compat_zod_to_json.zodToJsonSchema)(inputSchema);
		if (groundTruthSchema !== void 0 && groundTruthSchema !== null && (0, _mastra_schema_compat.isZodType)(groundTruthSchema)) groundTruthSchema = (0, _mastra_schema_compat_zod_to_json.zodToJsonSchema)(groundTruthSchema);
	...
	*/
	async startExperimentAsync(config) {
		...
		if (!dataset) throw new require_error.MastraError({
			id: "DATASET_NOT_FOUND",
			text: `Dataset not found: ${this.id}`,
			domain: "STORAGE",
			category: "USER"
		...
		if (items.length === 0) throw new require_error.MastraError({
			id: "EXPERIMENT_NO_ITEMS",
	...
	*/
	async listExperiments(args) {
		...
		return (await this.#getExperimentsStore()).listExperiments({
			...
			...args?.experimentSetId !== void 0 ? { experimentSetId: args.experimentSetId } : {},
			...args?.comparisonId !== void 0 ? { comparisonId: args.comparisonId } : {},
			...args?.variantId !== void 0 ? { variantId: args.variantId } : {},
			...args?.trialIndex !== void 0 ? { trialIndex: args.trialIndex } : {},
	...
	*/
	async #assertExperimentOwnership(experimentId) {
		...
		if (!experiment || experiment.datasetId !== this.id) throw new require_error.MastraError({
	...
	*/
	async updateExperimentResult(input) {
		if (!input.experimentId) throw new require_error.MastraError({
...
var DatasetsManager = class {
	...
	*/
	async create(input) {
		const store = await this.#getDatasetsStore();
		let { inputSchema, groundTruthSchema, ...rest } = input;
		if (inputSchema !== void 0 && (0, _mastra_schema_compat.isZodType)(inputSchema)) inputSchema = (0, _mastra_schema_compat_zod_to_json.zodToJsonSchema)(inputSchema);
		if (groundTruthSchema !== void 0 && (0, _mastra_schema_compat.isZodType)(groundTruthSchema)) groundTruthSchema = (0, _mastra_schema_compat_zod_to_json.zodToJsonSchema)(groundTruthSchema);
	...
	*/
	async compareExperiments(args) {
		...
		if (experimentIds.length < 2) throw new require_error.MastraError({
...
Object.defineProperty(exports, "EXPERIMENT_ITEM_BEFORE_EACH_FAILED", {
	enumerable: true,
	get: function() {
		return EXPERIMENT_ITEM_BEFORE_EACH_FAILED;
	}
});
Object.defineProperty(exports, "EXPERIMENT_ITEM_SCORER_NOT_FOUND", {
	enumerable: true,
	get: function() {
		return EXPERIMENT_ITEM_SCORER_NOT_FOUND;
	}
});
Object.defineProperty(exports, "TOOL_MOCK_EXHAUSTED", {
	enumerable: true,
	get: function() {
		return TOOL_MOCK_EXHAUSTED;
	}
});
Object.defineProperty(exports, "TOOL_MOCK_MISMATCH", {
	enumerable: true,
	get: function() {
		return TOOL_MOCK_MISMATCH;
	}
});
Object.defineProperty(exports, "TOOL_MOCK_NOT_DECLARED", {
	enumerable: true,
	get: function() {
		return TOOL_MOCK_NOT_DECLARED;
	}
});
Object.defineProperty(exports, "ToolMockMatcher", {
	enumerable: true,
	get: function() {
		return ToolMockMatcher;
	}
});
Object.defineProperty(exports, "compareExperiments", {
	enumerable: true,
	get: function() {
		return compareExperiments;
	}
});
...
Object.defineProperty(exports, "runScorersForItem", {
	enumerable: true,
	get: function() {
		return runScorersForItem;
	}
});

//# sourceMappingURL=manager-D99YS21Q.cjs.map