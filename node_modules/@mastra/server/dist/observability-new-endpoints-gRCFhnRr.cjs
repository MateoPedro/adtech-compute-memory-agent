...
zod_v4.z.object({ feedback: zod_v4.z.preprocess(normalizeLegacyFeedbackActor, feedbackRecordObjectSchema) }).describe("Arguments for creating feedback");
...
zod_v4.z.object({ feedbacks: zod_v4.z.array(zod_v4.z.preprocess(normalizeLegacyFeedbackActor, feedbackRecordObjectSchema)) }).describe("Arguments for batch recording feedback");
...
const LIST_FEEDBACK = createNewRoute(require_server_handlers_observability_shared.NEW_ROUTE_DEFS.LIST_FEEDBACK, {
	...
	handler: async ({ mastra, mode, after, limit, ...params }) => {
		...
		const observabilityStore = await require_server_handlers_observability_shared.getObservabilityStore(mastra);
		if (mode === "delta") {
			require_server_handlers_observability_shared.assertObservabilityDeltaSupported(observabilityStore, require_server_handlers_observability_shared.OBSERVABILITY_LIST_ENDPOINTS.feedback);
		...
		return await observabilityStore.listFeedback(mode === "page" ? {
			mode,
			filters,
			pagination,
			orderBy
...
//# sourceMappingURL=observability-new-endpoints-gRCFhnRr.cjs.map