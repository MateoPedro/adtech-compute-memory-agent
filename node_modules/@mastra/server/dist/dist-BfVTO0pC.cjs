...
function convertToAnthropicMessagesPrompt({ prompt, sendReasoning, warnings }) {
	...
	for (let i = 0; i < blocks.length; i++) {
		...
		switch (type) {
			case "system":
				if (system != null) throw new require_dist.UnsupportedFunctionalityError({ functionality: "Multiple system messages that are separated by user/assistant messages" });
							...
							for (let j = 0; j < content.length; j++) {
								...
								switch (part.type) {
									...
									case "image":
										...
										break;
									case "file":
										if (part.mimeType !== "application/pdf") throw new require_dist.UnsupportedFunctionalityError({ functionality: "Non-PDF files in user messages" });
...
var AnthropicMessagesLanguageModel = class {
	...
	async getArgs({ mode, prompt, maxTokens = 4096, temperature, topP, topK, frequencyPenalty, presencePenalty, stopSequences, responseFormat, seed, providerMetadata: providerOptions }) {
		...
		const anthropicOptions = require_dist.parseProviderOptions({
			provider: "anthropic",
			providerOptions,
		...
		if (isThinking) {
			if (thinkingBudget == null) throw new require_dist.UnsupportedFunctionalityError({ functionality: "thinking requires a budget" });
		...
		switch (type) {
			...
			case "object-json": throw new require_dist.UnsupportedFunctionalityError({ functionality: "json-mode object generation" });
			case "object-tool": {
	...
	async getHeaders({ betas, headers }) {
		return require_dist.combineHeaders(await require_dist.resolve(this.config.headers), betas.size > 0 ? { "anthropic-beta": Array.from(betas).join(",") } : {}, headers);
	}
...
var anthropicMessagesChunkSchema = zod.z.discriminatedUnion("type", [
	...
	zod.z.object({ type: zod.z.literal("message_stop") }),
	zod.z.object({ type: zod.z.literal("ping") })
]);
var anthropicProviderOptionsSchema = zod.z.object({ thinking: zod.z.object({
...
function createAnthropic(options = {}) {
	var _a;
	const baseURL = (_a = require_dist.withoutTrailingSlash(options.baseURL)) != null ? _a : "https://api.anthropic.com/v1";
	const getHeaders = () => ({
		"anthropic-version": "2023-06-01",
		"x-api-key": require_dist.loadApiKey({
...
//# sourceMappingURL=dist-BfVTO0pC.cjs.map