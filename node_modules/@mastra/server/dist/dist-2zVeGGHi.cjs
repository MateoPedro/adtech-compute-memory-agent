...
const require_dist = require("./dist-CJKQBH69.cjs");
let zod = require("zod");
//#region ../../../../../setup-pnpm/node_modules/.bin/store/v11/links/@ai-sdk/openai/1.3.24/cee8396ffdebf5a37340749e684d586cdd2d3051871fb36290f1e42c93f1d246/node_modules/@ai-sdk/openai/dist/index.mjs
	...
	for (const { role, content } of prompt) switch (role) {
				...
				content: content.map((part, index) => {
					...
					switch (part.type) {
						...
						case "image": return {
							type: "image_url",
							image_url: {
								url: part.image instanceof URL ? part.image.toString() : `data:${(_a = part.mimeType) != null ? _a : "image/jpeg"};base64,${require_dist.convertUint8ArrayToBase64(part.image)}`,
								detail: (_c = (_b = part.providerMetadata) == null ? void 0 : _b.openai) == null ? void 0 : _c.imageDetail
							}
						};
						case "file":
							if (part.data instanceof URL) throw new require_dist.UnsupportedFunctionalityError({ functionality: "'File content parts with URL data' functionality not supported." });
							switch (part.mimeType) {
								...
								default: throw new require_dist.UnsupportedFunctionalityError({ functionality: `File content part type ${part.mimeType} in user messages` });
							}
					}
				})
			});
		...
		case "assistant": {
			...
			if (useLegacyFunctionCalling) {
				if (toolCalls.length > 1) throw new require_dist.UnsupportedFunctionalityError({ functionality: "useLegacyFunctionCalling with multiple tool calls in one message" });
				messages.push({
					role: "assistant",
					content: text,
...
var openaiErrorDataSchema = zod.z.object({ error: zod.z.object({
	message: zod.z.string(),
	type: zod.z.string().nullish(),
	param: zod.z.any().nullish(),
	code: zod.z.union([zod.z.string(), zod.z.number()]).nullish()
}) });
var openaiFailedResponseHandler = require_dist.createJsonErrorResponseHandler({
...
function prepareTools({ mode, useLegacyFunctionCalling = false, structuredOutputs }) {
	...
	if (useLegacyFunctionCalling) {
		...
		switch (toolChoice.type) {
			...
			case "required": throw new require_dist.UnsupportedFunctionalityError({ functionality: "useLegacyFunctionCalling and toolChoice: required" });
			default: return {
				functions: openaiFunctions,
	...
	switch (type) {
		...
		default: throw new require_dist.UnsupportedFunctionalityError({ functionality: `Unsupported tool choice type: ${type}` });
	}
}
var OpenAIChatLanguageModel = class {
	...
	getArgs({ mode, prompt, maxTokens, temperature, topP, topK, frequencyPenalty, presencePenalty, stopSequences, responseFormat, seed, providerMetadata }) {
		...
		if (useLegacyFunctionCalling && this.settings.parallelToolCalls === true) throw new require_dist.UnsupportedFunctionalityError({ functionality: "useLegacyFunctionCalling with parallelToolCalls" });
		if (useLegacyFunctionCalling && this.supportsStructuredOutputs) throw new require_dist.UnsupportedFunctionalityError({ functionality: "structuredOutputs with useLegacyFunctionCalling" });
	...
	async doGenerate(options) {
		...
		const { responseHeaders, value: response, rawValue: rawResponse } = await require_dist.postJsonToApi({
			...
			successfulResponseHandler: require_dist.createJsonResponseHandler(openaiChatResponseSchema),
		...
		return {
			...
			toolCalls: this.settings.useLegacyFunctionCalling && choice.message.function_call ? [{
				...
				toolCallId: require_dist.generateId(),
				...
				return {
					toolCallType: "function",
					toolCallId: (_a2 = toolCall.id) != null ? _a2 : require_dist.generateId(),
	...
	async doStream(options) {
		...
		const { responseHeaders, value: response } = await require_dist.postJsonToApi({
			...
			body,
			failedResponseHandler: openaiFailedResponseHandler,
			successfulResponseHandler: require_dist.createEventSourceResponseHandler(openaiChatChunkSchema),
			abortSignal: options.abortSignal,
			fetch: this.config.fetch
		});
					...
					if (mappedToolCalls != null) for (const toolCallDelta of mappedToolCalls) {
						...
						if (toolCalls[index] == null) {
							...
							if (((_c = toolCall2.function) == null ? void 0 : _c.name) != null && ((_d = toolCall2.function) == null ? void 0 : _d.arguments) != null) {
								...
								if (require_dist.isParsableJson(toolCall2.function.arguments)) {
									controller.enqueue({
										type: "tool-call",
										toolCallType: "function",
										toolCallId: (_e = toolCall2.id) != null ? _e : require_dist.generateId(),
										toolName: toolCall2.function.name,
						...
						if (((_j = toolCall.function) == null ? void 0 : _j.name) != null && ((_k = toolCall.function) == null ? void 0 : _k.arguments) != null && require_dist.isParsableJson(toolCall.function.arguments)) {
							controller.enqueue({
								type: "tool-call",
								toolCallType: "function",
								toolCallId: (_l = toolCall.id) != null ? _l : require_dist.generateId(),
								toolName: toolCall.function.name,
								args: toolCall.function.arguments
							});
...
var openaiTokenUsageSchema = zod.z.object({
	...
	prompt_tokens_details: zod.z.object({ cached_tokens: zod.z.number().nullish() }).nullish(),
...
var openaiChatResponseSchema = zod.z.object({
	...
	choices: zod.z.array(zod.z.object({
		message: zod.z.object({
			...
			tool_calls: zod.z.array(zod.z.object({
				id: zod.z.string().nullish(),
		...
		index: zod.z.number(),
		logprobs: zod.z.object({ content: zod.z.array(zod.z.object({
			token: zod.z.string(),
			logprob: zod.z.number(),
			top_logprobs: zod.z.array(zod.z.object({
				token: zod.z.string(),
				logprob: zod.z.number()
			}))
		})).nullable() }).nullish(),
...
var openaiChatChunkSchema = zod.z.union([zod.z.object({
	id: zod.z.string().nullish(),
	created: zod.z.number().nullish(),
	model: zod.z.string().nullish(),
	choices: zod.z.array(zod.z.object({
...
`;
	...
	for (const { role, content } of prompt) switch (role) {
		case "system": throw new require_dist.InvalidPromptError({
		...
		case "user": {
			const userMessage = content.map((part) => {
				switch (part.type) {
					...
					case "image": throw new require_dist.UnsupportedFunctionalityError({ functionality: "images" });
				}
			}).join("");
			text += `${user}:
${userMessage}
...
`;
		...
		case "assistant": {
			const assistantMessage = content.map((part) => {
				switch (part.type) {
					case "text": return part.text;
					case "tool-call": throw new require_dist.UnsupportedFunctionalityError({ functionality: "tool-call messages" });
				}
			}).join("");
			text += `${assistant}:
${assistantMessage}
...
var OpenAICompletionLanguageModel = class {
	...
	getArgs({ mode, inputFormat, prompt, maxTokens, temperature, topP, topK, frequencyPenalty, presencePenalty, stopSequences: userStopSequences, responseFormat, seed }) {
		...
		switch (type) {
			case "regular":
				if ((_a = mode.tools) == null ? void 0 : _a.length) throw new require_dist.UnsupportedFunctionalityError({ functionality: "tools" });
				if (mode.toolChoice) throw new require_dist.UnsupportedFunctionalityError({ functionality: "toolChoice" });
				return {
					args: baseArgs,
					warnings
				};
			case "object-json": throw new require_dist.UnsupportedFunctionalityError({ functionality: "object-json mode" });
			case "object-tool": throw new require_dist.UnsupportedFunctionalityError({ functionality: "object-tool mode" });
			default: throw new Error(`Unsupported type: ${type}`);
		}
	}
	async doGenerate(options) {
		...
		const { responseHeaders, value: response, rawValue: rawResponse } = await require_dist.postJsonToApi({
			...
			body: args,
			failedResponseHandler: openaiFailedResponseHandler,
			successfulResponseHandler: require_dist.createJsonResponseHandler(openaiCompletionResponseSchema),
			abortSignal: options.abortSignal,
			fetch: this.config.fetch
		});
	...
	async doStream(options) {
		...
		const { responseHeaders, value: response } = await require_dist.postJsonToApi({
			...
			body,
			failedResponseHandler: openaiFailedResponseHandler,
			successfulResponseHandler: require_dist.createEventSourceResponseHandler(openaiCompletionChunkSchema),
			abortSignal: options.abortSignal,
			fetch: this.config.fetch
		});
		const { prompt: rawPrompt, ...rawSettings } = args;
...
var openaiCompletionChunkSchema = zod.z.union([zod.z.object({
	id: zod.z.string().nullish(),
	created: zod.z.number().nullish(),
	model: zod.z.string().nullish(),
	choices: zod.z.array(zod.z.object({
...
var OpenAIEmbeddingModel = class {
	...
	async doEmbed({ values, headers, abortSignal }) {
		...
		const { responseHeaders, value: response } = await require_dist.postJsonToApi({
			...
			failedResponseHandler: openaiFailedResponseHandler,
			successfulResponseHandler: require_dist.createJsonResponseHandler(openaiTextEmbeddingResponseSchema),
			abortSignal,
			fetch: this.config.fetch
		});
		return {
...
var OpenAIImageModel = class {
	...
	async doGenerate({ prompt, n, size, aspectRatio, seed, providerOptions, headers, abortSignal }) {
		...
		const { value: response, responseHeaders } = await require_dist.postJsonToApi({
			...
			failedResponseHandler: openaiFailedResponseHandler,
			successfulResponseHandler: require_dist.createJsonResponseHandler(openaiImageResponseSchema),
			abortSignal,
			fetch: this.config.fetch
		});
		return {
...
var openaiImageResponseSchema = zod.z.object({ data: zod.z.array(zod.z.object({ b64_json: zod.z.string() })) });
...
var OpenAITranscriptionModel = class {
	...
	getArgs({ audio, mediaType, providerOptions }) {
		var _a, _b, _c, _d, _e;
		const warnings = [];
		const openAIOptions = require_dist.parseProviderOptions({
			provider: "openai",
			providerOptions,
	...
	async doGenerate(options) {
		...
		const { value: response, responseHeaders, rawValue: rawResponse } = await require_dist.postFormDataToApi({
			...
			formData,
			failedResponseHandler: openaiFailedResponseHandler,
			successfulResponseHandler: require_dist.createJsonResponseHandler(openaiTranscriptionResponseSchema),
			abortSignal: options.abortSignal,
			fetch: this.config.fetch
		});
...
var openaiTranscriptionResponseSchema = zod.z.object({
	...
	words: zod.z.array(zod.z.object({
		word: zod.z.string(),
		start: zod.z.number(),
		end: zod.z.number()
	})).nullish()
			...
			messages.push({
				...
				content: content.map((part, index) => {
					...
					switch (part.type) {
						...
						case "image": return {
							type: "input_image",
							image_url: part.image instanceof URL ? part.image.toString() : `data:${(_a = part.mimeType) != null ? _a : "image/jpeg"};base64,${require_dist.convertUint8ArrayToBase64(part.image)}`,
							detail: (_c = (_b = part.providerMetadata) == null ? void 0 : _b.openai) == null ? void 0 : _c.imageDetail
						};
						case "file":
							if (part.data instanceof URL) throw new require_dist.UnsupportedFunctionalityError({ functionality: "File URLs in user messages" });
							switch (part.mimeType) {
								case "application/pdf": return {
									type: "input_file",
									filename: (_d = part.filename) != null ? _d : `part-${index}.pdf`,
								...
								default: throw new require_dist.UnsupportedFunctionalityError({ functionality: "Only PDF files are supported in user messages" });
							}
					}
				})
			});
...
var OpenAIResponsesLanguageModel = class {
	...
	async doGenerate(options) {
		...
		const { responseHeaders, value: response, rawValue: rawResponse } = await require_dist.postJsonToApi({
			...
			body,
			failedResponseHandler: openaiFailedResponseHandler,
			successfulResponseHandler: require_dist.createJsonResponseHandler(zod.z.object({
				id: zod.z.string(),
				...
				model: zod.z.string(),
				output: zod.z.array(zod.z.discriminatedUnion("type", [
					zod.z.object({
						...
						content: zod.z.array(zod.z.object({
							...
							text: zod.z.string(),
							annotations: zod.z.array(zod.z.object({
					...
					zod.z.object({ type: zod.z.literal("web_search_call") }),
					zod.z.object({ type: zod.z.literal("computer_call") }),
					zod.z.object({
						type: zod.z.literal("reasoning"),
						summary: zod.z.array(zod.z.object({
		...
		if (response.error) throw new require_dist.APICallError({
		...
		return {
			...
			sources: outputTextElements.flatMap((content) => content.annotations.map((annotation) => {
				...
				return {
					sourceType: "url",
					id: (_c2 = (_b2 = (_a2 = this.config).generateId) == null ? void 0 : _b2.call(_a2)) != null ? _c2 : require_dist.generateId(),
					url: annotation.url,
	...
	async doStream(options) {
		...
		const { responseHeaders, value: response } = await require_dist.postJsonToApi({
			...
			failedResponseHandler: openaiFailedResponseHandler,
			successfulResponseHandler: require_dist.createEventSourceResponseHandler(openaiResponsesChunkSchema),
			abortSignal: options.abortSignal,
			fetch: this.config.fetch
		});
		const self = this;
			...
			stream: response.pipeThrough(new TransformStream({
				transform(chunk, controller) {
					...
					if (isResponseOutputItemAddedChunk(value)) {
						...
						source: {
							sourceType: "url",
							id: (_h = (_g = (_f = self.config).generateId) == null ? void 0 : _g.call(_f)) != null ? _h : require_dist.generateId(),
...
var responseOutputItemDoneSchema = zod.z.object({
	...
	item: zod.z.discriminatedUnion("type", [zod.z.object({ type: zod.z.literal("message") }), zod.z.object({
		type: zod.z.literal("function_call"),
		id: zod.z.string(),
		call_id: zod.z.string(),
		name: zod.z.string(),
...
var OpenAIProviderOptionsSchema = zod.z.object({
	instructions: zod.z.string().nullish(),
	speed: zod.z.number().min(.25).max(4).default(1).nullish()
});
var OpenAISpeechModel = class {
	...
	async doGenerate(options) {
		...
		const { value: audio, responseHeaders, rawValue: rawResponse } = await require_dist.postJsonToApi({
			url: this.config.url({
				path: "/audio/speech",
				modelId: this.modelId
			}),
...
function createOpenAI(options = {}) {
	var _a, _b, _c;
	const baseURL = (_a = require_dist.withoutTrailingSlash(options.baseURL)) != null ? _a : "https://api.openai.com/v1";
...
//# sourceMappingURL=dist-2zVeGGHi.cjs.map