...
let _mastra_core_auth_ee = require("@mastra/core/auth/ee");
_mastra_core_auth_ee = require_rolldown_runtime.__toESM(_mastra_core_auth_ee, 1);
let _mastra_core_memory = require("@mastra/core/memory");
...
const realPermissions = _mastra_core_auth_ee.MastraFGAPermissions;
...
function validateBody(body) {
	...
	if (Object.keys(errorResponse).length > 0) throw new require_http_exception.HTTPException(400, { message: Object.values(errorResponse)[0] });
}
/**
...
function requireEffectiveResourceId(effectiveResourceId) {
	if (!effectiveResourceId) throw new require_http_exception.HTTPException(400, { message: "A resource ID is required when using memory. Provide memory.resource in the request body, or configure server auth with mapUserToResourceId to derive it from the authenticated user." });
}
/**
...
async function validateThreadOwnership(thread, effectiveResourceId) {
	if (thread && effectiveResourceId && thread.resourceId && thread.resourceId !== effectiveResourceId) throw new require_http_exception.HTTPException(403, { message: "Access denied: thread belongs to a different resource" });
}
/**
...
async function enforceThreadAccess({ mastra, requestContext, threadId, thread, effectiveResourceId, permission = MastraFGAPermissions.MEMORY_READ }) {
	...
	if (!user || typeof user !== "object") throw new require_http_exception.HTTPException(403, { message: "FGA authorization denied: authenticated user is required" });
	await _mastra_core_memory.MastraMemory.checkThreadFGA({
		mastra,
		user,
		threadId,
...
async function validateRunOwnership(run, effectiveResourceId) {
	if (run && effectiveResourceId && run.resourceId && run.resourceId !== effectiveResourceId) throw new require_http_exception.HTTPException(403, { message: "Access denied: workflow run belongs to a different resource" });
}
//#endregion
...
Object.defineProperty(exports, "requireEffectiveResourceId", {
	enumerable: true,
	get: function() {
		return requireEffectiveResourceId;
	}
});
...
//# sourceMappingURL=utils-DJMUhfvY.cjs.map