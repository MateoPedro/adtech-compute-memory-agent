				...
				messages.push({
					...
					content: content.map((part) => {
						...
						switch (part.type) {
							...
							case "image": return {
								type: "image_url",
								image_url: { url: part.image instanceof URL ? part.image.toString() : `data:${(_a = part.mimeType) != null ? _a : "image/jpeg"};base64,${require_dist.convertUint8ArrayToBase64(part.image)}` },
								...partMetadata
							};
							case "file": throw new require_dist.UnsupportedFunctionalityError({ functionality: "File content parts in user messages" });
						}
					}),
					...metadata
				});
...
var defaultOpenAICompatibleErrorStructure = {
	errorSchema: zod.z.object({ error: zod.z.object({
...
var OpenAICompatibleChatLanguageModel = class {
	constructor(modelId, settings, config) {
		...
		this.failedResponseHandler = require_dist.createJsonErrorResponseHandler(errorStructure);
	...
	async doGenerate(options) {
		...
		const { responseHeaders, value: responseBody, rawValue: rawResponse } = await require_dist.postJsonToApi({
			...
			successfulResponseHandler: require_dist.createJsonResponseHandler(OpenAICompatibleChatResponseSchema),
	...
	async doStream(options) {
		...
		const { responseHeaders, value: response } = await require_dist.postJsonToApi({
			...
			body,
			failedResponseHandler: this.failedResponseHandler,
			successfulResponseHandler: require_dist.createEventSourceResponseHandler(this.chunkSchema),
			abortSignal: options.abortSignal,
			fetch: this.config.fetch
		});
		const { messages: rawPrompt, ...rawSettings } = args;
				...
				transform(chunk, controller) {
					...
					if (delta.tool_calls != null) for (const toolCallDelta of delta.tool_calls) {
						...
						if (toolCalls[index] == null) {
							...
							if (((_a2 = toolCallDelta.function) == null ? void 0 : _a2.name) == null) throw new require_dist.InvalidResponseDataError({
								data: toolCallDelta,
...
var createOpenAICompatibleChatChunkSchema = (errorSchema) => zod.z.union([zod.z.object({
	id: zod.z.string().nullish(),
	created: zod.z.number().nullish(),
	model: zod.z.string().nullish(),
	choices: zod.z.array(zod.z.object({
...
var OpenAICompatibleImageModel = class {
	...
	async doGenerate({ prompt, n, size, aspectRatio, seed, providerOptions, headers, abortSignal }) {
		...
		const { value: response, responseHeaders } = await require_dist.postJsonToApi({
			...
			failedResponseHandler: require_dist.createJsonErrorResponseHandler((_e = this.config.errorStructure) != null ? _e : defaultOpenAICompatibleErrorStructure),
			successfulResponseHandler: require_dist.createJsonResponseHandler(openaiCompatibleImageResponseSchema),
			abortSignal,
			fetch: this.config.fetch
		});
		return {
...
var openaiCompatibleImageResponseSchema = zod.z.object({ data: zod.z.array(zod.z.object({ b64_json: zod.z.string() })) });
//#endregion
...
//# sourceMappingURL=dist-Be7Caz3Z.cjs.map