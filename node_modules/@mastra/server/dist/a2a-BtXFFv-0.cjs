...
let _mastra_core_a2a = require("@mastra/core/a2a");
...
let net = require("net");
//#region ../../../../../setup-pnpm/node_modules/.bin/store/v11/links/@/canonicalize/1.0.8/8d710dcf19a48d178b2fc853e16c59951fde29eb27b6037da6c3f4dbb968b03d/node_modules/canonicalize/lib/canonicalize.js
var require_canonicalize = /* @__PURE__ */ require_rolldown_runtime.__commonJSMin(((exports, module) => {
...
var import_canonicalize = /* @__PURE__ */ require_rolldown_runtime.__toESM(require_canonicalize(), 1);
...
function normalizeError(error, reqId, taskId, logger) {
	let a2aError;
	if (error instanceof _mastra_core_a2a.MastraA2AError) a2aError = error;
	else if (error instanceof Error) a2aError = _mastra_core_a2a.MastraA2AError.internalError(error.message, { stack: error.stack });
	else a2aError = _mastra_core_a2a.MastraA2AError.internalError("An unknown error occurred.", error);
...
function isDisallowedIpAddress(address) {
	const version = (0, net.isIP)(address);
...
var DefaultPushNotificationSender = class {
	...
	async resolveValidatedDestination(rawUrl) {
		...
		const resolvedAddresses = (0, net.isIP)(hostname) === 0 ? await (this.options.lookup ?? dns_promises.lookup)(hostname, {
			all: true,
			verbatim: true
		}) : [{
			address: hostname,
			family: (0, net.isIP)(hostname)
		}];
		...
		return {
			...
			requestUrl,
			hostHeader: url.host,
			servername: (0, net.isIP)(hostname) === 0 ? hostname : void 0
		};
	}
...
var a2a_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
...
const messagePartSchema = zod_v4.z.discriminatedUnion("kind", [
	zod_v4.z.object({
		kind: zod_v4.z.literal("text"),
		text: zod_v4.z.string(),
		metadata: zod_v4.z.record(zod_v4.z.string(), zod_v4.z.any()).optional()
	}),
	zod_v4.z.object({
		kind: zod_v4.z.literal("file"),
		file: zod_v4.z.union([zod_v4.z.object({
			bytes: zod_v4.z.string(),
	...
	zod_v4.z.object({
		kind: zod_v4.z.literal("data"),
		data: zod_v4.z.record(zod_v4.z.string(), zod_v4.z.any()),
		metadata: zod_v4.z.record(zod_v4.z.string(), zod_v4.z.any()).optional()
	})
]);
const messageSendParamsSchema = zod_v4.z.object({
	message: zod_v4.z.object({
		role: zod_v4.z.enum(["user", "agent"]),
		parts: zod_v4.z.array(messagePartSchema),
		kind: zod_v4.z.literal("message"),
		messageId: zod_v4.z.string(),
		contextId: zod_v4.z.string().optional(),
		taskId: zod_v4.z.string().optional(),
		referenceTaskIds: zod_v4.z.array(zod_v4.z.string()).optional(),
		extensions: zod_v4.z.array(zod_v4.z.string()).optional(),
		metadata: zod_v4.z.record(zod_v4.z.string(), zod_v4.z.any()).optional()
	}),
	configuration: zod_v4.z.object({
		...
		pushNotificationConfig: zod_v4.z.object({
			...
			token: zod_v4.z.string().optional(),
			authentication: zod_v4.z.object({
				schemes: zod_v4.z.array(zod_v4.z.string()),
				credentials: zod_v4.z.string().optional()
			}).optional()
		}).optional()
	}).optional()
});
...
async function getAgentCardByIdHandler({ mastra, agentId, executionUrl = `/a2a/${agentId}`, provider = {
	...
	const agentCard = {
		...
		description: require_utils.convertInstructionsToString(instructions),
		url: executionUrl,
		provider,
		version,
...
function getA2AExecutionUrl({ agentId, request, routePrefix }) {
	...
	return `${require_server_handlers_auth.getPublicOrigin(request)}${executionPath}`;
}
function validateMessageSendParams(params) {
	try {
		messageSendParamsSchema.parse(params);
	} catch (error) {
		if (error instanceof zod_v4.z.ZodError) throw _mastra_core_a2a.MastraA2AError.invalidParams(error.issues[0].message);
		throw error;
	}
}
...
function areStatusMessagePartsEqual(left, right) {
	return left === right || (0, util.isDeepStrictEqual)(left, right);
}
function areStatusMessagesEqual(left, right) {
	if (left === right) return true;
	if (!left || !right) return left === right;
	return left.messageId === right.messageId && left.kind === right.kind && left.role === right.role && left.contextId === right.contextId && left.taskId === right.taskId && (0, util.isDeepStrictEqual)(left.referenceTaskIds, right.referenceTaskIds) && (0, util.isDeepStrictEqual)(left.extensions, right.extensions) && (0, util.isDeepStrictEqual)(left.metadata, right.metadata) && areStatusMessagePartsEqual(left.parts, right.parts);
}
...
async function handleGetTaskPushNotificationConfig({ requestId, taskStore, pushNotificationStore, agentId, params }) {
	...
	if (!config) throw _mastra_core_a2a.MastraA2AError.invalidParams(`Push notification config not found: ${params.pushNotificationConfigId ?? params.id}`);
...
async function getAgentExecutionHandler({ requestId, mastra, agentId, requestContext, method, params, taskStore, pushNotificationStore, pushNotificationSender, logger, abortSignal, protocolVersion = "0.3" }) {
	...
	try {
		...
		switch (method) {
			...
			case "agent/getAuthenticatedExtendedCard": throw _mastra_core_a2a.MastraA2AError.extendedAgentCardNotConfigured();
			default: throw _mastra_core_a2a.MastraA2AError.methodNotFound(method);
		}
	} catch (error) {
		if (error instanceof _mastra_core_a2a.MastraA2AError && taskId && !error.taskId) error.taskId = taskId;
...
const GET_AGENT_CARD_ROUTE = require_route_builder.createRoute({
	...
	responseType: "json",
	pathParamSchema: require_a2a.a2aAgentIdPathParams,
	responseSchema: require_a2a.agentCardResponseSchema,
...
const AGENT_EXECUTION_ROUTE = require_route_builder.createRoute({
	method: "POST",
	path: "/a2a/:agentId",
	responseType: "datastream-response",
	pathParamSchema: require_a2a.a2aAgentIdPathParams,
	bodySchema: require_a2a.agentExecutionBodySchema,
	responseSchema: require_a2a.agentExecutionResponseSchema,
...
Object.defineProperty(exports, "AGENT_EXECUTION_ROUTE", {
	enumerable: true,
	get: function() {
		return AGENT_EXECUTION_ROUTE;
	}
});
Object.defineProperty(exports, "GET_AGENT_CARD_ROUTE", {
	enumerable: true,
	get: function() {
		return GET_AGENT_CARD_ROUTE;
	}
});
Object.defineProperty(exports, "a2a_exports", {
	enumerable: true,
	get: function() {
		return a2a_exports;
	}
...
Object.defineProperty(exports, "handleSetTaskPushNotificationConfig", {
	enumerable: true,
	get: function() {
		return handleSetTaskPushNotificationConfig;
	}
});
Object.defineProperty(exports, "handleTaskCancel", {
	enumerable: true,
	get: function() {
		return handleTaskCancel;
	}
});
...
//# sourceMappingURL=a2a-BtXFFv-0.cjs.map