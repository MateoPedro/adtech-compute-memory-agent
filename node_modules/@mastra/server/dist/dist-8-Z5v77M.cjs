...
function convertToGoogleGenerativeAIMessages(prompt) {
	...
	for (const { role, content } of prompt) switch (role) {
		case "system":
			if (!systemMessagesAllowed) throw new require_dist.UnsupportedFunctionalityError({ functionality: "system messages are only supported at the beginning of the conversation" });
			...
			for (const part of content) switch (part.type) {
				...
				case "image":
					parts.push(part.image instanceof URL ? { fileData: {
						...
						data: require_dist.convertUint8ArrayToBase64(part.image)
					} });
					break;
				case "file":
			...
			contents.push({
				...
				parts: content.map((part) => {
					switch (part.type) {
						...
						case "file":
							if (part.mimeType !== "image/png") throw new require_dist.UnsupportedFunctionalityError({ functionality: "Only PNG images are supported in assistant messages" });
							if (part.data instanceof URL) throw new require_dist.UnsupportedFunctionalityError({ functionality: "File data URLs in assistant messages are not supported" });
							return { inlineData: {
								mimeType: part.mimeType,
								data: part.data
							} };
...
var googleFailedResponseHandler = require_dist.createJsonErrorResponseHandler({
...
var GoogleGenerativeAILanguageModel = class {
	...
	async getArgs({ mode, prompt, maxTokens, temperature, topP, topK, frequencyPenalty, presencePenalty, stopSequences, responseFormat, seed, providerMetadata }) {
		...
		const warnings = [];
		const googleOptions = require_dist.parseProviderOptions({
			provider: "google",
	...
	async doGenerate(options) {
		...
		const mergedHeaders = require_dist.combineHeaders(await require_dist.resolve(this.config.headers), options.headers);
		const { responseHeaders, value: response, rawValue: rawResponse } = await require_dist.postJsonToApi({
			...
			successfulResponseHandler: require_dist.createJsonResponseHandler(responseSchema),
	...
	async doStream(options) {
		const { args, warnings } = await this.getArgs(options);
		const body = JSON.stringify(args);
		const headers = require_dist.combineHeaders(await require_dist.resolve(this.config.headers), options.headers);
		const { responseHeaders, value: response } = await require_dist.postJsonToApi({
			...
			body: args,
			failedResponseHandler: googleFailedResponseHandler,
			successfulResponseHandler: require_dist.createEventSourceResponseHandler(chunkSchema),
			abortSignal: options.abortSignal,
			fetch: this.config.fetch
		});
		const { contents: rawPrompt, ...rawSettings } = args;
...
var contentSchema = zod.z.object({ parts: zod.z.array(zod.z.union([
	zod.z.object({ functionCall: zod.z.object({
		name: zod.z.string(),
		args: zod.z.unknown()
	}) }),
	zod.z.object({ inlineData: zod.z.object({
...
var groundingMetadataSchema = zod.z.object({
	...
	groundingSupports: zod.z.array(zod.z.object({
	...
	retrievalMetadata: zod.z.union([zod.z.object({ webDynamicRetrievalScore: zod.z.number() }), zod.z.object({})]).nullish()
});
...
var GoogleGenerativeAIEmbeddingModel = class {
	...
	async doEmbed({ values, headers, abortSignal }) {
		...
		const mergedHeaders = require_dist.combineHeaders(await require_dist.resolve(this.config.headers), headers);
		const { responseHeaders, value: response } = await require_dist.postJsonToApi({
			...
			failedResponseHandler: googleFailedResponseHandler,
			successfulResponseHandler: require_dist.createJsonResponseHandler(googleGenerativeAITextEmbeddingResponseSchema),
			abortSignal,
			fetch: this.config.fetch
		});
		return {
...
var googleGenerativeAITextEmbeddingResponseSchema = zod.z.object({ embeddings: zod.z.array(zod.z.object({ values: zod.z.array(zod.z.number()) })) });
function isSupportedFileUrl(url) {
	return url.toString().startsWith("https://generativelanguage.googleapis.com/v1beta/files/");
}
function createGoogleGenerativeAI(options = {}) {
	var _a;
	const baseURL = (_a = require_dist.withoutTrailingSlash(options.baseURL)) != null ? _a : "https://generativelanguage.googleapis.com/v1beta";
	const getHeaders = () => ({
		"x-goog-api-key": require_dist.loadApiKey({
...
//# sourceMappingURL=dist-8-Z5v77M.cjs.map