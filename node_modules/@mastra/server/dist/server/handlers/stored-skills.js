...
*/
const LIST_STORED_SKILLS_ROUTE = createRoute({
	method: "GET",
	path: "/stored/skills",
	responseType: "json",
	...
	handler: async ({ mastra, requestContext, page, perPage, orderBy, status, authorId, visibility, metadata, favoritedOnly, pinFavoritedFor }) => {
		try {
			...
			if (honoredStarredOnly) {
				...
				})).skills.filter((record) => matchesAuthorFilter(record, filter));
			...
			if (!favoritesEnabled) return {
				...result,
				skills: visibleSkills.map(stripFavoriteFields)
			};
			const enrichment = await prepareFavoritesEnrichment(mastra, requestContext, "skill", visibleSkills.map((s) => s.id));
...
*/
const GET_STORED_SKILL_ROUTE = createRoute({
	method: "GET",
...
*/
const CREATE_STORED_SKILL_ROUTE = createRoute({
	method: "POST",
	path: "/stored/skills",
	responseType: "json",
	...
	handler: async ({ mastra, requestContext, id: providedId, name, description, instructions, license, compatibility, source, references, scripts, assets, files, metadata, visibility: bodyVisibility }) => {
		try {
			...
			if (!id) throw new HTTPException(400, { message: "Could not derive skill ID from name. Please provide an explicit id." });
			if (await skillStore.getById(id)) throw new HTTPException(409, { message: `Skill with id ${id} already exists` });
			...
			if (!resolved) throw new HTTPException(500, { message: "Failed to resolve created skill" });
...
*/
const UPDATE_STORED_SKILL_ROUTE = createRoute({
	method: "PATCH",
	path: "/stored/skills/:storedSkillId",
	responseType: "json",
	pathParamSchema: storedSkillIdPathParams,
	...
	handler: async ({ mastra, requestContext, storedSkillId, authorId, visibility, name, description, instructions, license, compatibility, source, references, scripts, assets, files, metadata }) => {
		try {
			...
			if (!existing) throw new HTTPException(404, { message: `Stored skill with id ${storedSkillId} not found` });
			...
			if (metadata !== void 0) update.metadata = scopeStoredResourceMetadata({
				...existing.metadata ?? {},
				...metadata
			}, scope);
			await skillStore.update(update);
			const resolved = await skillStore.getByIdResolved(storedSkillId);
			if (!resolved) throw new HTTPException(500, { message: "Failed to resolve updated skill" });
			return enrichOrStripFavorites(mastra, requestContext, "skill", resolved);
		} catch (error) {
...
*/
const DELETE_STORED_SKILL_ROUTE = createRoute({
	method: "DELETE",
	path: "/stored/skills/:storedSkillId",
	responseType: "json",
	pathParamSchema: storedSkillIdPathParams,
...
*/
const PUBLISH_STORED_SKILL_ROUTE = createRoute({
	method: "POST",
	...
	handler: async ({ mastra, requestContext, storedSkillId, skillPath }) => {
		try {
			...
			if (!blobStore) throw new HTTPException(500, { message: "Blob storage domain is not available" });
			const existing = await skillStore.getByIdResolved(storedSkillId);
			if (!existing) throw new HTTPException(404, { message: `Stored skill with id ${storedSkillId} not found` });
			assertStoredResourceScope(existing, await getStoredResourceScope(mastra, requestContext));
			assertWriteAccess({
			...
			if (!resolvedPath.startsWith(allowedBase + path.default.sep) && resolvedPath !== allowedBase) throw new HTTPException(400, { message: `skillPath must be within the allowed directory: ${allowedBase}` });
			try {
				if (!(await fs.stat(resolvedPath)).isDirectory()) throw new HTTPException(400, { message: `skillPath is not a directory: ${resolvedPath}` });
			} catch (err) {
				if (err instanceof HTTPException) throw err;
				if (err?.code === "ENOENT") throw new HTTPException(400, { message: `skillPath does not exist on the server filesystem: ${resolvedPath}. Create the skill directory (with a SKILL.md) before publishing, or use a skill that was materialized to disk.` });
				throw err;
			}
			try {
				await fs.stat(path.default.join(resolvedPath, "SKILL.md"));
			} catch (err) {
				if (err?.code === "ENOENT") throw new HTTPException(400, { message: `skillPath is missing SKILL.md: ${resolvedPath}` });
				throw err;
			}
			...
			const resolved = await skillStore.getByIdResolved(storedSkillId);
			if (!resolved) throw new HTTPException(500, { message: "Failed to resolve skill after publish" });
			return enrichOrStripFavorites(mastra, requestContext, "skill", resolved);
		} catch (error) {
