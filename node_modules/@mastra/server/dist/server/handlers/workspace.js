...
import { B as skillsShPopularQuerySchema, C as searchQuerySchema, D as searchSkillsResponseSchema, E as searchSkillsQuerySchema, G as skillsShSearchQuerySchema, H as skillsShPreviewResponseSchema, K as skillsShSearchResponseSchema, L as skillsShInstallBodySchema, M as skillReferencePathParams, N as skillReferenceResponseSchema, O as skillDisambiguationQuerySchema, Q as workspaceInfoResponseSchema, R as skillsShInstallResponseSchema, U as skillsShRemoveBodySchema, V as skillsShPreviewQuerySchema, W as skillsShRemoveResponseSchema, X as skillsShUpdateResponseSchema, Y as skillsShUpdateBodySchema, Z as workspaceIdPathParams, _ as indexBodySchema, a as fsListResponseSchema, b as listSkillsResponseSchema, d as fsStatQuerySchema, f as fsStatResponseSchema, g as getSkillResponseSchema, i as fsListQuerySchema, j as skillNamePathParams, l as fsReadQuerySchema, m as fsWriteResponseSchema, n as fsDeleteQuerySchema, o as fsMkdirBodySchema, p as fsWriteBodySchema, r as fsDeleteResponseSchema, s as fsMkdirResponseSchema, u as fsReadResponseSchema, v as indexResponseSchema, w as searchResponseSchema, x as listWorkspacesResponseSchema, y as listReferencesResponseSchema, z as skillsShListResponseSchema } from "../../workspace-S5GiJ3Nx.js";
...
function handleWorkspaceError(error, defaultMessage) {
	if (isFilesystemNotFoundError(error)) throw new HTTPException(404, { message: error instanceof Error ? error.message : "Not found" });
	if (isFilesystemPermissionError(error)) throw new HTTPException(403, { message: error instanceof Error ? error.message : "Permission denied" });
...
function requireWorkspaceV1Support() {
	if (!coreFeatures.has("workspaces-v1")) throw new HTTPException(501, { message: "Workspace v1 not supported by this version of @mastra/core. Please upgrade to a newer version." });
}
/**
...
function buildSkillInstallPath(filesystem, safeSkillId, requestedMount) {
	if (isCompositeFilesystem(filesystem)) {
		if (requestedMount) {
			...
			if (!mountFs) throw new HTTPException(400, { message: `Mount "${requestedMount}" not found. Available mounts: ${filesystem.mountPaths.join(", ")}` });
			if (mountFs.readOnly) throw new HTTPException(403, { message: `Mount "${requestedMount}" is read-only` });
...
const LIST_WORKSPACES_ROUTE = createRoute({
	method: "GET",
	path: "/workspaces",
	responseType: "json",
...
const GET_WORKSPACE_ROUTE = createRoute({
	method: "GET",
...
const WORKSPACE_FS_READ_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ mastra, path, encoding, workspaceId }) => {
		try {
			...
			const workspace = await getWorkspaceById(mastra, workspaceId);
			if (!workspace?.filesystem) throw new HTTPException(404, { message: "No workspace filesystem configured" });
			const decodedPath = decodeURIComponent(path);
			if (!await workspace.filesystem.exists(decodedPath)) throw new HTTPException(404, { message: `Path "${decodedPath}" not found` });
...
const WORKSPACE_FS_WRITE_ROUTE = createRoute({
	method: "POST",
	...
	handler: async ({ mastra, path, content, encoding, recursive, workspaceId }) => {
		try {
			requireWorkspaceV1Support();
			if (!path || content === void 0) throw new HTTPException(400, { message: "Path and content are required" });
			const workspace = await getWorkspaceById(mastra, workspaceId);
			if (!workspace?.filesystem) throw new HTTPException(404, { message: "No workspace filesystem configured" });
			if (workspace.filesystem?.readOnly) throw new HTTPException(403, { message: "Workspace is in read-only mode" });
			const decodedPath = decodeURIComponent(path);
...
const WORKSPACE_FS_LIST_ROUTE = createRoute({
	method: "GET",
...
const WORKSPACE_FS_DELETE_ROUTE = createRoute({
	method: "DELETE",
	...
	handler: async ({ mastra, path, recursive, force, workspaceId }) => {
		try {
			...
			if (!exists && !force) throw new HTTPException(404, { message: `Path "${decodedPath}" not found` });
			if (exists) try {
...
const WORKSPACE_FS_MKDIR_ROUTE = createRoute({
	method: "POST",
...
const WORKSPACE_FS_STAT_ROUTE = createRoute({
	method: "GET",
...
const WORKSPACE_SEARCH_ROUTE = createRoute({
	method: "GET",
...
const WORKSPACE_INDEX_ROUTE = createRoute({
	method: "POST",
	...
	handler: async ({ mastra, path, content, metadata, workspaceId }) => {
		try {
			...
			if (!path || content === void 0) throw new HTTPException(400, { message: "Path and content are required" });
			const workspace = await getWorkspaceById(mastra, workspaceId);
			if (!workspace) throw new HTTPException(404, { message: "No workspace configured" });
			if (!(workspace.canBM25 || workspace.canVector)) throw new HTTPException(400, { message: "Workspace does not have search configured" });
...
const WORKSPACE_LIST_SKILLS_ROUTE = createRoute({
	method: "GET",
...
const WORKSPACE_GET_SKILL_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ mastra, skillName, path, workspaceId, requestContext }) => {
		try {
			requireWorkspaceV1Support();
			if (!skillName) throw new HTTPException(400, { message: "Skill name is required" });
			const identifier = path ? decodeURIComponent(path) : skillName;
			const skills = await getSkillsById(mastra, workspaceId);
			if (!skills) throw new HTTPException(404, { message: "No workspace with skills configured" });
			await skills.maybeRefresh({ requestContext });
...
const WORKSPACE_LIST_SKILL_REFERENCES_ROUTE = createRoute({
	method: "GET",
...
const WORKSPACE_GET_SKILL_REFERENCE_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ mastra, skillName, path: skillPath, referencePath, workspaceId, requestContext }) => {
		try {
			requireWorkspaceV1Support();
			if (!skillName || !referencePath) throw new HTTPException(400, { message: "Skill name and reference path are required" });
			...
			if (content === null) throw new HTTPException(404, { message: `Reference "${decodedPath}" not found in skill "${identifier}"` });
			return {
				skillName: skill.name,
...
const WORKSPACE_SEARCH_SKILLS_ROUTE = createRoute({
	method: "GET",
...
const WORKSPACE_SKILLS_SH_SEARCH_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ q, limit }) => {
		try {
			...
			if (!response.ok) throw new HTTPException(502, { message: `Skills API error: ${response.status} ${response.statusText}` });
...
const WORKSPACE_SKILLS_SH_POPULAR_ROUTE = createRoute({
	method: "GET",
...
function assertSafeSkillName(name) {
	if (!SKILL_NAME_REGEX.test(name)) throw new HTTPException(400, { message: `Invalid skill name "${name}". Names must start with alphanumeric and contain only letters, numbers, hyphens, and underscores.` });
	return name;
}
/**
...
function assertSafeFilePath(filePath) {
	if (filePath.startsWith("/") || /^[a-zA-Z]:/.test(filePath)) throw new HTTPException(400, { message: `Invalid file path "${filePath}". Absolute paths are not allowed.` });
	const segments = filePath.split("/");
	for (const segment of segments) if (segment === ".." || segment === ".") throw new HTTPException(400, { message: `Invalid file path "${filePath}". Path traversal is not allowed.` });
	return filePath;
}
/**
...
const WORKSPACE_SKILLS_SH_PREVIEW_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ owner, repo, path: skillName }) => {
		try {
			...
			const response = await fetch(url, { signal: controller.signal });
			clearTimeout(timeoutId);
			if (!response.ok) throw new HTTPException(404, { message: `Could not find skill "${skillName}" for ${owner}/${repo}` });
			const data = await response.json();
			const content = data.instructions || data.raw || "";
			if (!content) throw new HTTPException(404, { message: `No content available for skill "${skillName}"` });
			return { content };
		} catch (error) {
			if (error instanceof HTTPException) throw error;
...
const WORKSPACE_SKILLS_SH_INSTALL_ROUTE = createRoute({
	method: "POST",
	...
	handler: async ({ mastra, workspaceId, owner, repo, skillName, mount }) => {
		try {
			...
			if (!workspace.filesystem) throw new HTTPException(400, { message: "Workspace filesystem not available" });
			if (workspace.filesystem.readOnly) throw new HTTPException(403, { message: "Workspace is read-only" });
...
const WORKSPACE_SKILLS_SH_REMOVE_ROUTE = createRoute({
	method: "POST",
...
const WORKSPACE_SKILLS_SH_UPDATE_ROUTE = createRoute({
	method: "POST",
