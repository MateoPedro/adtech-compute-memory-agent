...
import { a as wrapSchemaForQueryParams } from "../../route-builder-YQ_gLIvW.js";
import { deltaCursorSchema, deltaLimitSchema, listModeSchema, paginationArgsSchema } from "./observability-list-query-schemas.js";
...
function getStorage(mastra) {
	...
	if (!storage) throw new HTTPException(500, { message: "Storage is not available" });
	return storage;
}
...
async function getObservabilityStore(mastra) {
	...
	if (!observability) throw new HTTPException(501, { message: "Observability storage domain is not available" });
...
async function getScoresStore(mastra) {
	...
	if (!scores) throw new HTTPException(501, { message: "Scores storage domain is not available" });
	return scores;
}
function assertObservabilityDeltaSupported(observabilityStore, endpoint) {
	if (!coreFeatures.has("observability-delta-polling")) throw new HTTPException(501, { message: `${OBSERVABILITY_DELTA_POLLING_UPGRADE_MESSAGE} (endpoint: ${endpoint})` });
	if (getFeatures(observabilityStore)?.includes(OBSERVABILITY_DELTA_POLLING_STORAGE_FEATURE)) return;
	throw new HTTPException(501, { message: `Delta polling is not supported by the configured observability store for ${endpoint}` });
}
...
//#endregion
export { NEW_ROUTE_DEFS, OBSERVABILITY_DELTA_POLLING_FEATURE, OBSERVABILITY_DELTA_POLLING_UPGRADE_MESSAGE, OBSERVABILITY_LIST_ENDPOINTS, assertObservabilityDeltaSupported, createObservabilityListQuerySchema, getObservabilityStore, getScoresStore, getStorage };
