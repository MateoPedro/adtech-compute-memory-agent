/**
 * GET /tool-providers — List all registered tool providers with their
 * capabilities (when the provider exposes them).
 */
export declare const LIST_TOOL_PROVIDERS_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: import("../server-adapter").ServerContext): Promise<{
        providers: {
            id: string;
            name: string;
            description?: string | undefined;
            displayName?: string | undefined;
            capabilities?: {
                multipleConnectionsPerToolkit: boolean;
                batchConnectionStatus: boolean;
                reauthorizeReusesConnectionId: boolean;
                supportsRevoke?: boolean | undefined;
            } | undefined;
        }[];
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<undefined, undefined, undefined, import("zod").ZodObject<{
        providers: import("zod").ZodArray<import("zod").ZodObject<{
            id: import("zod").ZodString;
            name: import("zod").ZodString;
            description: import("zod").ZodOptional<import("zod").ZodString>;
            displayName: import("zod").ZodOptional<import("zod").ZodString>;
            capabilities: import("zod").ZodOptional<import("zod").ZodObject<{
                multipleConnectionsPerToolkit: import("zod").ZodBoolean;
                batchConnectionStatus: import("zod").ZodBoolean;
                reauthorizeReusesConnectionId: import("zod").ZodBoolean;
                supportsRevoke: import("zod").ZodOptional<import("zod").ZodBoolean>;
            }, import("zod/v4/core").$strip>>;
        }, import("zod/v4/core").$strip>>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * GET /tool-providers/:providerId/toolkits — Toolkits exposed by a provider.
 */
export declare const LIST_TOOL_PROVIDER_TOOLKITS_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers/:providerId/toolkits";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
    } & import("../server-adapter").ServerContext): Promise<{
        data: {
            slug: string;
            name: string;
            description?: string | undefined;
            icon?: string | undefined;
        }[];
        pagination?: {
            hasMore: boolean;
            total?: number | undefined;
            page?: number | undefined;
            perPage?: number | undefined;
        } | undefined;
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, undefined, undefined, import("zod").ZodObject<{
        data: import("zod").ZodArray<import("zod").ZodObject<{
            slug: import("zod").ZodString;
            name: import("zod").ZodString;
            description: import("zod").ZodOptional<import("zod").ZodString>;
            icon: import("zod").ZodOptional<import("zod").ZodString>;
        }, import("zod/v4/core").$strip>>;
        pagination: import("zod").ZodOptional<import("zod").ZodObject<{
            total: import("zod").ZodOptional<import("zod").ZodNumber>;
            page: import("zod").ZodOptional<import("zod").ZodNumber>;
            perPage: import("zod").ZodOptional<import("zod").ZodNumber>;
            hasMore: import("zod").ZodBoolean;
        }, import("zod/v4/core").$strip>>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * GET /tool-providers/:providerId/tools — List tools, optionally filtered.
 */
export declare const LIST_TOOL_PROVIDER_TOOLS_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers/:providerId/tools";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
    } & {
        toolkit?: string | undefined;
        search?: string | undefined;
        page?: number | undefined;
        perPage?: number | undefined;
    } & import("../server-adapter").ServerContext): Promise<{
        data: {
            slug: string;
            name: string;
            description?: string | undefined;
            toolkit?: string | undefined;
        }[];
        pagination?: {
            hasMore: boolean;
            total?: number | undefined;
            page?: number | undefined;
            perPage?: number | undefined;
        } | undefined;
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, import("zod").ZodObject<{
        toolkit: import("zod").ZodOptional<import("zod").ZodString>;
        search: import("zod").ZodOptional<import("zod").ZodString>;
        page: import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>;
        perPage: import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>;
    }, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
        data: import("zod").ZodArray<import("zod").ZodObject<{
            slug: import("zod").ZodString;
            name: import("zod").ZodString;
            description: import("zod").ZodOptional<import("zod").ZodString>;
            toolkit: import("zod").ZodOptional<import("zod").ZodString>;
        }, import("zod/v4/core").$strip>>;
        pagination: import("zod").ZodOptional<import("zod").ZodObject<{
            total: import("zod").ZodOptional<import("zod").ZodNumber>;
            page: import("zod").ZodOptional<import("zod").ZodNumber>;
            perPage: import("zod").ZodOptional<import("zod").ZodNumber>;
            hasMore: import("zod").ZodBoolean;
        }, import("zod/v4/core").$strip>>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * GET /tool-providers/:providerId/tools/:toolSlug/schema — Tool schema.
 */
export declare const GET_TOOL_PROVIDER_TOOL_SCHEMA_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers/:providerId/tools/:toolSlug/schema";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
        toolSlug: string;
    } & import("../server-adapter").ServerContext): Promise<Record<string, unknown>>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
        toolSlug: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, undefined, undefined, import("zod").ZodRecord<import("zod").ZodString, import("zod").ZodUnknown>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * POST /tool-providers/:providerId/authorize — Start an OAuth flow and persist
 * a `tool_provider_connections` row for label / scope joins.
 */
export declare const AUTHORIZE_TOOL_PROVIDER_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "POST";
    path: "/tool-providers/:providerId/authorize";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
    } & {
        toolkit: string;
        connectionId?: string | undefined;
        toolName?: string | undefined;
        config?: Record<string, unknown> | undefined;
        label?: string | null | undefined;
        scope?: "shared" | "per-author" | "caller-supplied" | undefined;
    } & import("../server-adapter").ServerContext): Promise<{
        url: string;
        authId: string;
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
        toolkit: import("zod").ZodString;
        connectionId: import("zod").ZodOptional<import("zod").ZodString>;
        toolName: import("zod").ZodOptional<import("zod").ZodString>;
        config: import("zod").ZodOptional<import("zod").ZodRecord<import("zod").ZodString, import("zod").ZodUnknown>>;
        label: import("zod").ZodOptional<import("zod").ZodNullable<import("zod").ZodString>>;
        scope: import("zod").ZodOptional<import("zod").ZodEnum<{
            shared: "shared";
            "per-author": "per-author";
            "caller-supplied": "caller-supplied";
        }>>;
    }, import("zod/v4/core").$strip>, import("zod").ZodObject<{
        url: import("zod").ZodString;
        authId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * GET /tool-providers/:providerId/auth-status/:authId — Poll OAuth flow status.
 */
export declare const GET_TOOL_PROVIDER_AUTH_STATUS_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers/:providerId/auth-status/:authId";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
        authId: string;
    } & import("../server-adapter").ServerContext): Promise<{
        status: "failed" | "pending" | "completed";
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
        authId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, undefined, undefined, import("zod").ZodObject<{
        status: import("zod").ZodEnum<{
            failed: "failed";
            pending: "pending";
            completed: "completed";
        }>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * POST /tool-providers/:providerId/connection-status — Batch-check connection liveness.
 */
export declare const TOOL_PROVIDER_CONNECTION_STATUS_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "POST";
    path: "/tool-providers/:providerId/connection-status";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
    } & {
        items: {
            connectionId: string;
            toolkit: string;
        }[];
    } & import("../server-adapter").ServerContext): Promise<{
        items: Record<string, {
            connected: boolean;
        }>;
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
        items: import("zod").ZodArray<import("zod").ZodObject<{
            connectionId: import("zod").ZodString;
            toolkit: import("zod").ZodString;
        }, import("zod/v4/core").$strip>>;
    }, import("zod/v4/core").$strip>, import("zod").ZodObject<{
        items: import("zod").ZodRecord<import("zod").ZodString, import("zod").ZodObject<{
            connected: import("zod").ZodBoolean;
        }, import("zod/v4/core").$strip>>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * GET /tool-providers/:providerId/connections — Existing provider connections
 * scoped to a toolkit. Admin callers can pass `authorId` and `scope` filters;
 * non-admins always see only their own + shared rows.
 */
export declare const LIST_TOOL_PROVIDER_CONNECTIONS_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers/:providerId/connections";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
    } & {
        toolkit: string;
        authorId?: string | undefined;
        scope?: "shared" | "per-author" | "caller-supplied" | undefined;
        page?: number | undefined;
        perPage?: number | undefined;
    } & import("../server-adapter").ServerContext): Promise<{
        items: {
            connectionId: string;
            status: "failed" | "pending" | "active" | "inactive";
            createdAt?: string | undefined;
            label?: string | null | undefined;
            authorId?: string | undefined;
            scope?: "shared" | "per-author" | "caller-supplied" | undefined;
        }[];
        pagination?: {
            hasMore: boolean;
            total?: number | undefined;
            page?: number | undefined;
            perPage?: number | undefined;
        } | undefined;
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, import("zod").ZodObject<{
        toolkit: import("zod").ZodString;
        authorId: import("zod").ZodOptional<import("zod").ZodString>;
        scope: import("zod").ZodOptional<import("zod").ZodEnum<{
            shared: "shared";
            "per-author": "per-author";
            "caller-supplied": "caller-supplied";
        }>>;
        page: import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>;
        perPage: import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>;
    }, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
        items: import("zod").ZodArray<import("zod").ZodObject<{
            connectionId: import("zod").ZodString;
            status: import("zod").ZodEnum<{
                failed: "failed";
                pending: "pending";
                active: "active";
                inactive: "inactive";
            }>;
            createdAt: import("zod").ZodOptional<import("zod").ZodString>;
            label: import("zod").ZodOptional<import("zod").ZodNullable<import("zod").ZodString>>;
            authorId: import("zod").ZodOptional<import("zod").ZodString>;
            scope: import("zod").ZodOptional<import("zod").ZodEnum<{
                shared: "shared";
                "per-author": "per-author";
                "caller-supplied": "caller-supplied";
            }>>;
        }, import("zod/v4/core").$strip>>;
        pagination: import("zod").ZodOptional<import("zod").ZodObject<{
            total: import("zod").ZodOptional<import("zod").ZodNumber>;
            page: import("zod").ZodOptional<import("zod").ZodNumber>;
            perPage: import("zod").ZodOptional<import("zod").ZodNumber>;
            hasMore: import("zod").ZodBoolean;
        }, import("zod/v4/core").$strip>>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * GET /tool-providers/:providerId/connection-fields — Dynamic auth fields
 * the picker should collect before authorize (e.g. Confluence subdomain).
 */
export declare const LIST_TOOL_PROVIDER_CONNECTION_FIELDS_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers/:providerId/connection-fields";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
    } & {
        toolkit: string;
    } & import("../server-adapter").ServerContext): Promise<{
        fields: {
            name: string;
            type: "string" | "number" | "boolean";
            required: boolean;
            displayName?: string | undefined;
            description?: string | undefined;
            default?: unknown;
        }[];
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, import("zod").ZodObject<{
        toolkit: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
        fields: import("zod").ZodArray<import("zod").ZodObject<{
            name: import("zod").ZodString;
            displayName: import("zod").ZodOptional<import("zod").ZodString>;
            description: import("zod").ZodOptional<import("zod").ZodString>;
            type: import("zod").ZodEnum<{
                string: "string";
                number: "number";
                boolean: "boolean";
            }>;
            required: import("zod").ZodBoolean;
            default: import("zod").ZodOptional<import("zod").ZodUnknown>;
        }, import("zod/v4/core").$strip>>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * DELETE /tool-providers/:providerId/connections/:connectionId — Disconnect.
 * Without `?force=true` rejects when the connection is still pinned by any
 * agent. With `?force=true` revokes at the provider (best-effort) and drops
 * the persisted row.
 */
export declare const DISCONNECT_TOOL_PROVIDER_CONNECTION_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "DELETE";
    path: "/tool-providers/:providerId/connections/:connectionId";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
        connectionId: string;
    } & {
        force?: boolean | "true" | "false" | undefined;
        toolkit?: string | undefined;
    } & import("../server-adapter").ServerContext): Promise<{
        ok: true;
        revoked: boolean;
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
        connectionId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, import("zod").ZodObject<{
        force: import("zod").ZodOptional<import("zod").ZodUnion<readonly [import("zod").ZodBoolean, import("zod").ZodEnum<{
            true: "true";
            false: "false";
        }>]>>;
        toolkit: import("zod").ZodOptional<import("zod").ZodString>;
    }, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
        ok: import("zod").ZodLiteral<true>;
        revoked: import("zod").ZodBoolean;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * PATCH /tool-providers/:providerId/connections/:connectionId — Update a
 * connection's persisted display label. Idempotent. Ownership-gated the same
 * way as DISCONNECT: only the connection owner (or an admin) may rename,
 * unless the row is `scope: 'shared'`.
 *
 * Pass `label: null` (or an empty string) to clear the existing label.
 */
export declare const UPDATE_TOOL_PROVIDER_CONNECTION_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "PATCH";
    path: "/tool-providers/:providerId/connections/:connectionId";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
        connectionId: string;
    } & {
        label: string | null;
    } & import("../server-adapter").ServerContext): Promise<{
        ok: true;
        label: string | null;
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
        connectionId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
        label: import("zod").ZodUnion<readonly [import("zod").ZodString, import("zod").ZodLiteral<"">, import("zod").ZodNull]>;
    }, import("zod/v4/core").$strip>, import("zod").ZodObject<{
        ok: import("zod").ZodLiteral<true>;
        label: import("zod").ZodNullable<import("zod").ZodString>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * GET /tool-providers/:providerId/connections/:connectionId/usage — Lists agents
 * that currently pin the given connection in their `toolProviders` config.
 */
export declare const GET_TOOL_PROVIDER_CONNECTION_USAGE_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers/:providerId/connections/:connectionId/usage";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
        connectionId: string;
    } & {
        toolkit?: string | undefined;
    } & import("../server-adapter").ServerContext): Promise<{
        agents: {
            id: string;
            name: string;
        }[];
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
        connectionId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, import("zod").ZodObject<{
        toolkit: import("zod").ZodOptional<import("zod").ZodString>;
    }, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
        agents: import("zod").ZodArray<import("zod").ZodObject<{
            id: import("zod").ZodString;
            name: import("zod").ZodString;
        }, import("zod/v4/core").$strip>>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
/**
 * GET /tool-providers/:providerId/health — Provider-level health check.
 */
export declare const GET_TOOL_PROVIDER_HEALTH_ROUTE: Omit<import("@mastra/core/server").ApiRoute, "path" | "method" | "handler" | "createHandler" | "openapi"> & {
    method: "GET";
    path: "/tool-providers/:providerId/health";
    responseType: "json";
    streamFormat?: "sse" | "stream";
    sseFlushOnConnect?: boolean;
    handler(params: {
        providerId: string;
    } & import("../server-adapter").ServerContext): Promise<{
        ok: boolean;
        message?: string | undefined;
        details?: Record<string, unknown> | undefined;
    }>;
    pathParamSchema?: import("zod").ZodType;
    queryParamSchema?: import("zod").ZodType;
    bodySchema?: import("zod").ZodType;
    responseSchema?: import("zod").ZodType;
    openapi?: import("../server-adapter/openapi-utils").OpenAPIRoute;
    maxBodySize?: number;
    deprecated?: boolean;
    requiresPermission?: import("@mastra/core/auth").MastraFGAPermissionInput | import("@mastra/core/auth").MastraFGAPermissionInput[];
    fga?: import("@mastra/core/auth").FGARouteConfig;
    onValidationError?: import("@mastra/core/server").ValidationErrorHook;
    readonly __schemas?: import("../server-adapter").RouteSchemas<import("zod").ZodObject<{
        providerId: import("zod").ZodString;
    }, import("zod/v4/core").$strip>, undefined, undefined, import("zod").ZodObject<{
        ok: import("zod").ZodBoolean;
        message: import("zod").ZodOptional<import("zod").ZodString>;
        details: import("zod").ZodOptional<import("zod").ZodRecord<import("zod").ZodString, import("zod").ZodUnknown>>;
    }, import("zod/v4/core").$strip>> | undefined;
} & {
    readonly _mastraSchemaRoute: true;
};
//# sourceMappingURL=tool-providers.d.ts.map