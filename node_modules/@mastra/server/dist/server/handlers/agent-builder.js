...
import { b as workflowRunResultSchema, g as workflowInfoSchema, i as listWorkflowsResponseSchema, m as workflowExecutionResultSchema, n as createWorkflowRunResponseSchema, p as workflowControlResponseSchema, r as listWorkflowRunsQuerySchema, x as workflowRunsResponseSchema, y as workflowRunResultQuerySchema } from "../../workflows-B2ooCiwN.js";
import { a as streamAgentBuilderBodySchema, i as startAsyncAgentBuilderBodySchema, n as actionRunPathParams, o as streamLegacyAgentBuilderBodySchema, r as resumeAgentBuilderBodySchema, t as actionIdPathParams } from "../../agent-builder-DNTFty1u.js";
import { K as streamResponseSchema } from "../../agents-BetrUB_N.js";
import { n as createRoute } from "../../route-builder-YQ_gLIvW.js";
import { t as WorkflowRegistry } from "../../utils-B2vd77Fi.js";
import { handleError } from "./error.js";
import { CANCEL_WORKFLOW_RUN_ROUTE, CREATE_WORKFLOW_RUN_ROUTE, GET_WORKFLOW_BY_ID_ROUTE, GET_WORKFLOW_RUN_BY_ID_ROUTE, LIST_WORKFLOWS_ROUTE, LIST_WORKFLOW_RUNS_ROUTE, OBSERVE_STREAM_LEGACY_WORKFLOW_ROUTE, OBSERVE_STREAM_WORKFLOW_ROUTE, RESUME_ASYNC_WORKFLOW_ROUTE, RESUME_NO_WAIT_WORKFLOW_ROUTE, RESUME_STREAM_WORKFLOW_ROUTE, RESUME_WORKFLOW_ROUTE, START_ASYNC_WORKFLOW_ROUTE, START_WORKFLOW_RUN_ROUTE, STREAM_LEGACY_WORKFLOW_ROUTE, STREAM_WORKFLOW_ROUTE } from "./workflows.js";
//#region src/server/handlers/agent-builder.ts
var agent_builder_exports = /* @__PURE__ */ __exportAll({
...
async function loadAgentBuilderWorkflows() {
	agentBuilderWorkflowsPromise ??= import("../../dist-Cw-T1gX6.js").then((mod) => mod.agentBuilderWorkflows);
...
const LIST_AGENT_BUILDER_ACTIONS_ROUTE = createRoute({
	method: "GET",
	path: "/agent-builder",
	responseType: "json",
...
const GET_AGENT_BUILDER_ACTION_BY_ID_ROUTE = createRoute({
	method: "GET",
	...
	handler: async (ctx) => {
		...
		try {
			...
			if (actionId && !WorkflowRegistry.isAgentBuilderWorkflow(actionId)) throw new HTTPException(400, { message: `Invalid agent-builder action: ${actionId}. Valid actions are: ${Object.keys(agentBuilderWorkflows).join(", ")}` });
			logger.info("Getting agent builder action by ID", { actionId });
			return await GET_WORKFLOW_BY_ID_ROUTE.handler({
				...ctx,
				workflowId: actionId
			});
		} catch (error) {
...
const LIST_AGENT_BUILDER_ACTION_RUNS_ROUTE = createRoute({
	method: "GET",
	...
	handler: async (ctx) => {
		...
		try {
			await registerAgentBuilderWorkflows(mastra);
			if (actionId && !WorkflowRegistry.isAgentBuilderWorkflow(actionId)) throw new HTTPException(400, { message: `Invalid agent-builder action: ${actionId}` });
...
const GET_AGENT_BUILDER_ACTION_RUN_BY_ID_ROUTE = createRoute({
	method: "GET",
...
const CREATE_AGENT_BUILDER_ACTION_RUN_ROUTE = createRoute({
	method: "POST",
...
const STREAM_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
...
const START_ASYNC_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
...
const START_AGENT_BUILDER_ACTION_RUN_ROUTE = createRoute({
	method: "POST",
...
const OBSERVE_STREAM_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
...
const RESUME_ASYNC_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
...
*/
const RESUME_NO_WAIT_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
...
const RESUME_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
...
const RESUME_STREAM_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
...
const CANCEL_AGENT_BUILDER_ACTION_RUN_ROUTE = createRoute({
	method: "POST",
...
const STREAM_LEGACY_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
...
const OBSERVE_STREAM_LEGACY_AGENT_BUILDER_ACTION_ROUTE = createRoute({
	method: "POST",
