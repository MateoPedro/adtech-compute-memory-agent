...
import { o as optionalRunIdSchema } from "../../common-5ZZuR6wj.js";
import { W as serializedToolSchema, X as toolIdPathParams, _ as executeToolBodySchema, l as agentToolPathParams, v as executeToolContextBodySchema, w as listToolsResponseSchema, y as executeToolResponseSchema } from "../../agents-BetrUB_N.js";
...
import { l as stringify } from "../../utils-B2vd77Fi.js";
import { handleError } from "./error.js";
import { c as validateBody, d as MastraFGAPermissions } from "../../utils-Hz8Hc2cd.js";
import { stripInjectedToolOverrideFields } from "./tool-schema-overrides.js";
import { getAgentFromSystem } from "./agents.js";
import { isProviderDefinedTool, isVercelTool } from "@mastra/core/tools";
...
const LIST_TOOLS_ROUTE = createRoute({
	method: "GET",
	path: "/tools",
	responseType: "json",
...
const GET_TOOL_BY_ID_ROUTE = createRoute({
	method: "GET",
	path: "/tools/:toolId",
	responseType: "json",
...
const EXECUTE_TOOL_ROUTE = createRoute({
	method: "POST",
	...
	handler: async ({ mastra, runId, toolId, registeredTools, requestContext, ...bodyParams }) => {
		try {
			if (!toolId) throw new HTTPException(400, { message: "Tool ID is required" });
			let tool;
			if (registeredTools && Object.keys(registeredTools).length > 0) tool = Object.values(registeredTools).find((t) => t.id === toolId);
			if (!tool) try {
				tool = mastra.getToolById(toolId);
			...
			if (!tool) tool = await findToolInAgents(mastra, toolId, requestContext);
			if (!tool) throw new HTTPException(404, { message: "Tool not found" });
			if (!tool?.execute) throw new HTTPException(400, { message: "Tool is not executable" });
			const { data } = bodyParams;
			validateBody({ data });
			let result;
			if (isVercelTool(tool)) result = await tool.execute(data);
...
const GET_AGENT_TOOL_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ mastra, agentId, toolId, requestContext }) => {
		try {
			...
			const agentTools = await (await getAgentFromSystem({
				mastra,
				agentId
...
const EXECUTE_AGENT_TOOL_ROUTE = createRoute({
	method: "POST",
...
//#endregion
export { EXECUTE_AGENT_TOOL_ROUTE, EXECUTE_TOOL_ROUTE, GET_AGENT_TOOL_ROUTE, GET_TOOL_BY_ID_ROUTE, LIST_TOOLS_ROUTE, tools_exports as t };
