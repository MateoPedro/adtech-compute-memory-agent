...
import { i as getStoredResourceScope, n as assertStoredResourceScope } from "../../utils-B2vd77Fi.js";
import { handleError } from "./error.js";
import { MCP_CLIENT_SNAPSHOT_CONFIG_FIELDS, calculateChangedFields, computeVersionDiffs, createVersionWithRetry, enforceRetentionLimit, extractConfigFromVersion } from "./version-helpers.js";
import { a as createMCPClientVersionResponseSchema, c as listMCPClientVersionsQuerySchema, d as mcpClientVersionPathParams, i as createMCPClientVersionBodySchema, l as listMCPClientVersionsResponseSchema, n as compareMCPClientVersionsQuerySchema, o as deleteMCPClientVersionResponseSchema, p as restoreMCPClientVersionResponseSchema, r as compareMCPClientVersionsResponseSchema, s as getMCPClientVersionResponseSchema, t as activateMCPClientVersionResponseSchema, u as mcpClientVersionIdPathParams } from "../../mcp-client-versions-Co8d39A6.js";
...
*/
const LIST_MCP_CLIENT_VERSIONS_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ mastra, mcpClientId, page, perPage, orderBy, requestContext }) => {
		try {
			...
			if (!mcpClient) throw new HTTPException(404, { message: `MCP client with id ${mcpClientId} not found` });
...
*/
const CREATE_MCP_CLIENT_VERSION_ROUTE = createRoute({
	method: "POST",
	path: "/stored/mcp-clients/:mcpClientId/versions",
	requiresAuth: true,
	responseType: "json",
	...
	handler: async ({ mastra, mcpClientId, changeMessage, requestContext }) => {
		try {
			...
			if (mcpClient.activeVersionId) {
				...
				if (activeVersion) currentConfig = extractConfigFromVersion(activeVersion, MCP_CLIENT_SNAPSHOT_CONFIG_FIELDS);
			}
			const latestVersion = await mcpClientStore.getLatestVersion(mcpClientId);
			if (!mcpClient.activeVersionId && latestVersion) currentConfig = extractConfigFromVersion(latestVersion, MCP_CLIENT_SNAPSHOT_CONFIG_FIELDS);
			const changedFields = calculateChangedFields(latestVersion ? extractConfigFromVersion(latestVersion, MCP_CLIENT_SNAPSHOT_CONFIG_FIELDS) : null, currentConfig);
			const { versionId } = await createVersionWithRetry(mcpClientStore, mcpClientId, "mcpClientId", currentConfig, changedFields, { changeMessage });
...
*/
const GET_MCP_CLIENT_VERSION_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ mastra, mcpClientId, versionId, requestContext }) => {
		try {
			...
			if (version.mcpClientId !== mcpClientId) throw new HTTPException(404, { message: `Version with id ${versionId} not found for MCP client ${mcpClientId}` });
			assertStoredResourceScope(await mcpClientStore.getById(mcpClientId), await getStoredResourceScope(mastra, requestContext));
			return version;
		} catch (error) {
...
*/
const ACTIVATE_MCP_CLIENT_VERSION_ROUTE = createRoute({
	method: "POST",
...
*/
const RESTORE_MCP_CLIENT_VERSION_ROUTE = createRoute({
	method: "POST",
	...
	handler: async ({ mastra, mcpClientId, versionId, requestContext }) => {
		try {
			...
			if (versionToRestore.mcpClientId !== mcpClientId) throw new HTTPException(404, { message: `Version with id ${versionId} not found for MCP client ${mcpClientId}` });
			const restoredConfig = extractConfigFromVersion(versionToRestore, MCP_CLIENT_SNAPSHOT_CONFIG_FIELDS);
			await mcpClientStore.update({
				id: mcpClientId,
				...restoredConfig
			});
			const latestVersion = await mcpClientStore.getLatestVersion(mcpClientId);
			const { versionId: newVersionId } = await createVersionWithRetry(mcpClientStore, mcpClientId, "mcpClientId", restoredConfig, calculateChangedFields(latestVersion ? extractConfigFromVersion(latestVersion, MCP_CLIENT_SNAPSHOT_CONFIG_FIELDS) : null, restoredConfig), { changeMessage: `Restored from version ${versionToRestore.versionNumber}` });
...
*/
const DELETE_MCP_CLIENT_VERSION_ROUTE = createRoute({
	method: "DELETE",
	path: "/stored/mcp-clients/:mcpClientId/versions/:versionId",
	requiresAuth: true,
	responseType: "json",
	...
	handler: async ({ mastra, mcpClientId, versionId, requestContext }) => {
		try {
			...
			if (!version) throw new HTTPException(404, { message: `Version with id ${versionId} not found` });
			if (version.mcpClientId !== mcpClientId) throw new HTTPException(404, { message: `Version with id ${versionId} not found for MCP client ${mcpClientId}` });
			if (mcpClient.activeVersionId === versionId) throw new HTTPException(400, { message: "Cannot delete the active version. Activate a different version first." });
...
*/
const COMPARE_MCP_CLIENT_VERSIONS_ROUTE = createRoute({
	method: "GET",
	...
	handler: async ({ mastra, mcpClientId, from, to, requestContext }) => {
		try {
			...
			if (fromVersion.mcpClientId !== mcpClientId) throw new HTTPException(404, { message: `Version with id ${from} not found for MCP client ${mcpClientId}` });
			...
			if (toVersion.mcpClientId !== mcpClientId) throw new HTTPException(404, { message: `Version with id ${to} not found for MCP client ${mcpClientId}` });
			return {
				diffs: computeVersionDiffs(extractConfigFromVersion(fromVersion, MCP_CLIENT_SNAPSHOT_CONFIG_FIELDS), extractConfigFromVersion(toVersion, MCP_CLIENT_SNAPSHOT_CONFIG_FIELDS)),
				fromVersion,
				toVersion
			};
		} catch (error) {
