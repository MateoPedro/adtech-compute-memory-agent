import { a as wrapSchemaForQueryParams, i as pickParams, n as createRoute, o as convertCustomRoutesToOpenAPIPaths, r as jsonQueryParam, s as generateOpenAPIDocument, t as createPublicRoute } from "../../route-builder-YQ_gLIvW.js";
import { o as normalizeRoutePath, t as WorkflowRegistry } from "../../utils-B2vd77Fi.js";
...
function getStoredResourceRouteFGAConfig(path, permission) {
	...
	return {
		...
		resourceId: (params, { requestContext }) => {
			...
			const scopedResourceId = requestContext?.get(MASTRA_RESOURCE_ID_KEY$1);
...
*/
var MastraServer = class MastraServer extends MastraServerBase {
	mastra;
	bodyLimitOptions;
	tools;
	prefix;
	...
	*/
	...
	applyRequestMetadataToContext({ requestContext, getHeader }) {
		if (isStudioClientTypeHeader(getHeader("x-mastra-client-type"))) requestContext.set(MASTRA_IS_STUDIO_KEY, true);
	}
	/**
	...
	*/
	getFrameworkPublicMatcher() {
		...
		return (path, method) => {
			const upperMethod = method.toUpperCase();
			for (const r of publicRoutes) if ((r.method === upperMethod || r.method === "ALL") && pathMatchesPattern(path, r.pattern)) return true;
...
Learn more: https://github.com/mastra-ai/mastra/blob/main/ee/LICENSE`);
	...
	async writeCustomRouteResponse(response, nodeRes, signal) {
		...
		if (response.body) {
			...
			try {
				...
				if (signal) await pipeline(responseStream, nodeRes, { signal });
				else await pipeline(responseStream, nodeRes);
			} catch (error) {
	...
	*/
	...
	async registerRoutes() {
		for (const route of SERVER_ROUTES) await this.registerRoute(this.app, route, { prefix: this.prefix });
...
//#endregion
export { MASTRA_AUTH_MODE_KEY, MASTRA_CLIENT_TYPE_HEADER, MASTRA_FRAMEWORK_PUBLIC_KEY, MASTRA_IS_STUDIO_KEY, MASTRA_STUDIO_CLIENT_TYPE, MastraServer, SERVER_ROUTES, WorkflowRegistry, applyMcpRequestAuth, buildMcpAuthInfoFromRequestContext, checkRouteFGA, createPublicRoute, createRoute, deriveAction, derivePermission, extractResource, generateOpenAPIDocument, getEffectivePermission, isReservedRequestContextKey, isStudioClientTypeHeader, isZodError, jsonQueryParam, normalizeQueryParams, normalizeRoutePath, pickParams, redactStreamChunk, serializeStreamChunk, wrapSchemaForQueryParams };
